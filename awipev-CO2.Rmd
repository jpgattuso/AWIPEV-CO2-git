...
#plot of S/T
poster_temp <- ggplot(data = d_all%>%dplyr::filter(temp_fb<10 ), aes(x=datetime,y=temp_fb)) +
...
poster_sal <- ggplot(data = d_all%>%dplyr::filter(sal_fb>30 ), aes(x=datetime,y=sal_fb)) +
  geom_point(col="blue", size=0.5 )+
...
p_at_sal <- ggplot(data = d_all%>%dplyr::filter(at_calc > 2000), aes(x=datetime,y=at_calc,col="TA calculated from salinity") ) +
  geom_point(col="blue", size=0.5 )+
  geom_point(aes(y=at, fill= "Reference measured samples"), col="red") +
  labs(x = "Time", y = "Calculated TA from salinity (µmoles/kg)") +
  scale_x_datetime(date_minor_breaks ="1 month")+
  theme_bw() + theme(legend.position="bottom",legend.title = element_blank())
print(p_at_sal)
