...
## Impact K1K2  {.tabset .tabset-pills}
### Lueker vs Sulpis
...
# save elements for k1k2 report
k1k2_at_ct <- list(mean_discrete = mean_discrete, const_choice = const_choice, tmp2 = tmp2,
             errors_pco2_at = errors_pco2_at, diff_table = diff_table, 
             diff_ls_quant = diff_ls_quant)
save(k1k2_at_ct, file="reports/k1k2_at_ct.RData")
```
Here we compare pH measured spectrophotometrically with pH calculated from AT and CT using various formulations of K1 and K2. All pH values were normalized to a temperature of 4°C. The absolute differences are generally large, up to `r round(max(abs(diff_table[, 2:5])), digits=3)` pH units. In general, all formulations overestimate spectrophotometric pH. pH calculated using the formulation of Papadimitriou et al. (2018) is closer to measured pH, with a mean difference of `r round(diff_table[4, 4], digits=3)` pH units. This difference is `r abs(round(diff_table[4, 4]/errors_at_ct$pH, digits=1))` times larger than the uncertainty for pH calculated from AT and CT according to seacarb (`r round(errors_at_ct$pH, digits=3)`).
...
pangaea <- as_tibble(d_all) %>% 
  dplyr::select(datetime, pressure_insitu_ctd, sal_insitu_ctd, sal_fb, sal_mix, temp_insitu_11m, temp_insitu_ctd, temp_fb, temp_sf, temp_dur, at, ct, ph_s_sf_temp_insi, ph_s_dur_temp_fb, ph_calc_fb, pco2_fb, pco2_calc_fb, at_calc, ph_dur_corr, pHint_tot_sf) %>%
