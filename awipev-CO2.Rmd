...
deconv_dat <- tibble(sampling_date=deconv_tmp$sampling_date, mAT=deconv_monthly$at_calc[deconv_tmp$monthd]*1e-6, mCT=deconv_monthly$ct_calc[deconv_tmp$monthd]*1e-6, mS=deconv_monthly$sal_fb[deconv_tmp$monthd], mT=deconv_monthly$temp_fb[deconv_tmp$monthd], P=deconv_tmp$depth/10, Pt=rep("NA", length.out=nrow(deconv_tmp)), Sit=mean0m$mean_Sit, AT=mean0m$mean_ta*1e-6, CT=mean0m$mean_dic*1e-6, S=mean0m$mean_salinity, T=mean0m$mean_temperature)
