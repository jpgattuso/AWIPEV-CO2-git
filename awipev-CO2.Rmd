...
#restore datetime and date which are mangled by the murtates above
...
d_all <- full_join(d_all, discrete2merge, by="datetime")
d_all <- d_all %>% 
  dplyr::mutate(ph_s_sf_temp_mix = ifelse(n_depth_ctd == 4, ph_s_sf_temp_mix, NA)) %>% # eliminate ph spectro data when the profiler was at several depth during the same hour
  dplyr::mutate(ph_s_sf_temp_mix = ifelse(!is.na(temp_insitu_ctd_7m) | !is.na(temp_insitu_ctd_9m), 
                                          ph_s_sf_temp_mix, NA)) # only use ph spectro data when profiler was below 6 m
...
d_all <- d_all %>%
  dplyr::mutate(
    sal_mix_interp =  ifelse(
      datetime > as.POSIXct("2018-03-09 00:00:00") &
        datetime < as.POSIXct("2018-04-16 16:00:00"),
      sal_mix_interp,
      sal_mix
    )
  )
