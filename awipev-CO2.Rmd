...
ggreg <- function (xdata, ydata, type = "OLS", plot = TRUE, title = "title", xlab = "x", 
                   ylab = "y", proplot = TRUE, ...) 
...
print(reg$plot)
pco2_calc_meas <- reg$plot
ggsave(filename = "figures/pco2_calc_meas.png", plot = reg$plot)
```
...
offset_ph <- ph_sf %>% 
  ...
  #scale_x_datetime(date_breaks="years", date_minor_breaks="3 months", labels=date_format("%b %Y")) +
  Mytheme() + 
  #theme(panel.grid.major.x = element_blank(), axis.text.x = element_text(angle = 45, hjust = 1)) +
scale_colour_discrete(guide="none")
print(offset_ph)
ggsave(filename = "figures/offset_ph.png", plot = offset_ph, width = 25, height = 20, units = "cm")
```
...
Initially (before 2022-08-09), there was 2 outiers on 209-07-12 and 2018+09-11. On 2022-08-09, code was addeed above to put a no good qf (3) in order to eliminate those two oiutliers. Now we are fine. The average offset is 'r average_offset'.
...
reg <- ggreg(
  ...
  type = "MA",
  plot = TRUE,
  title = "pH spectro vs pH seafet (at in situ temperature)",
  xlab = "pH spectro",
  ylab = "pH seafet",
  proplot = TRUE
)
