...
discrete_is_na$pco2_calc_insitu <- NA
discrete <- bind_rows(discrete_no_na, discrete_is_na) %>% 
  dplyr::arrange(date)
...
# pH calculated in situ
discrete2merge <- discrete %>%
  dplyr::select(closest_datetime, phcalc_insitu) %>%
  dplyr::filter(!is.na(closest_datetime)) %>%
  rename(datetime=closest_datetime)
d_all <- full_join(d_all, discrete2merge, by="datetime")
...
# calculated pCO2 
discrete2merge <- discrete %>%
  dplyr::select(closest_datetime, pco2_calc_insitu) %>%
  dplyr::filter(!is.na(closest_datetime)) %>%
  rename(datetime=closest_datetime)
d_all <- full_join(d_all, discrete2merge, by="datetime")
...
fit <- lmodel2(data = d_all,  pco2_corr ~ pco2_calc_insitu , nperm = 99)

p <-  ggplot(d_all,aes(x=pco2_calc_insitu, y= pco2_corr, label = month))+
   geom_text(aes(color = factor(month)))+
  scale_color_discrete(guide="none")+
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  geom_abline(aes(intercept = fit$regression.results[2,2], slope = fit$regression.results[2,3]),colour = "blue") +
...
print(p) # pco2_calc_insitu_pco2_corr_text
```
...
pco2_contros_xts <-
  dplyr::select(d_all, datetime, pco2_calc_insitu, pco2_corr)
pco2_contros_xts <-
...
* pco2_calc_insitu: pco2 calc from AT/CT ferrybox = carb avec temp_fb +sal_fb
* calibrate seafet pH
