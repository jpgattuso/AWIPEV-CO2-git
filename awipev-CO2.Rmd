...
g <- g + plot_annotation(tag_levels = 'A')
ggsave(filename = "figures/ts_boxplots.png", plot = g, width = 15, height = 30, units = "cm")
rm(list = ls(pattern = "ts_")) #delete big ggplot objects
gc() #garbage collection
...
Salinity <- round(quantile(d_all$sal_fb, na.rm =TRUE), digits=1)
Temperature <- round(quantile(d_all$temp_insitu_11m, na.rm =TRUE), digits=1)
pCO2 <- round(quantile(d_all$pco2_fb, na.rm =TRUE), digits=1)
pH <- round(quantile(d_all$pHint_tot_sf, na.rm =TRUE), digits=3)
AT <- round(quantile(d_all$at_calc, na.rm =TRUE), digits=1)
