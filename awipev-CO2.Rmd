...
sal_delta_plt <- tmp_delta %>% 
  ...
  labs(y = "Difference in median salinity", x = "") +
  geom_col(fill="blue") +
...
temp_delta_plt <- tmp_delta %>% 
  ...
  labs(y = "Difference in median temperature", x = "") +
  geom_col(fill="blue") +
  geom_hline(yintercept = 0) +
...
den_delta_plt <- tmp_delta %>% 
  ...
  labs(y = "Difference in median density", x = "") +
  geom_col(fill="blue") +
  geom_hline(yintercept = 0) +
...
pH_delta_plt <- tmp_delta %>% 
  ...
  labs(y = "Difference in median pH at in situ temperature", x = "Month") +
  geom_col(fill="blue") +
  geom_hline(yintercept = 0) +
  annotate("text", x = 1.5, y = 0.1, label = substitute("pH"["surface"] ~ "> pH"["deep"])) +
  annotate("text", x = 1.5, y = -0.1, label = substitute("pH"["surface"] ~ "< pH"["deep"])) +
  Mytheme(size_labs = 12) +
  theme(panel.grid.major.x = element_blank(), panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank())

deltas <- sal_delta_plt / temp_delta_plt / den_delta_plt
