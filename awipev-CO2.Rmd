...
d2 <- d_all %>% 
  dplyr::select(montht, pco2_inst, phINT)
count_pco2_inst_no_NA <- sum(!is.na(d2$pco2_inst)) # total non-NA pCO2 values
...
d2_summary <- d2 %>% 
  ...
  dplyr::summarise(pco2_pc = round(100 * sum(!is.na(pco2_inst))/count_pco2_inst_no_NA, digits = 1),
...
# 
# d_ts <- d %>%
#   dplyr::select(datetime, temp_insitu_11m, sal_fb, par_air, pco2_inst,
...
#   ggplot() +
#   scale_x_datetime(breaks = date_breaks("1 year"), labels = date_format("%Y")) +
#   geom_point(aes(x = datetime, y = pco2_inst), 
