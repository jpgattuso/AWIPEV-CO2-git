...
fit <- lmodel2(data = d_all,   pco2_calc_fb_insi_temp ~ pco2_fb_insi_temp, nperm = 99)
print(fit)
p <-  ggplot(d_all,aes(x = pco2_fb, y = pco2_calc_fb))+
  geom_point()+
  scale_color_discrete(guide="none")+
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  ...
  #                    "; P =", signif(fit$P.param, 3))) +
  theme(aspect.ratio=1, plot.title = element_text(size=7)) + 
  coord_fixed(ratio = 1, xlim=c(200,420), ylim=c(200,420))+
  labs(title = "", x="Measured pCO2", y="Calculated pCO2") + theme_bw()  
print(p)
