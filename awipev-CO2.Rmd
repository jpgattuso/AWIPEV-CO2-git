...
#   labs(x="Salinity", y=latex2exp::TeX("Total alkalinity ($\\mu mol\\phantom{x}kg^{-1})$"))  +
#   stat_smooth(method = "lm", se=FALSE, col = "blue", size = 0.5) +
#   Mytheme(size_labs = 9)
# ggsave(filename = "figures/at_s.png", plot = at_s, width = 7, height = 7, units = "cm")
...
```{r plot Discrete salinity vs TA, eval=FALSE, echo=FALSE, warning=FALSE, message=FALSE, out.width="100%", fig.align="center"}
# PLOT
# print(p_at_s)
# rm(p_at_s)
```
...
pHspec_pHcalc <- reg$plot + xlim(c(8.05, 8.20)) + ylim(c(8.05, 8.20))
p
#print(reg$plot + xlim(c(8, 8.25)) + ylim(c(8, 8.25)))
...
pHspec_pHcalc <- d_all %>% 
  ...
  theme_bw() + Mytheme(size_labs = 9) + theme(aspect.ratio = 1) +
  coord_fixed(ratio = 1, expand = TRUE, clip = "on")
ggsave(filename = "figures/pHspec_pHcalc.png", plot = pHspec_pHcalc, width = 25, height = 25, units = "cm")
```
...
#  annotate("text", x = 2, y = 0.65, label = substitute("S"["surface"] ~ "> S"["deep"])) +
#  annotate("text", x = 2, y = -0.9, label = substitute("S"["surface"] ~ "< S"["deep"])) +
...
temp_delta_plt <- tmp_delta %>% 
  ...
  geom_col(fill="blue") +
  geom_hline(yintercept = 0) +
  annotate("text", x = 2, y = 0.3, parse = TRUE, label = "S['surface'] > S['deep']") +
  annotate("text", x = 2, y = -1.8, parse = TRUE, label = "S['surface'] <  S['deep']") +
  # annotate("text", x = 2, y = 0.3, label = substitute("T"["surface"] ~ "> T"["deep"])) +
  # annotate("text", x = 2, y = -1.8, label = substitute("T"["surface"] ~ "< T"["deep"])) +
  Mytheme(size_labs = 12) +
  theme(panel.grid.major.x = element_blank(), panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank())
...
den_delta_plt <- tmp_delta %>% 
  ...
  geom_col(fill="blue") +
  geom_hline(yintercept = 0) +
  annotate("text", x = 2, y = 0.25, parse = TRUE, label = "S['surface'] > S['deep']") +
  annotate("text", x = 2, y = -1.2, parse = TRUE, label = "S['surface'] <  S['deep']") +
  # annotate("text", x = 2, y = 0.25, label = substitute("D"["surface"] ~ "> D"["deep"])) +
  # annotate("text", x = 2, y = -1.2, label = substitute("D"["surface"] ~ "< D"["deep"])) +
  Mytheme(size_labs = 12) +
  theme(panel.grid.major.x = element_blank(), panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank())
print(den_delta_plt)
...
pHinsi_delta_plt <- tmp_delta %>% 
  ...
  geom_col(fill="blue") +
  geom_hline(yintercept = 0) +
  annotate("text", x = 1.5, y = 0.02, parse = TRUE, label = "S['surface'] > S['deep']") +
  annotate("text", x = 1.5, y = -0.06, parse = TRUE, label = "S['surface'] <  S['deep']") +
...
pH_delta_plt <- tidyr::drop_na(tmp_delta) %>% 
  ...
  geom_col(fill="blue") +
  geom_hline(yintercept = 0) +
  annotate("text", x = 1.5, y = 0.8, parse = TRUE, label = "S['surface'] > S['deep']") +
  annotate("text", x = 1.5, y = -0.15, parse = TRUE, label = "S['surface'] <  S['deep']") +
...
```{r, echo=FALSE, message= FALSE, warning=FALSE}
g <- (at_s | pco2_calc_meas) / (offset_ph | pHspec_pHcalc)
