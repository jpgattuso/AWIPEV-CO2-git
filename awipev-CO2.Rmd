...
d_hour <- d_hour %>%
  ...
  dplyr::rename(at_contros=AT,pco2_contros=PCO2_corr_contros_filtered,pH_durafet=HW_pH1_filtered,temp_insitu=Temp_SBE38_filtered, sal_fb=Salinity_filtered,temp_fb=Temperature_filtered)
```
...
# #plot of S/T
# poster_temp <- ggplot(data = d_all%>%dplyr::filter(temp_fb<10 &sal_fb>30 )) + geom_point(aes(x=datetime,y=temp_fb),col="blue", size=0.5) +
#   geom_point(aes(x=datetime,y=sal_fb/5),col="red", size=0.5 )+
...
# #
# plot(sal_fb ~ datetime,d_all,pch=16, axes=T, ylim=c(30.5,36), cex=0.5, col="red", ylab="", xlab="", yaxt ="n", lty=1, cex.axis=1.8) +
...
# #
# poster_sal <- ggplot(data = d_all%>%dplyr::filter(sal_fb>30 ), aes(x=datetime,y=sal_fb), colour="sal_fb") +
...
####
seafet_calib <- sf_calc(calEint = d_all$voltINT, calEext = d_all$voltEXT, E0int25 = d_all$E0int25_sf,  E0ext25 = d_all$E0ext25_sf, calT = d_all$temp_insitu, calSal = d_all$sal_fb_interp)
...
# # #@@@@@ test plot #@@@@@ #@@@@@ #@@@@@
# seafet_contros_xts <- dplyr::select(d_all, datetime, voltINT, phINT_filtered,pH_s_seafet_temp_field, T_seaF, temp_fb, temp_insitu, sal_fb)
# #seafet_contros_xts <- dplyr::select(d_all, datetime,voltINT)
# seafet_contros_xts <- as.xts(seafet_contros_xts, order.by = d_all$datetime)
# dygraph(seafet_contros_xts, group = "awipev", main="pH seaFET (in situ)", ylab="pHT") %>%
...
#   dySeries("temp_fb", label = "temp_fb", color ="orange",axis= "y2", strokeWidth=0, pointSize=2) %>%
# 
#   dySeries("sal_fb", label = "sal_fb", color ="pink",axis= "y2", strokeWidth=0, pointSize=2) %>%
...
range_month <-
  ...
  summarize(Tinsi_max =round( max(temp_insitu, na.rm=TRUE), digits = 2),
            ...
            sal_max = round(max(sal_fb, na.rm=TRUE), digits = 2),
            sal_min = round(min(sal_fb, na.rm=TRUE), digits = 2),
...
pH_sf <- d_all %>%
  ...
  dplyr::filter(!is.na(pHint_tot_sf), !is.na(temp_insitu), !is.na(temp_fb), !is.na(sal_fb)) %>%
...
pH_dur <- d_all %>%
  ...
  dplyr::filter(!is.na(pH_durafet_corrected), !is.na(temp_insitu), !is.na(temp_fb), !is.na(sal_fb))
