...
d_all$sal_mix_interp <- ifelse(d_all$datetime > "2018-03-09 00:00:00" & d_all$datetime < "2018-04-16 16:00:00", d_all$sal_mix_interp, d_all$sal_mix)
...
calib_spectro <- sf_calib(calEint = d_all$voltINT, calEext = d_all$voltEXT, calpH = d_all$pH_s_seafet_temp_insitu,
                          calT = d_all$temp_mix, calSal = d_all$sal_mix )
...
####
seafet_calib <- sf_calc(calEint = d_all$voltINT, calEext = d_all$voltEXT, E0int25 = d_all$E0int25_sf,  E0ext25 = d_all$E0ext25_sf, calT = d_all$temp_mix, calSal = d_all$sal_mix_interp)
...
range_month <-
  ...
  #summarize(pH_range_month = mean(pH_range, na.rm=TRUE))%>%
  summarize(Tinsi_max =round( max(temp_mix, na.rm=TRUE), digits = 2),
            Tinsi_min = round(min(temp_mix, na.rm=TRUE), digits = 2),
            sal_max = round(max(sal_mix, na.rm=TRUE), digits = 2),
            sal_min = round(min(sal_mix, na.rm=TRUE), digits = 2),
...
pH_sf <- d_all %>%
  dplyr::select(datetime, sal_mix,temp_fb,temp_mix, pHint_tot_sf)%>%
  dplyr::filter(!is.na(pHint_tot_sf), !is.na(temp_mix), !is.na(temp_fb), !is.na(sal_mix)) %>%
  dplyr::filter(datetime< "2018-01-01" | datetime> "2018-02-03")
pH_sf$pHint_tot_sf_norm4 <- pHinsi(pH=pH_sf$pHint_tot_sf, ALK=2.273e-3, Tinsi=4, Tlab=pH_sf$temp_mix, Pinsi=11, S=pH_sf$sal_mix, Pt=0, Sit=0, k1k2 = "x", kf = "x", ks="d", pHscale = "T", b="u74")

pH_dur <- d_all %>%
  dplyr::select(datetime, sal_mix,temp_fb,temp_mix, pH_durafet_corrected)%>%
  dplyr::filter(!is.na(pH_durafet_corrected), !is.na(temp_mix), !is.na(temp_fb), !is.na(sal_mix))
pH_dur$pH_durafet_norm4 <- pHinsi(pH=pH_dur$pH_durafet_corrected, ALK=2.273e-3, Tinsi=4, Tlab=pH_dur$temp_mix, Pinsi=11, S=pH_dur$sal_mix, Pt=0, Sit=0, k1k2 = "x", kf = "x", ks="d", pHscale = "T", b="u74")
...
pH_calc <- discrete %>%
  dplyr::select(datetime, sal_mix,temp, phcalc, at)%>%
  dplyr::filter(!is.na(phcalc), !is.na(temp), !is.na(sal_mix))
pH_calc$pH_calc_norm4 <- pHinsi(pH=pH_calc$phcalc, ALK=pH_calc$at*1e-6, Tinsi=4, Tlab=pH_calc$temp, Pinsi=11, S=pH_calc$sal_mix, Pt=0, Sit=0, k1k2 = "x", kf = "x", ks="d", pHscale = "T", b="u74")
