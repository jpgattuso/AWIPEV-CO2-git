...
####
calib_spectro <- sf_calib(calEint = d_all$voltINT, calEext = d_all$voltEXT, calpH = d_all$ph_s_sf_temp_insi, calT = d_all$temp_sf, calSal = d_all$sal_insitu_ctd_interp)
...
```{r seaFET pH calib sf_calib 9m, echo=FALSE, message= FALSE, warning=FALSE}
###
#creaate new dataframe and CALIB to take into consideration only seafet data at the same depth than spectro (9m)
##
####
d_all_calib_depth <- d_all%>%
  ...
  dplyr::filter(depth_discrete == 9)

calib_spectro_depth_9m <- sf_calib(calEint = d_all_calib_depth$voltINT, calEext = d_all_calib_depth$voltEXT, calpH = d_all_calib_depth$ph_s_sf_temp_insi, calT = d_all_calib_depth$temp_sf, calSal = d_all_calib_depth$sal_insitu_ctd_interp)
# put number of deployment 
calib_spectro_depth_9m$deployment_sf <- d_all_calib_depth$deployment_sf 

# make the mean of E0 by period of deployement
calib_spectro_depth_9m <- group_by(calib_spectro_depth_9m, deployment_sf) %>%
  summarise(E0int25 = mean(E0int25, na.rm = TRUE),
            E0ext25 = mean(E0ext25, na.rm = TRUE) 
            )
names(calib_spectro_depth_9m) <- c("deployment_sf", "E0int25_sf_9m", "E0ext25_sf_9m")

# add E0 mean to seaFET data per deployment
d_all <-left_join(d_all, calib_spectro_depth_9m,by="deployment_sf" )
```
...
Final calibrated seaFET pHT data (green dots) are shown here after calibration with spectro measurements (red dots) according to each period of deployement, defined between 2 spectro measurements (shading grey).
...
```{r seaFET pH calculation sf_calc 9m, echo=FALSE, message= FALSE, warning=FALSE}
####
## calculation of the calibrated pH ##
##       new sf_calc function       ##
####
seafet_calib_9m <- sf_calc(calEint = d_all$voltINT, calEext = d_all$voltEXT, E0int25 = d_all$E0int25_sf_9m,  E0ext25 = d_all$E0ext25_sf_9m, calT = d_all$temp_sf, calSal = d_all$sal_insitu_ctd_interp)

# bind new calibrated pH data to the seafet data set.
names(seafet_calib_9m) <- c("pHint_tot_sf_calib9m", "pHext_tot_sf_calib9m")
##########  Despike() ###########
d_all <- d_all %>%   
  dplyr::mutate(pHint_tot_sf_calib9m = oce::despike(seafet_calib_9m$pHint_tot_sf_calib9m, reference= "median", n=1, k=5001, replace="NA"),
                pHextt_tot_sf_calib9m = oce::despike(seafet_calib_9m$pHextt_tot_sf_calib9m, reference= "median", n=1, k=5001, replace="NA")
  )

d_all <- d_all %>%
  #dplyr::mutate(pHint_tot_sf = despike(d_all$pHint_tot_sf, reference= "median", n=2, k=5761, replace="NA")) %>%
  dplyr::mutate(pHint_tot_sf_calib9m = seafet_calib_9m$pHint_tot_sf_calib9m) 

seafet_depth_xts <- dplyr::select(d_all, datetime, ph_s_sf_temp_insi, pHint_tot_sf_1m, pHint_tot_sf_3m, pHint_tot_sf_5m, pHint_tot_sf_7m, pHint_tot_sf_9m, pHint_tot_sf_calib9m)
seafet_depth_xts <- as.xts(seafet_depth_xts, order.by = d_all$datetime)
dygraph(seafet_depth_xts, group = "depth", main="pH seaFET", ylab="pHT") %>%
      dySeries("pHint_tot_sf_calib9m", label = "pH calib 9m", color =  "black", strokeWidth = 0, pointSize = 2 ) %>%
...
ph_sf <- d_all%>%
  dplyr::select(datetime, ph_s_sf_temp_insi,pHint_tot_sf,pHext_tot_sf)%>%
    dplyr::mutate( diff_spec_sf = ph_s_sf_temp_insi - pHint_tot_sf,
                   diff_sf_int_ext = pHint_tot_sf -pHext_tot_sf )
