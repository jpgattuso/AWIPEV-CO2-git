...
-   large for pH (`r round(diff_ls_quant[4, 2], digits = 3)` pH units on
    ...
    compared with the uncertainty calculated with seacarb (see above;
    `r round(errors[1, 7], digits = 3)`). In general, pH is higher with
    Sulpis than with Lueker.
...
were used to calculate pH using the formulations for K1 and K2 of Lueker
et al. (2000) and Sulpis et al. (2020). The differences induced by the
choice of the carbonic acid constants are:
...
all data (a). The mean and median difference between the two pCO2s are
...
`r round(sum(air_sea_month$f_air_sea), digits = 0)` and
`r round(sum(air_sea_corr_month$f_air_sea_corr), digits = 0)` mol/(m2
year) without and with correction, respectively.
