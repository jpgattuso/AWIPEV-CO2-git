...
tmp <- d_all %>% 
  ...
  dplyr::select(ph_s_sf_temp_4, pco2_fb, at_calc, sal_fb, temp_fb)
# Using Lueker  
carb_l <-
  carb(
    24,
...
tmp2 <- tibble(ph_s_sf_temp_4 = tmp$ph_s_sf_temp_4, pH_l = carb_l$pH, pH_s = carb_s$pH) %>% 
  tidyr::drop_na(ph_s_sf_temp_4)
# summary of differences
diff_lueker <- as.numeric(summary(tmp2$ph_s_sf_temp_4 - tmp2$pH_l))
diff_sulpis <- as.numeric(summary(tmp2$ph_s_sf_temp_4 - tmp2$pH_s))
lueker_sulpis_table <- tibble(var = c("Min", "Q1", "Median", "Mean", "Q3", "Max"), diff_lueker, diff_sulpis)
# make plots
p1 <- ggreg(xdata = tmp2$ph_s_sf_temp_4, ydata = tmp2$pH_l, type = "MA", plot = TRUE, 
...
p2 <- ggreg(xdata = tmp2$ph_s_sf_temp_4, ydata = tmp2$pH_s, type = "MA", plot = TRUE, 
