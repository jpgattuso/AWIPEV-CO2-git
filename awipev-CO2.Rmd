...
tmp <- d_all %>%
  ...
  tidyr::drop_na()
tmp$pH_dur_temp_4 <- seacarb::pHinsi(pH=tmp$pH_dur, ALK=mean(tmp$at*1e-6, na.rm=TRUE), Tinsi=4, Tlab=tmp$temp_fb, Pinsi=11/10, S=tmp$sal_fb_mix, Pt=0, Sit=0, k1k2 = "l", kf = "dg", ks="d", pHscale = "T", b="u74")
d_all <- d_all %>% 
...
pH_calc <- discrete %>%
  ...
  dplyr::filter(!is.na(ph_calc_fb), !is.na(temp_fb), !is.na(sal_fb_mix))
pH_calc$pH_calc_fb_temp_4 <- pHinsi(pH = pH_calc$ph_calc_fb, ALK=pH_calc$at*1e-6, Tinsi=4, Tlab=pH_calc$temp_fb, Pinsi=11/10, S=pH_calc$sal_fb_mix, Pt=0, Sit=0, k1k2 = "l", kf = "dg", ks="d", pHscale = "T", b="u74")
