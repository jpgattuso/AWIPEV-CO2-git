...
#                 pressure_insitu_ctd, pHint_tot_sf, ph_s_sf_temp_insi, at_calc, at) #%>%
...
# SEAFET pH
d_pH <- d_all %>% 
  dplyr::select(datetime, pressure_insitu_ctd, pHint_tot_sf) %>% tidyr::pivot_longer(-datetime, values_to = "value")
ts_pH <- d_all %>%
  ggplot(na.rm=TRUE) +
  scale_x_datetime(breaks = date_breaks("1 year"), labels = date_format("%Y")) +
  geom_point(aes(x = datetime, y = pHint_tot_sf_9m), colour="blue", alpha=alpha, na.rm=TRUE, size=size_point) + 
