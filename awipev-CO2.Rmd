...
lab <- c( expression(paste(italic(p),CO[2]," (µatm)")),
          ...
          expression(paste(italic(p),CO[2]," (ppm; NOAA)"))
          )
...
poster_pco2 <- ggplot( ) +
  ...
  geom_point() + geom_point(data=noaa%>%dplyr::filter(analysis_value > 150), aes(x=datetime, y= analysis_value, shape="lab", color ="green" ),size=0.5)+
