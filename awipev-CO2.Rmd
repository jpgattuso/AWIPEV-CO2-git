...
author: "Jean-Pierre Gattuso and Samir Alliouane and Bernard Gentili"
...
ggreg <- function (xdata, ydata, type = "OLS", plot = TRUE, title = "title", subtitle = "subtitle", xlab = "x", ylab = "y", proplot = TRUE, ...) 
...
predicted <- fit$regression.results$Intercept[1] + xdata * fit$regression.results$Slope[1]
error <- xdata - predicted
if (plot == TRUE) {
  ...
  labs(title = paste(title, 
       ...
       x = xlab,
       y = ylab) +
#    lims(x = c(min(fit_data$xdata), max(fit_data$xdata)), y = c(min(fit_data$ydata), max(fit_data$ydata))) +
    theme_bw() +
...
d_hour <- d_hour %>%
  ...
  # data post 2020-03-01 eliminated because no maintenance and lots of algae
...
d_all <- d_hour %>%
  ...
  dplyr::mutate(depth_group = as.factor(dplyr::case_when(depth > 0 & depth <= 2 ~ '0-2',
                depth > 2 & depth <= 4 ~ '2-4',
                depth > 4 & depth <= 6 ~ '4-6',
                depth > 6 & depth <= 8 ~ '6-8',
                depth > 8 & depth <= 12 ~ '>8', #8 to 12.4
                TRUE ~ 'NA'
                )),
...
d_all <- d_all %>%
  dplyr::mutate(sal_mix = ifelse(!is.na(sal_fb), sal_fb, sal_insitu_ctd_10m))
```
   
```{r Temp filter, fig.width=10, fig.height=4, echo=FALSE, message= FALSE}
# Create temp_mix: filling gaps in temp_insitu_11m using temp_insitu_10m
d_all <- d_all %>%
  dplyr::mutate(temp_mix = ifelse(!is.na(temp_insitu_11m), temp_insitu_11m, temp_insitu_ctd_10m))
```

```{r read and prepare discrete AT+CT, echo=FALSE, warning=FALSE, message=FALSE}
d <- read_excel(paste0(path, "fb_data/Discrete_analyses_AT_CT/Discrete_sampling_AWIPEV.xlsx"),
                col_names = TRUE) %>% 
#   d <- tibble(
...
# ) %>% 
  ...
  #dplyr::filter(datetime < as.POSIXct("2021-01-01 00:00:00")) %>% #eliminate 2021 data
...
d$at <- as.numeric(d$at) * 1.0002
d$ct <- as.numeric(d$ct) * 1.0002
...
# interpolation of temp_mix/sal_mix in d_all in order to have a full temperature for pHinsi()
# We take temp_mix/sal_mix because of the depth (temp11m insitu + temp10m ctd)
tempmixinterp <-approx(d_all$datetime, d_all$temp_mix, xout=d_all$datetime, method="linear", rule=2)
names(tempmixinterp) <- c("datetime", "temp_mix_interp")
tempmixinterp <- as.data.frame(tempmixinterp)
d_all$temp_mix_interp <- tempmixinterp$temp_mix_interp

salmixinterp <-approx(d_all$datetime, d_all$sal_mix, xout=d_all$datetime, method="linear", rule=2)
names(salmixinterp) <- c("datetime", "sal_mix_interp")
salmixinterp <- as.data.frame(salmixinterp)
d_all$sal_mix_interp <- salmixinterp$sal_mix_interp

# ta_xts <- dplyr::select(d_all, datetime, sal_mix, sal_mix_interp)
# ta_xts <- as.xts(ta_xts, order.by = ta_xts$datetime)
# dygraph(ta_xts, group = "awipev", main="sal mix", ylab="temp") %>%
#   dySeries("sal_mix", color = "black", pointSize =4, strokeWidth = 1, label = "ta") %>%
#   dySeries("sal_mix_interp", color = "blue", strokeWidth = 1, label = "ta interp") %>%
...
pH_seafet <- d %>%
  dplyr::filter(!is.na(ph_s_sf) & !is.na(temp_mix_interp))

pH_seafet <- pH_seafet %>% 
  dplyr::mutate(
    ph_s_sf_temp_insi = pHinsi(pH = pH_seafet$ph_s_sf, 
                         ALK = pH_seafet$at_mean_interp*1e-6, Tinsi = pH_seafet$temp_mix_interp, 
                         Tlab = pH_seafet$temp_s_lab, Pinsi = 11/10, 
                         S = pH_seafet$sal_mix_interp, Pt = 0, Sit = 0, k1k2 = "l", kf = "dg", ks="d", 
                         pHscale = "T", b="u74"),
...
pH_durafet <- pH_durafet %>% 
  dplyr::mutate(
   ...
   ph_s_dur_temp_4 = pHinsi(pH = pH_durafet$ph_s_dur, # lueker
...
#bind all spectro measurements, just to assess sd, change name of dataframe only to do the rbind.
# seafetph <- pH_seafet_mean
# colnames(seafetph) <- c("date", "mean", "sd")
# durafetph <- pH_durafet_mean
# colnames(durafetph) <- c("date", "mean", "sd")
#pH_seafet_durafet <- rbind(seafetph, durafetph)
# #calculations for all pH spectro measurement for sd
# sd_min <- min(pH_seafet_durafet$sd)
# sd_max <- max(pH_seafet_durafet$sd)
# sd_mean <- mean(pH_seafet_durafet$sd)
...
discrete <- d %>%
  group_by(datetime) %>%
#dplyr::select( datetime, date,hour,sal_fb,sal_mix_interp ,sal_mix,temp_fb, at_mean_interp,at ,ct_mean_interp,ct, temp_mix,temp_mix_interp,temp_insitu_11m) %>%
  summarise(datetime = mean(datetime, na.rm = TRUE),
    sal_fb = mean(sal_fb, na.rm = TRUE),
    sal_mix = mean(sal_mix, na.rm = TRUE),
    sal_mix_interp = mean(sal_mix_interp, na.rm = TRUE),
    temp_fb = mean(temp_fb, na.rm = TRUE),
     temp_mix = mean(temp_mix, na.rm = TRUE),
    temp_mix_interp = mean(temp_mix_interp, na.rm = TRUE),
...
# library("RColorBrewer")
# d_xts <- dplyr::select(discrete, datetime, sal_fb, sal_mix,temp_fb,temp_mix,temp_insitu_11m, at_mean_interp,at,ct_mean_interp,ct)
# d_xts <- as.xts(d_xts, order.by = d_xts$datetime)
# dygraph(d_xts, group = "awipev", main="Salinity (in FerryBox)", ylab="Salinity") %>%
#             dySeries("sal_mix", color = brewer.pal(n=9, name = "Paired")[1], strokeWidth = 1, pointSize = 4, label = "sal_mix") %>%
#             dySeries("sal_fb", color = brewer.pal(n=9, name = "Paired")[2], strokeWidth = 1, pointSize = 2, label = "sal_fb") %>%
#    dySeries("temp_mix", color = brewer.pal(n=9, name = "Paired")[3], strokeWidth = 1, pointSize = 4, label = "temp_mix") %>%
#    dySeries("temp_fb", color = brewer.pal(n=9, name = "Paired")[4], strokeWidth = 1, pointSize = 2, label = "temp_fb") %>%
#    dySeries("temp_insitu_11m", color = brewer.pal(n=9, name = "Paired")[5], strokeWidth = 1, pointSize = 2, label = "temp_insitu_11m") %>%
#     dySeries(axis="y2", "at_mean_interp", color = brewer.pal(n=9, name = "Paired")[6], strokeWidth = 1, pointSize = 4, label = "at_mean_interp") %>%
#    dySeries(axis="y2","ct_mean_interp", color =brewer.pal(n=9, name = "Paired")[7], strokeWidth = 1, pointSize = 4, label = "ct_mean_interp") %>%
#  dySeries(axis="y2","at", color = brewer.pal(n=9, name = "Paired")[8], strokeWidth = 1, pointSize = 2, label = "at") %>%
#    dySeries(axis="y2","ct", color =brewer.pal(n=9, name = "Paired")[9], strokeWidth = 1, pointSize = 2, label = "ct") %>%
#   dyHighlight(highlightCircleSize = 8,highlightSeriesBackgroundAlpha =0.2,hideOnMouseOut =TRUE) %>%
#       dyOptions( drawGrid = TRUE, drawPoints = TRUE,useDataTimezone = TRUE) %>%
#     dyLegend(show = "follow")   %>%
...
discrete_is_na <- discrete %>%
  dplyr::filter(is.na(sal_fb)  | is.na(temp_fb) | is.na(at) | is.na(ct))
carb <- carb(15, discrete_no_na$at_mean_interp*1e-6, discrete_no_na$ct_mean_interp*1e-6, S = discrete_no_na$sal_fb, T = discrete_no_na$temp_fb, P = 0, Pt = 0, Sit = 0, k1k2 = "l", kf = "dg", ks = "d", pHscale = "T", b = "u74")
...
df <- d_all %>% 
  ...
  summarize(mean1m = mean(sal_insitu_ctd_1m, na.rm = TRUE),
            ...
            range = mean(sal_insitu_ctd_1m, na.rm = TRUE) - mean(sal_insitu_ctd_10m, na.rm = TRUE)) %>% 
  ungroup()
...
rm(sal_xts)
```
Extreme differences: `r dplyr::filter(df, range < -3 | range > 2)`.

There are `r sum(!is.na(df$range))` days with salinity values at both 1 m (0-2 m) and 10 m (8-12 m). The salinity difference between 1 and 10 m ranges between `r min(df$range, na.rm=TRUE)` and `r max(df$range, na.rm=TRUE)` (mean: `r mean(df$range, na.rm=TRUE)`; `r median(df$range, na.rm=TRUE)`). The difference with depth is therefore significant.


### **Sal FB / Sal CTD 10 m **

```{r Sal comparaison, echo=FALSE, message= FALSE, warning=FALSE, out.width="80%",fig.show='hold', fig.align="center"}
lm <- lm2(x = d_all$sal_insitu_ctd_10m, y = d_all$sal_fb)
p <-  ggplot(d_all,aes(x=sal_insitu_ctd_10m, y= sal_fb))+
  geom_point( aes(color=factor(monthd))) + 
  geom_abline(slope=1, intercept=0, linetype = "dashed")+
  geom_abline(aes(intercept = lm$intercept_MA, slope = lm$slope_MA),colour = "blue") +
  ylim(c(30.5,35.5)) +xlim(c(28,35.5)) +
  labs(title = paste("","\n Adj R2 = ", signif(lm$rsquare, 3),
                     "; Intercept =", signif(lm$intercept_MA, 3),
                     "; Slope =", signif(lm$slope_MA, 3),
                     "; P =", signif(lm$P, 3),
                     "; RMSE = ", signif(lm$rmse, 3))) +
  theme(aspect.ratio = 1, plot.title = element_text(size = 7)) + #
  coord_fixed(ratio = 1 )+
  labs( x="Salinity CTD 10 m", y="Salinity FB ", color = "Months") + theme_bw()
...
The mean difference between salinity FB and in situ is: `r mean_diff_sal_fb_insitu_10m` ± `r sd_diff_sal_fb_insitu_10m` PSU.
...
lm <- lm2(x = d_all$temp_insitu_ctd_10m, y = d_all$temp_insitu_11m)
p <-  ggplot(d_all, aes(x=temp_insitu_ctd_10m, y= temp_insitu_11m, label = monthd))+
  geom_text( aes(color=factor(monthd))) + 
 # scale_color_viridis_d(option = "C", guide="none")+
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  geom_abline(aes(intercept = lm$intercept_MA, slope = lm$slope_MA),colour = "blue")  +  labs(title = paste("T CTD 9 m vs T 11 m","\n Adj R2 = ", signif(lm$rsquare, 3),
                     "; Intercept =", signif(lm$intercept_MA, 3),
                     "; Slope =", signif(lm$slope_MA, 3),
                     "; P =", signif(lm$P, 3),
                     "; RMSE = ", signif(lm$rmse, 3))) +
...
ggsave(filename = "figures/at_s.png", plot = at_s, width = 25, height = 20, units = "cm")

# Same plot for paper
p_at_s <- ggplot(d_all_at_discrete,aes(x=sal_mix, y= at)) +
  geom_point(col="blue", size = 0.2) +
  labs(x="Salinity", y="Discrete AT")  +
  stat_smooth(method = "lm", col = "blue", size = 0.3) +
...
ggsave(filename = "figures/p_at_s.png", plot = p_at_s, width = 7, height = 7, units = "cm")
...
**Note** that the Contros pCO2 values after 2020-03-01 were not used because the Covid-19 pandemic prevented maintenance and the setup of a freshly calibrated sensor. As the result, algae becalme inceasingly abundant, pulling pCO2 down and further away from values calculated from CT and AT. 

### **Corrected Contros vs calculated *p*CO~2~ data**

```{r pco2 and fco2 at in situ temp, echo=FALSE, warning=FALSE, message=FALSE}
d_all <- d_all %>%
  dplyr::mutate(
...
We first calculate fCO2 from pCO2 and then express both pCO2 and fCO2 at in situ temperature. Ideally, this should be done with SST as the goal is to estimate air-sea CO2 fluxes. I therefore use the in situ temperature at 11 m and we will need to speculate on stratification.
...
reg <- ggreg(
  ...
  subtitle = "",
  xlab = "Measured pCO2",
  ylab = "Calculated pCO2",
  proplot = TRUE
)
p <- reg$plot
ggsave(filename = "figures/pco2_calc_meas.png", plot = p)
```

The relationship between the measured (Contros) and calculated pCO2 (blue line) is relatively poor. It is far from the 1:1 relationship (black dashed line). The slope is `r signif(reg$fit$regression.results[2,3], 3)`) and its 95% confidence interval does include 1; r2 is `r round(reg$fit$rsquare, digits=2)`.

**Figure legend**: pCO2 calculated from AT and CT as a function of pCO2 measured using a Contros sensor. All data are normalized at in situ temperature. The black dotted line is the 1:1 line while the blue solid line shows the line obtained by a major axis regression (Legendre, 2018- Pierre Legendre (2018). lmodel2: Model II Regression. R package version 1.7-3. https://CRAN.R-project.org/package=lmodel2
...
pH is measured continuously by an autonomous seaFET pH sensor. Measurement periodicity was set to 1 hour from 2018-02-02 to 2018-04-17 it was then measured once every minute. pH calibration is based on monthly, when possible, discrete samples collected at 11 m that were analysed with spectrophotometric method in Villefranche by Samir Alliouane. Measurments were performed several months after sampling. When sampling was not possible in the vicinity of the sensor for practical reasons (in winter), sampling was done in the harbour and is not taken in consideration for calibration for spatial reasons.  *Note that the pH sensor is mounted on a profiling structure. Here, in principle, only seaFET data recorded with a depth > 8 m should be taken into consideration.* 

Calibration was done following the methods and code described in Bresnahan et al. (2014) and adapted to use in R (seacarb package). 3 deployment periods were defined, corresponding to a change of sensors. 

Within a deployment period, calibration is made with E0 calculation (first plot) and final corrected pH is calculated (second plot). To allow calibration, one spectrophotometric pH measurement must match: seaFET voltage, in situ temperature (seaFET) and salinity (REMOS CTD). When one of these parameters is missing for any reasons (Ferrybox maintenance, breakdown...), the gap is filled with a mean of the 4 previous or next hours when available. If the pH spectrophotometric measurement is too far from the available values, the calibration is not performed (environemental variability reasons).

Only 5 spectrophotometric pH which could be used for calibration when seaFET at > 8 m: 2018-11-16, 2019-04-17, 2019-10-11, 2020-01-27. All deployment meriods are covered but we had to alter the sampling time of:

* 2017-08-24 14:00 to 2017-08-24 19:00 (5 h later)
* 2017-11-28 09:00 to 2017-11-28 13:00 (4 h later)

EO is calculated making a mean of several E0 that come from one deployment period (sensor in the water for several months).
...
#discrete <- left_join(discrete, d_all%>%dplyr::select(closest_datetime,depth, depth_discrete),by="closest_datetime")
...
#offset_xts <- select(discrete, closest_datetime, depth,depth_discrete, ph_s_sf_temp_insi)
...
#dySeries("depth_discrete", drawPoints = TRUE, pointSize=3, strokeWidth = 1,label = "depth_discrete", color="black")%>%
...
names(calib_spectro_all) <- c("deployment_sf", "E0int25_sf", "E0ext25_sf")
...
calib_spectro_depth_10m <- sf_calib(calEint = d_all_calib_depth$voltINT, calEext = d_all_calib_depth$voltEXT, calpH = d_all_calib_depth$ph_s_sf_temp_insi, calT = d_all_calib_depth$temp_sf, calSal = d_all_calib_depth$sal_insitu_ctd_interp)
# put number of deployment 
calib_spectro_depth_10m$deployment_sf <- d_all_calib_depth$deployment_sf 

# make the mean of E0 by period of deployement
calib_spectro_depth_10m <- group_by(calib_spectro_depth_10m, deployment_sf) %>%
  summarise(E0int25 = mean(E0int25, na.rm = TRUE),
            E0ext25 = mean(E0ext25, na.rm = TRUE) 
            ) %>% 
  dplyr::filter(!is.na(deployment_sf))
names(calib_spectro_depth_10m) <- c("deployment_sf", "E0int25_sf_10m", "E0ext25_sf_10m")

# add E0 mean to seaFET data per deployment
d_all <- dplyr::left_join(d_all, calib_spectro_depth_10m, by="deployment_sf")
d_all <- dplyr::select(d_all, order(colnames(d_all)))
```

Below are the seafet calibration parameters using all data (irrespective of the depth of the seafet) and with the seafet data collected below 8 m. They are very close but we nevertheless use the calibration data below 8 m (easier to explain in the manuscript).
...
calib_spectro <- cbind(calib_spectro_all, calib_spectro_depth_10m)
kbl(calib_spectro)%>%
    kable_paper() %>% 
...
Final calibrated seaFET pHT data (small green dots) are shown here after calibration with spectro measurements (red dots) according to each period of deployement, defined between 2 spectro measurements (shading grey).
...
##########  Despike() ###########
d_all <- d_all %>%   
  dplyr::mutate(pHint_tot_sf = oce::despike(seafet_calib$pHint_tot_sf, reference= "median", n=1, k=5001, replace="NA"),
                pHext_tot_sf = oce::despike(seafet_calib$pHext_tot_sf, reference= "median", n=1, k=5001, replace="NA")
  )
...
# Il faut par contre créér une nouvelle variable avec pH deeper than 8 m, called *_10m for consistency with salinity and temp
d_all <- d_all %>%
  ...
  dplyr::mutate(pHint_tot_sf_1m = ifelse(depth > 0 & depth < 2, pHint_tot_sf, NA),
                ...
                pHint_tot_sf_10m = ifelse(depth >= 8 , pHint_tot_sf, NA),
                depth_discrete = case_when(
      !is.na(pHint_tot_sf_1m) ~ 1,
      !is.na(pHint_tot_sf_3m) ~ 3,
      !is.na(pHint_tot_sf_5m) ~ 5,
      !is.na(pHint_tot_sf_7m) ~ 7,
      !is.na(pHint_tot_sf_10m) ~ 10
      ),
      depth_discrete = as.factor(depth_discrete)
  )

seafet_depth_xts <- dplyr::select(d_all, datetime, ph_s_sf_temp_insi, pHint_tot_sf_1m, pHint_tot_sf_3m, pHint_tot_sf_5m, pHint_tot_sf_7m, pHint_tot_sf_10m)
...
dySeries("pHint_tot_sf_10m", label = "pHT 10m", color =  RColorBrewer::brewer.pal(3, "Dark2")[1], strokeWidth = 0, pointSize = 1) %>%
...
# bind new calibrated pH data to the seafet data set.
names(seafet_calib_10m) <- c("pHint_tot_sf_calib10m", "pHext_tot_sf_calib10m")
##########  Despike() ###########
d_all <- d_all %>%   
  dplyr::mutate(pHint_tot_sf_calib10m = oce::despike(seafet_calib_10m$pHint_tot_sf_calib10m, reference= "median", n=1, k=5001, replace="NA"),
                pHext_tot_sf_calib10m = oce::despike(seafet_calib_10m$pHext_tot_sf_calib10m, reference= "median", n=1, k=5001, replace="NA")
  )

d_all <- d_all %>%
  #dplyr::mutate(pHint_tot_sf = despike(d_all$pHint_tot_sf, reference= "median", n=2, k=5761, replace="NA")) %>%
  dplyr::mutate(pHint_tot_sf_calib10m = seafet_calib_10m$pHint_tot_sf_calib10m) 

#seafet_depth_xts <- dplyr::select(d_all, datetime, ph_s_sf_temp_insi, pHint_tot_sf_1m, pHint_tot_sf_3m, pHint_tot_sf_5m, pHint_tot_sf_7m, pHint_tot_sf_10m, pHint_tot_sf_calib10m)
seafet_depth_xts <- dplyr::select(d_all, datetime, ph_s_sf_temp_insi, pHint_tot_sf_calib10m)
seafet_depth_xts <- as.xts(seafet_depth_xts, order.by = d_all$datetime)
dygraph(seafet_depth_xts, group = "depth", main="Spec pH and calibrated seaFET pH at 10 m", ylab="pHT") %>%
dySeries("pHint_tot_sf_calib10m", label = "pH calib 10m", color =  "black", strokeWidth = 0, pointSize = 1) %>%
#dySeries("pHint_tot_sf_10m", label = "pHT 10m", color =  RColorBrewer::brewer.pal(3, "Dark2")[1], strokeWidth = 0, pointSize = 1) %>%
#dySeries("pHint_tot_sf_7m", label = "pHT 7m", color =  RColorBrewer::brewer.pal(3, "Dark2")[2], strokeWidth = 0, pointSize = 1) %>%
#dySeries("pHint_tot_sf_5m", label = "pHT 5m", color =  RColorBrewer::brewer.pal(3, "Dark2")[3], strokeWidth = 0, pointSize = 1) %>%
#dySeries("pHint_tot_sf_3m", label = "pHT 3m", color =  RColorBrewer::brewer.pal(3, "Dark2")[4], strokeWidth = 0, pointSize = 1) %>%
...
“differences between calibrated sensor pH values and values obtained from discrete, check samples can be expected to be as small as 0.02 pH units, and perhaps less.” (McLaughlin et al. 2017).
...
ph_sf <- d_all%>%
  dplyr::select(datetime, ph_s_sf_temp_insi, pHint_tot_sf_calib10m)%>%
    dplyr::mutate( diff_spec_sf = ph_s_sf_temp_insi - pHint_tot_sf_calib10m)
...
- pH INT - pH EXT was plotted here.
...
tmp <- d_all %>% 
  dplyr::select(year, montht, pHint_tot_sf, depth_discrete) %>%
  tidyr::drop_na(pHint_tot_sf, depth_discrete) %>%
  dplyr::group_by(depth_discrete, year, montht)
summary(aov(tmp$pHint_tot_sf ~ tmp$depth_discrete * tmp$montht))

# remove outliers
quantile(tmp$pHint_tot_sf, na.rm =TRUE, probs = c(0.01, 0.99))

p <- tmp %>%
  ggplot(aes(x=depth_discrete, y = pHint_tot_sf)) +
...
pp <- tmp %>%
  ggplot(aes(x=depth_discrete, y = pHint_tot_sf)) +
  stat_boxplot(notch=FALSE, size=0.2, fill="grey50", outlier.color = "blue", outlier.size = 0.3, coef = 3)

# we look at 0 to 4 m vs more than 8 m
tmp <- d_all %>% 
  dplyr::select(montht, pHint_tot_sf, depth) %>%
  dplyr::mutate(depth2 = as.factor(case_when(depth < 4 ~ '0-4', 
                                             depth > 8~ '8-12',
                                             TRUE ~ NA_real_))) %>% 
  dplyr::filter(depth2 == '0-2' | depth2 == '8-12') %>% 
...
Depth and month of the year have a statistically significant effect on pH. The interaction is also significant, indicating that the effect of depth differs depending on the month.

 + To compare with pCO2/pCO2 measured in the Ferrybox seems therefore better to stick with pH_10m, that is at depths deeper than 8 m.
  + pCO2 would have the same behaviour as pH. For air-sea CO2 fluxes, one should use data from 0 to 4 m.

## pH durafet

pH is measured continuously by an autonomous durafet pH sensor. Final calibrated durafet pHT data (green doibb) are shown here after calibration with spectro measurements (red dots) according to each period of deployement, defined between 2 spectro measurements (shading grey). Calibration was done appliying an offset of pH between measured and spectro data per periods.
...
my_skim <- skim_with(numeric = sfl(ave = ~ mean(., na.rm = TRUE),
...
range_month <-
  d_all %>%
  dplyr::select(montht, sal_mix, temp_mix, temp_insitu_11m, pHint_tot_sf_10m) %>% 
  group_by(montht) %>%
  my_skim()
...
### **seaFET / durafet pHs (all at 9 m)**

```{r pHs comparaison, echo=FALSE, message= FALSE, warning=FALSE}
# prepare data and remove sf biofouling period
pH_sf <- d_all %>%
  dplyr::select(datetime, sal_insitu_ctd_10m , temp_insitu_ctd_10m, pHint_tot_sf)%>%
  dplyr::filter(!is.na(pHint_tot_sf), !is.na(temp_insitu_ctd_10m), !is.na(sal_insitu_ctd_10m)) %>%
  dplyr::filter(datetime < as.POSIXct("2018-01-01") | datetime > as.POSIXct("2018-02-03"))

pH_sf$pHint_tot_sf_temp_4 <- pHinsi(pH=pH_sf$pHint_tot_sf, ALK=mean(d_all$at, na.rm=TRUE), Tinsi=4, Tlab=pH_sf$temp_insitu_ctd_10m, Pinsi=11/10, S=pH_sf$sal_insitu_ctd_10m, Pt=0, Sit=0, k1k2 = "l", kf = "dg", ks="d", pHscale = "T", b="u74")
...
p <-  ggplot(d_all, aes(x=ph_s_dur_temp_fb, y= ph_dur_corr, label = monthd)) +
  ...
  scale_color_discrete(guide = "none")+
  geom_abline(slope=1, intercept =0, linetype = "dashed") +
  geom_abline(aes(intercept = lm$intercept_MA, slope = lm$slope_MA),
              colour = "blue") +
...
tmp <- d_all %>% 
  tidyr::drop_na(ph_s_dur_temp_fb, pco2_fb, at, sal_fb, temp_fb) %>% 
  dplyr::select(datetime, ph_s_dur_temp_fb, pco2_fb, at, sal_fb, temp_fb) %>% 
  dplyr::mutate(
    ph_s_dur_temp_4_l = pHinsi(pH = ph_s_dur_temp_fb, # lueker
                         ALK = at*1e-6, Tinsi = 4,
                         Tlab = temp_fb, Pinsi = 0,
                         S = sal_fb, Pt = 0, Sit = 0, k1k2 = "l", kf = "dg", ks="d",
                         pHscale = "T", b="u74"),
   ph_s_dur_temp_4_s20 = pHinsi(pH = ph_s_dur_temp_fb, # Sulpis
                         ALK = at*1e-6, Tinsi = 4,
                         Tlab = temp_fb, Pinsi = 0,
                         S = sal_fb, Pt = 0, Sit = 0, k1k2 = "s20", kf = "dg", ks="d",
                         pHscale = "T", b="u74"),       # Millero 02
   ph_s_dur_temp_4_p18 = pHinsi(pH = ph_s_dur_temp_fb, # Papadimitriou
                         ALK = at*1e-6, Tinsi = 4,
                         Tlab = temp_fb, Pinsi = 0,
                         S = sal_fb, Pt = 0, Sit = 0, k1k2 = "p18", kf = "dg", ks="d",
                         pHscale = "T", b="u74"),
   ph_s_dur_temp_4_m02 = pHinsi(pH = ph_s_dur_temp_fb,  # Millero 02
                         ALK = at*1e-6, Tinsi = 4,
                         Tlab = temp_fb, Pinsi = 0,
                         S = sal_fb, Pt = 0, Sit = 0, k1k2 = "m02", kf = "dg", ks="d",
                         pHscale = "T", b="u74")
   ) %>% 
...
tmp <- d_all %>% 
  tidyr::drop_na(ph_s_dur_temp_fb, ct, at, sal_fb, temp_fb) %>% 
  dplyr::select(datetime, ph_s_dur_temp_fb, ct, at, sal_fb, temp_fb) %>% 
  dplyr::mutate(
    ph_s_dur_temp_4_l = pHinsi(pH = ph_s_dur_temp_fb, # lueker
                         ALK = at*1e-6, Tinsi = 4,
                         Tlab = temp_fb, Pinsi = 0,
...
sal_delta_plt <- tmp_delta %>% 
  ...
  annotate("text", x = 1.5, y = 0.65, label = substitute("S"["surface"] ~ "> S"["deep"])) +
  annotate("text", x = 1.5, y = -0.9, label = substitute("S"["surface"] ~ "< S"["deep"])) +
...
temp_delta_plt <- tmp_delta %>% 
  ...
  scale_x_discrete(expand = c(0, 0)) +
  scale_y_continuous("Temperature", limits = c(-1.3, 0.8), expand = c(0, 0)) +
  geom_col(fill="blue") +
  geom_hline(yintercept = 0) +
  annotate("text", x = 1.5, y =0.6, label = substitute("T"["surface"] ~ "> T"["deep"])) +
  annotate("text", x = 1.5, y = -0.6, label = substitute("T"["surface"] ~ "< T"["deep"])) +
  Mytheme(size_labs = 12) +
  theme(panel.grid.major.x = element_blank(), panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank())
...
tmp_delta <- tmp %>% 
  dplyr::select(montht, depth_group, density) %>% 
  dplyr::mutate(depth_group2 = as.factor(case_when(depth_group == '0-2' | depth_group == '2-4' ~ '0-4', 
                                      depth_group == '>8' ~ '8-12'))) %>% 
  dplyr::filter(depth_group2 == '0-4' | depth_group2 == '8-12') %>% 
  group_by(depth_group2, montht) %>% 
...
den_delta_plt <- tmp_delta %>% 
  ...
  geom_col(fill="blue") +
  geom_hline(yintercept = 0) +
  annotate("text", x = 1.5, y = 0.1, label = substitute("D"["surface"] ~ "> D"["deep"])) +
  annotate("text", x = 1.5, y = -1.2, label = substitute("D"["surface"] ~ "< D"["deep"])) +
  Mytheme(size_labs = 12) +
  theme(panel.grid.major.x = element_blank(), panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank())
...
tmp_delta <- tmp %>% 
  ...
  tidyr::pivot_wider(names_from = depth, values_from = pH_tot_sf_med) %>% 
...
pH_delta_plt <- tmp_delta %>% 
  ggplot(aes(x=montht, y=pH_tot_sf_med_delta)) +
  #labs(y = substitute("pH"["T"] ~ "at in situ temp."), x = "Month") +
  scale_x_discrete("Month", expand = c(0, 0)) +
  scale_y_continuous(substitute("pH"["T"] ~ "at in situ temp."), limits = c(-0.2, 0.1), expand = c(0, 0)) +
  geom_col(fill="blue") +
  geom_hline(yintercept = 0) +
  annotate("text", x = 1.5, y = 0.8, label = substitute("pH"["surface"] ~ "> pH"["deep"])) +
  annotate("text", x = 1.5, y = -0.15, label = substitute("pH"["surface"] ~ "< pH"["deep"])) +
  Mytheme(size_labs = 12) +
  theme(panel.grid.major.x = element_blank(), panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank())

deltas <- sal_delta_plt / temp_delta_plt / den_delta_plt / pH_delta_plt
deltas <- deltas + plot_annotation(tag_levels = 'a')
ggsave(filename = "figures/deltas.png", plot = deltas, width = 15, height = 20, units = "cm")
```
...
```{r Takahashi thermal vs non-thermal, include=FALSE}
# According to Takahashi et al. (2002) and McCutcheon et al. (2021)
# Select data
taka <- d_all %>% # just for this chunk
  dplyr::select(datetime, monthd, sal_fb, temp_insitu_11m, pco2_fb_insi_temp) %>% 
...
For seasonal analysis, December to February was considered winter, March to May was considered spring, June to August was considered summer, and September to November was considered fall. 

The control of pCO2 by non-thermal (physical and biological processes) drivers is always greater than the control from temperature on both annual and seasonal basis. Thermal control is highest in winter and lowest in summer.
...
deconv_tmp <- d_all %>% # just for this chunk
  dplyr::select(datetime, monthd, sal_fb, temp_fb, pco2_fb, at_calc) %>% 
  tidyr::drop_na()
deconv_tmp <- deconv_tmp %>% 
  dplyr::mutate(ct_calc = seacarb::carb(
    flag = 24, var1 = deconv_tmp$pco2_fb, var2 = deconv_tmp$at_calc*1e-6, S=deconv_tmp$sal_fb, T=deconv_tmp$temp_fb, 
    Patm=1, P=1.1, Pt=0, Sit=0, 
    k1k2="l", kf="dg", ks="d", pHscale="T", b="u74")$DIC * 1e6
    )
...
deconv_dat <- tibble(sampling_date=deconv_tmp$datetime,
                     ...
                     mAT=deconv_monthly$at_calc[deconv_tmp$monthd]*1e-6, mCT=deconv_monthly$ct_calc[deconv_tmp$monthd]*1e-6, mS=deconv_monthly$sal_fb[deconv_tmp$monthd], mT=deconv_monthly$temp_fb[deconv_tmp$monthd], P=11/10, Pt=rep(0, length.out=nrow(deconv_tmp)), Sit=rep(0, length.out=nrow(deconv_tmp)), AT=deconv_tmp$at_calc*1e-6, CT=deconv_tmp$ct_calc*1e-6, S=deconv_tmp$sal_fb, T=deconv_tmp$temp_fb)
...
deconv_pCO2_T <- tibble(date = d$date,
                        deconv_pCO2_T=carb(flag=15, var1=d$mAT, var2=d$mCT, S=d$mS, T=d$T, P=d$P, Pt=d$Pt, Sit=d$Sit, k1k2="l", kf="dg", ks="d", pHscale="T", b="u74")$pCO2
)
...
deconv_pCO2_S <- tibble(date=d$date,
                     deconv_pCO2_S=carb(flag=15, var1=d$mAT, var2=d$mCT, S=d$S, T=d$mT, P=d$P, Pt=d$Pt, 
                                      Sit=d$Sit, k1k2="l", kf="dg", ks="d", pHscale="T", b="u74")$pCO2
                     )
d <- filter(deconv_dat, !is.na(AT), !is.na(mCT), !is.na(mS), !is.na(mT)) # we eliminate date with NAs
deconv_pCO2_AT <- tibble(date=d$date,
                     deconv_pCO2_AT=carb(flag=15, var1=d$AT, var2=d$mCT, S=d$mS, T=d$mT, P=d$P, Pt=d$Pt, Sit=d$Sit, k1k2="l", kf="dg", ks="d", pHscale="T", b="u74")$pCO2
)
d <- filter(deconv_dat, !is.na(mAT), !is.na(CT), !is.na(mS), !is.na(mT)) # we eliminate date with NAs
deconv_pCO2_CT <- tibble(date=d$date,
                     deconv_pCO2_CT=carb(flag=15, var1=d$mAT, var2=d$CT, S=d$mS, T=d$mT, P=d$P, Pt=d$Pt, Sit=d$Sit, k1k2="l", kf="dg", ks="d", pHscale="T", b="u74")$pCO2
)
...
+ wind speed data collected by the AWI at 10 m height (sent to JPG by P. Fischer, Aug. 2020)
...
# Below was an attempt to smooth but
# zz <- as_tsibble(atm_co2) %>% 
#   mutate(smoothed_co2_air = slide_dbl(co2_air, .f = mean, .before = 12, .after = 12, .complete = TRUE))
# ggplot(zz) +
#   geom_line(aes(x = datetime, y = smoothed_co2_air))
...
d_all <- d_all %>%
  dplyr::mutate(delta_pco2 = pco2_fb - co2_air, # ppm
...
The annual air-sea CO2 flux is `r round(sum(air_sea_month$f_air_sea), digits = 0)` mol/(m2 year).
...
* sal_mix: when ferrybox data (sal_fb) are missing, they are replaced by data from the profiling ctd for depths > 8 m (sal_insitu_ctd_10m)
* sal_mix_interp: interpolated salinity sal_mix. It is used to have as many spec pH to display (some are during missing data)
...
* temp_mix_interp: interpolated temperature temp_mix. It is used to have as many spec pH to display (some are during missing data)
...
* calibrate seafet pH
    ...
    - E0int25_sf: calculé à partir du voltage interne + ph_s_sf_temp_insi (temp 11m + temp_insitu_ctd_10m si trous) + temp CTD 10m + sal CTD 10m (calib seaFET fonction 1)
