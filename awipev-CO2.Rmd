...
d_all <- d_all %>%
  dplyr::mutate(deployment_sf = ifelse(datetime >= sfinterv[1] & datetime <= sfinterv[2], 01, 
                                ...
                                ifelse(datetime >= sfinterv[26] , 26,NA )))))))))))))))))))))))))))
...
d_all <- d_all%>%
  dplyr::mutate(voltINT = ifelse(datetime == "2017-08-24 14:00:00" , mean_voltINT_4h, voltINT))
...
d_all <- d_all%>%
  dplyr::mutate(voltEXT = ifelse(datetime == "2017-08-24 14:00:00" , mean_voltEXT_4h, voltEXT))
...
d_all <- d_all%>%
  dplyr::mutate(temp_insitu = ifelse(datetime == "2017-08-24 14:00:00" , mean_TseaF_4h, temp_insitu))
...
dygraph(seafet_contros_xts, group = "awipev", main="pH seaFET (in situ)", ylab="pHT") %>%
  ...
  dyShading(from = sfinterv[22], to= sfinterv[23], color = "#E0E0E0") %>%
  dyShading(from = sfinterv[23], to= sfinterv[24], color = "#CCCCCC") %>%
  dyShading(from = sfinterv[24], to= sfinterv[25], color = "#E0E0E0") %>%
  dyShading(from = sfinterv[25], to= Sys.Date(), color = "#CCCCCC") %>%
  dyLegend(show = "auto")   %>%
  dyHighlight(highlightCircleSize = 8,highlightSeriesBackgroundAlpha = 0.5,hideOnMouseOut = TRUE) %>%
  dyOptions(drawGrid = TRUE, drawPoints = TRUE, pointSize = c(1,8,1),useDataTimezone = TRUE, digitsAfterDecimal=4) %>%
  dyRangeSelector(height = 30, dateWindow= NULL) 
...
d_all <- d_all %>%
  dplyr::mutate(deployment_dur = ifelse(datetime >= durinterv[1] & datetime <= durinterv[2], 01, 
                                ...
                                ifelse(datetime >= durinterv[19], 18,NA ))))))))))))))))))))
...
d_all <- d_all%>%
 dplyr::mutate(offset_durafet = ifelse(deployment_dur == 1, dat_dur$diff_pH_dur[1],
                                ...
                                ifelse(deployment_dur == 19, dat_dur$diff_pH_dur[19],0))))))))))))))))))),
...
durafet_contros_xts <- as.xts(durafet_contros_xts, order.by = d_all$datetime)
 dygraph(durafet_contros_xts, group = "awipev", main="pH durafet (in Ferrybox)", ylab="pHT") %>%
   ...
   dyShading(from = durinterv[17], to= Sys.Date(), color = "#CCCCCC") %>%
     dyLegend(show = "follow")   %>%
    dyHighlight(highlightCircleSize = 8,highlightSeriesBackgroundAlpha = 0.5,hideOnMouseOut = TRUE) %>%
    dyOptions(drawGrid = TRUE, drawPoints = TRUE, pointSize = c(1,8,1),useDataTimezone = TRUE, digitsAfterDecimal=4) %>%
    dyRangeSelector(height = 30, dateWindow= NULL) 
