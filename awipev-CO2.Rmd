...
```
The means used are:
`r knitr::kable(tibble(mean_at, mean_ct, mean_pco2, mean_sal, mean_temp), digits = c(1, 1, 0, 1, 1), caption = "Mean discrete values.", format = "simple")`
...
tmp2 <- tibble(ph_s_dur_temp_4 = tmp$ph_s_dur_temp_4, pH_l = carb_l$pH, pH_s = carb_s$pH) %>% 
  tidyr::drop_na(ph_s_dur_temp_4)
# summary of differences
diff_lueker <- as.numeric(summary(tmp2$ph_s_dur_temp_4 - tmp2$pH_l))
diff_sulpis <- as.numeric(summary(tmp2$ph_s_dur_temp_4 - tmp2$pH_s))
...
```
The discrete values of pCO2, AT (calculated from the AT-salinity relationship), salinity and temperature in the FerryBox were used to calculate pH from measured pCO2 and AT (from a salinity-AT relationship) using the formulations for K1 and K2 of Lueker et al. (2000) and Sulpis et al. (2020). The differences induced by the choice of the carbonic acid constants is:
+ large for pH (0.026 pH units on average; always higher with Sulpis than with Lueker)
+ large for CT (3.7 umol/kg on average; always lower with Sulpis than with Lueker)
...
Here we compare pH measured spectrophotometrically with pH calculated from pCO2 and AT using the sets of K1 and K2 of Lueker et al. (2000) and Sulpis et al. (2020). All pH values were normalized to a temperature of 4°C. The absloute differences are large, up to 0.14 pH units. On average, both formulations overestimate spectrophotometric pH, by 0.016 pH units with Lueker et al. (2020) and by 0.042 pH units with Sulpis et al. (2020).
`r knitr::kable(lueker_sulpis_table, digits = 3, caption = "Difference between spectrophotometric pH and pH calculated with pCO2 and AT using the formulations for K1 and K2 of Lueker and Sulpis.", format = "simple")`
