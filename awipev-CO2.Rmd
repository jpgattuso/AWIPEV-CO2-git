...
pH_seafet <- pH_seafet %>%
  dplyr::mutate(pH_s_seafet_temp_insitu = pHinsi(pH=pH_seafet$pH_s_seafet,ALK=pH_seafet$at_mean_interp*1e-6, Tinsi=pH_seafet$temp_field, Tlab=pH_seafet$temp_s_lab, Pinsi= pH_seafet$depth/10, S=pH_seafet$salinity_field,Pt=0,Sit=0))
                
pH_durafet <- d %>%
  dplyr::filter(!is.na(pH_s_durafet))
pH_durafet <- pH_durafet %>%
  dplyr::mutate(pH_s_durafet_temp_insitu = pHinsi(pH=pH_durafet$pH_s_durafet,ALK=pH_durafet$at_mean_interp*1e-6, Tinsi=pH_durafet$temp_field, Tlab=pH_durafet$temp_s_lab, Pinsi= pH_durafet$depth/10, S=pH_durafet$salinity_field,Pt=0,Sit=0))
 
# create separate seafet/durafet df to join to initial "d" df
pH_seafet_mean <- pH_seafet%>%
  dplyr::select(datetime,pH_s_seafet_temp_insitu)%>% 
  dplyr::group_by(datetime) %>% 
  dplyr::summarize(pH_s_seafet_temp_insitu = mean(pH_s_seafet_temp_insitu, na.rm = T))
pH_durafet_mean <- pH_durafet%>%
  dplyr::select(datetime,pH_s_durafet_temp_insitu)%>% 
  dplyr::group_by(datetime) %>% 
  dplyr::summarize(pH_s_durafet_temp_insitu = mean(pH_s_durafet_temp_insitu, na.rm = T))
...
##   *****Attention, I asked to the logisticians to log outside temp from 9 june 2019
...
d_hour <- d_hour %>%
  ...
  dplyr::rename(at_contros=AT,pco2_contros=PCO2_corr_contros_filtered,pH_durafet=HW_pH1_filtered,temp_insitu_11m=temp_insitu_11m_filtered, sal_fb = sal_fb_filtered, sal_insitu_ctd = sal_insitu_ctd_filtered, temp_fb = temp_fb_filtered, temp_insitu_ctd = temp_insitu_ctd_filtered)
```
...
discrete2merge <- discrete %>%
  #drop_na() %>%
  dplyr::select(closest_datetime, pH_s_seafet_temp_insitu) %>%
  dplyr::filter(!is.na(closest_datetime)) %>%
  rename(datetime=closest_datetime)
d_all <- full_join(d_all, discrete2merge, by="datetime")
# #voltINT
...
discrete2merge <- discrete %>%
  #drop_na() %>%
  dplyr::select(closest_datetime, pH_s_durafet_temp_insitu) %>%
  dplyr::filter(!is.na(closest_datetime)) %>%
  rename(datetime=closest_datetime)
d_all <- full_join(d_all, discrete2merge, by="datetime")
...
d_all <- d_all %>%
  dplyr::mutate(sal_mix = ifelse(!is.na(sal_fb), sal_fb, 
                               ifelse (pressure_insitu_ctd_filtered >=8 ,sal_insitu_ctd, NA )))
### remove sal_mix datat that seem to be to high compared to sal_fb
  d_all <- d_all%>%
  dplyr::mutate(sal_mix = ifelse(datetime > "2018-03-09 00:00:00" & datetime < "2018-04-13 18:00:00" , NA, 
                          ...
                          ifelse(datetime > "2018-05-28 06:00:00" & datetime < "2018-06-03 13:00:00" , NA,
...
dygraph(temp_fb_xts, group = "awipev", main="Temperature (in Ferrybox)", ylab="Temperature") %>%
      ...
      dyRangeSelector(height = 30, dateWindow= NULL)
# TEMP insitu SBE38
temp_insitu_xts <- dplyr::select(d_all, datetime, temp_mix)
...
dygraph(temp_insitu_xts, group = "awipev", main="Temperature (in situ 11 m)", ylab="Temperature") %>%
      dySeries("temp_mix", color = "blue", strokeWidth = 0, label = "T") %>%
...
dygraph(pco2_contros_xts, group = "awipev", main="pCO2", ylab="pCO2") %>%
    dySeries("pco2_contros", label="Corrected pCO2", color=  RColorBrewer::brewer.pal(3, "Dark2")[1],strokeWidth=0, pointSize=2) %>%
      dySeries("PCO2_Corr_filtered", label="Raw pCO2",color ="blue", strokeWidth = 0, pointSize = 1) %>%
    ...
    dyRangeSelector(height = 30, dateWindow= NULL)
# DURAFET 
durafet_contros_xts <- dplyr::select(d_all, datetime, pH_durafet, pH_s_durafet_temp_insitu)
...
dygraph(durafet_contros_xts, group = "awipev", main="pH durafet (in Ferrybox)", ylab="pHT") %>%
    ...
    dySeries("pH_s_durafet_temp_insitu", label = "pH spectro", color =  "red", strokeWidth = 0, pointSize = 4 ) %>%
    ...
    dyRangeSelector(height = 30, dateWindow= NULL) 
# SEAFET 
seafet_contros_xts <- dplyr::select(d_all, datetime, phINT_filtered,pH_s_seafet_temp_insitu)
...
dygraph(seafet_contros_xts, group = "awipev", main="pH seaFET (in situ)", ylab="pHT") %>%
  ...
  dySeries("pH_s_seafet_temp_insitu", label = "pH spectro", color =  "red", strokeWidth = 0, pointSize = 4 ) %>%
...
fit <- lmodel2(data = d_all%>%dplyr::filter(pressure_insitu_ctd_filtered >=8),  sal_fb ~ sal_insitu_ctd , nperm = 99)

```{r Sal comparaison, echo=FALSE, message= FALSE, warning=FALSE, out.width="80%",fig.show='hold', fig.align="center"}
fit <- lmodel2(data = d_all%>%dplyr::filter(pressure_insitu_ctd_filtered >=8),  sal_fb ~ sal_insitu_ctd , nperm = 99)

p_sal_fb_sal_insitu<-  ggplot(d_all,aes(x=sal_insitu_ctd, y= sal_fb, label = month))+
 ...
 # scale_color_viridis_d(option = "C", guide="none")+
  ...
  labs(title = paste("Sal in situ vs Sal FB","\n Adj R2 = ", signif(fit$rsquare, 3),
  ...
  labs( x="Salinity insitu", y="Salinity FB ", color = "Months")  +theme_bw()
print(p_sal_fb_sal_insitu)

d_all$diff_sal_fb_insitu <- d_all$sal_insitu - d_all$sal_fb
...
# # PLOT
# seafet_contros_xts <- dplyr::select(d_all, datetime, temp_insitu_11m, temp_insitu_ctd, pressure_insitu_ctd_filtered, temp_mix, sal_mix)
# seafet_contros_xts <- as.xts(seafet_contros_xts, order.by = d_all$datetime)
# dygraph(seafet_contros_xts, group = "awipev", main="pH seaFET (in situ)", ylab="pHT") %>%
# dySeries("temp_insitu_11m", label = "temp_insitu_11m", color ="red",axis= "y2", strokeWidth=0, pointSize=4) %>%
# dySeries("temp_insitu_ctd", label = "temp_insitu_ctd", color ="blue",axis= "y2", strokeWidth=0, pointSize=3) %>%
# dySeries("temp_mix", label = "temp_mix", color ="green",axis= "y2", strokeWidth=0, pointSize=2) %>%
# dySeries("sal_mix", label = "sal_mix", color ="orange",axis= "y2", strokeWidth=0, pointSize=2) %>%
# dySeries("pressure_insitu_ctd_filtered", label = "pressure_insitu_ctd_filtered", color ="black", strokeWidth=0, pointSize=2) %>%
...
- 5 CTD > 8 meters depth (sal_insitu_ctd)
- 5 fixed at 11 meters depth (sal_insitu_11m)
...
fit <- lmodel2(data = d_all,  T_seaF ~ temp_mix , nperm = 99)

```{r Temp comparaison, echo=FALSE, message= FALSE, warning=FALSE}
# seaF outliers (T - volt int - volt ext) to remove from 2017-12-10 to 2017-12-14
d_all <- d_all %>%
  dplyr::mutate(T_seaF = ifelse(datetime >= "2017-12-10 11:00:00" & datetime <= "2017-12-14 12:00:00", NA, T_seaF),
                voltINT = ifelse(datetime >= "2017-12-10 11:00:00" & datetime <= "2017-12-14 12:00:00", NA, voltINT), 
                voltEXT = ifelse(datetime >= "2017-12-10 11:00:00" & datetime <= "2017-12-14 12:00:00", NA, voltEXT))

fit <- lmodel2(data = d_all,  T_seaF ~ temp_mix , nperm = 99)

pTseaf_Tinsi<-  ggplot(d_all,aes(x=temp_mix, y= T_seaF, label = month))+
  geom_text( aes(color=factor(month))) + 
 # scale_color_viridis_d(option = "C", guide="none")+
  geom_line(aes(x = T_seaF, y = T_seaF), linetype = "dashed") +
  geom_abline(aes(intercept = fit$regression.results[2,2], slope = fit$regression.results[2,3]),colour = "blue") +
  labs(title = paste("T seaF vs T insi","\n Adj R2 = ", signif(fit$rsquare, 3),
                     "; Intercept =", signif(fit$regression.results[2,2], 3),
                     "; Slope =", signif( fit$regression.results[2,3], 3),
                     "; P =", signif(fit$P.param, 3))) +
  theme(aspect.ratio=1,plot.title = element_text(size=7)) + 
  coord_fixed(ratio = 1 )+ scale_colour_discrete(guide="none") +
  labs( x="T insi", y="T seaF ", color = "Months")  +theme_bw()
print(pTseaf_Tinsi)

d_all$diff_T_seaF_insitu <- d_all$T_seaF - d_all$temp_mix
mean_diff_T_seaF_insitu <- round(mean(d_all$diff_T_seaF_insitu, na.rm = TRUE), digits = 2)
sd_diff_T_seaF_insitu <- round(sd(d_all$diff_T_seaF_insitu, na.rm = TRUE), digits = 2)

```

The mean difference between T seaFET and in situ is: `r mean_diff_T_seaF_insitu` ± `r sd_diff_T_seaF_insitu`°C.

### **T Ferrybox / T mix **

Property-property temperature plot using:

fit <- lmodel2(data = d_all,  temp_fb ~ temp_mix , nperm = 99)
...
pTfb_Tinsi<-  ggplot(d_all,aes(x=temp_mix, y= temp_fb, label = month))+
  geom_text( aes(color=factor(month))) + 
 # scale_color_viridis_d(option = "C", guide="none")+
  geom_line(aes(x = temp_fb, y = temp_fb), linetype = "dashed") +
  geom_abline(aes(intercept = fit$regression.results[2,2], slope = fit$regression.results[2,3]),colour = "blue") +
  labs(title = paste("T FB vs T insi","\n Adj R2 = ", signif(fit$rsquare, 3),
                     "; Intercept =", signif(fit$regression.results[2,2], 3),
                     "; Slope =", signif( fit$regression.results[2,3], 3),
                     "; P =", signif(fit$P.param, 3))) +
  theme(aspect.ratio=1,plot.title = element_text(size=7)) + 
  coord_fixed(ratio = 1 )+ scale_colour_discrete(guide="none") +
  labs( x="T insi", y="T FB ", color = "Months")  +theme_bw()
print(pTfb_Tinsi)

d_all$diff_T_fb_insitu <- d_all$T_seaF - d_all$temp_mix
mean_diff_T_fb_insitu <- round(mean(d_all$diff_T_fb_insitu, na.rm = TRUE), digits = 2)
sd_diff_T_fb_insitu <- round(sd(d_all$diff_T_fb_insitu, na.rm = TRUE), digits = 2)
```

The mean difference between T seaFET and in situ is: `r mean_diff_T_fb_insitu` ± `r sd_diff_T_fb_insitu`°C.

### **T 11 m / T CTD > 8 m **

Property-property temperature plot using:

fit <- lmodel2(data = d_all%>%dplyr::filter(pressure_insitu_ctd_filtered >=8),  temp_insitu_11m ~ temp_insitu_ctd , nperm = 99)
...
p<-  ggplot(d_all,aes(x=temp_insitu_ctd, y= temp_insitu_11m, label = month))+
  geom_text( aes(color=factor(month))) + 
 # scale_color_viridis_d(option = "C", guide="none")+
  geom_line(aes(x = temp_insitu_11m, y = temp_insitu_11m), linetype = "dashed") +
  geom_abline(aes(intercept = fit$regression.results[2,2], slope = fit$regression.results[2,3]),colour = "blue") +
  labs(title = paste("T FB vs T insi","\n Adj R2 = ", signif(fit$rsquare, 3),
                     "; Intercept =", signif(fit$regression.results[2,2], 3),
                     "; Slope =", signif( fit$regression.results[2,3], 3),
...
d_all$diff_T_fb_insitu <- d_all$T_seaF - d_all$temp_insitu_ctd
mean_diff_T_fb_insitu <- round(mean(d_all$diff_T_fb_insitu, na.rm = TRUE), digits = 2)
sd_diff_T_fb_insitu <- round(sd(d_all$diff_T_fb_insitu, na.rm = TRUE), digits = 2)
```
...
```{r TA since November 2018, echo=FALSE, warning=FALSE, message=FALSE,out.width='100%',fig.align='hold'}
 ...
 at_nov_2018 <-
  dplyr::select(d_all, datetime, at_calc, AT_filtered, at) %>%
  filter(datetime > "2018-11-01", AT_filtered > 2250)
#despike
at_nov_2018 <- mutate(at_nov_2018, AT_filtered_despike = despike(AT_filtered, reference= "median", n=0.5, k=10, replace="NA"))
#plot
at_datetime <- ggplot(data = at_nov_2018, aes(x=datetime)) +
  geom_point(aes(y=AT_filtered, colour = "before"), size =3) +
  geom_point(aes(y=AT_filtered_despike, colour = "after"), size = 2) +
  labs(x = "Time", y = "Total alkalinity", color = "despike")
print(at_datetime)

at_nov_2018_no_NA <- dplyr::filter(at_nov_2018, at_calc != "NA" & AT_filtered_despike != "NA")
titre <- "Contros AT vs Calculated AT"
fit <- lmodel2(data = at_nov_2018_no_NA, AT_filtered_despike ~ at_calc, nperm = 99)
ggreg2(fit, x = "at_calc", y = "AT_filtered_despike")
...
```{r pCO2 2nd cleaning, echo=FALSE, warning=FALSE, message=FALSE}
...
pco2calc_pCO2_contros_text <-  ggplot(d_all,aes(x=pco2calc, y= pco2_contros, label = month))+
   geom_text(aes(color = factor(month)))+
  scale_color_discrete(guide="none")+
  geom_line(aes(x = pco2_contros, y = pco2_contros), linetype = "dashed") +
  geom_abline(aes(intercept = fit$regression.results[2,2], slope = fit$regression.results[2,3]),colour = "blue") +
  labs(title = paste(title,"\n Adj R2 = ", signif(fit$rsquare, 3),
                     "; Intercept =", signif(fit$regression.results[2,2], 3),
                     "; Slope =", signif( fit$regression.results[2,3], 3),
...
ggsave(paste0("figures/pco2calc_pCO2_contros.png"), pco2calc_pCO2_contros_text, units="cm")
```
...
#   geom_point(data=d_all, aes(x=datetime, y=pco2calc, shape="lab" , color= "red"))+
...
### **seaFET data cleaning**

```{r seaFET 2nd cleaning, echo=FALSE, message= FALSE, warning=FALSE, out.width= "60%"}
...
# sfPeriod <- d_all%>%
#   dplyr::select(datetime, pH_s_seafet_temp_insitu)%>%
#   dplyr::filter(!is.na(pH_s_seafet_temp_insitu))
...
mean_TseaF_4h <- d_all%>%
  dplyr::filter(datetime >= "2017-08-24 19:00:00" & datetime <= "2017-08-24 22:00:00")%>%
  dplyr::summarise( mean(T_seaF, na.rm = TRUE))
mean_TseaF_4h <- as.numeric(mean_TseaF_4h)
d_all <- d_all%>%
  dplyr::mutate(temp_insitu = ifelse(datetime == "2017-08-24 13:00:00" , mean_TseaF_4h, temp_insitu))
...
mean_TseaF_4h <- as.numeric(mean_TseaF_4h)
d_all <- d_all%>%
  dplyr::mutate(temp_insitu = ifelse(datetime == "2019-09-13 13:00:00" , mean_TseaF_4h, temp_insitu))

#interpolation of salinity during the FB off maintenance
salinterp <-approx(d_all$datetime, d_all$sal, xout=d_all$datetime, method="linear", rule=2)
names(salinterp) <- c("datetime", "sal_interp")
salinterp <- as.data.frame(salinterp)
d_all$sal_interp <- salinterp$sal_interp

d_all$sal <- ifelse(d_all$datetime > "2018-03-09 00:00:00" & d_all$datetime < "2018-04-16 16:00:00", d_all$sal_interp, d_all$sal)

# test <- d_all%>%
#   dplyr::filter(datetime=="2017-08-24 14:00:00")

####
## calculation of E0INT25/E0EXT25 ##
##     new sf_calib function      ##
####
calib_spectro <- sf_calib(calEint = d_all$voltINT, calEext = d_all$voltEXT, calpH = d_all$pH_s_seafet_temp_insitu,
                          calT = d_all$temp_insitu, calSal = d_all$sal )
...
####
seafet_calib <- sf_calc(calEint = d_all$voltINT, calEext = d_all$voltEXT, E0int25 = d_all$E0int25_sf,  E0ext25 = d_all$E0ext25_sf, calT = d_all$temp_insitu, calSal = d_all$sal_interp)
...
d_all <- d_all%>%
  dplyr::mutate( pH_s_sf_no_calb = ifelse(datetime == "2018-04-16 07:00:00" | datetime == "2018-12-14 09:00:00" | datetime == "2019-01-18 09:00:00", pH_s_seafet_temp_insitu, NA ))

seafet_contros_xts <- dplyr::select(d_all, datetime, phINT_filtered,pH_s_seafet_temp_insitu,pHint_tot_sf, pH_s_sf_no_calb)
...
dygraph(seafet_contros_xts, group = "awipev", main="pH seaFET (in situ)", ylab="pHT") %>%
  ...
  dySeries("pHint_tot_sf", label = "pHT corrected", color =  RColorBrewer::brewer.pal(3, "Dark2")[1], strokeWidth = 0, pointSize = 2) %>%
    dySeries("phINT_filtered", label = "Raw data INT", color ="blue", strokeWidth=0, pointSize=2) %>%
    dySeries("pH_s_seafet_temp_insitu", label = "pH spectro for calibration", color =  "red", strokeWidth = 0, pointSize = 4 ) %>%
  ...
  dyShading(from = sfinterv[11], to= sfinterv[12], color = "#CCCCCC") %>%
  dyShading(from = sfinterv[12], to= sfinterv[13], color = "#E0E0E0") %>%
  dyShading(from = sfinterv[13], to= sfinterv[14], color = "#CCCCCC") %>%
   dyShading(from = sfinterv[14], to= sfinterv[15], color = "#E0E0E0") %>%
   dyShading(from = sfinterv[15], to= sfinterv[16], color = "#CCCCCC") %>%
   dyShading(from = sfinterv[16], to= sfinterv[17], color = "#E0E0E0") %>%
  dyShading(from = sfinterv[17], to= sfinterv[18], color = "#CCCCCC") %>%
  dyShading(from = sfinterv[18], to= sfinterv[19], color = "#E0E0E0") %>%
  dyShading(from = sfinterv[19], to= sfinterv[20], color = "#CCCCCC") %>%
   dyShading(from = sfinterv[20], to= sfinterv[21], color = "#E0E0E0") %>%
   dyShading(from = sfinterv[21], to= sfinterv[22], color = "#CCCCCC") %>%
  dyShading(from = sfinterv[22], to= Sys.Date(), color = "#E0E0E0") %>%
  dyLegend(show = "auto")   %>%
  dyHighlight(highlightCircleSize = 8,highlightSeriesBackgroundAlpha = 0.5,hideOnMouseOut = TRUE) %>%
  dyOptions(drawGrid = TRUE, drawPoints = TRUE, pointSize = c(1,8,1),useDataTimezone = TRUE, digitsAfterDecimal=4) %>%
  dyRangeSelector(height = 30, dateWindow= NULL) 
...
# # #@@@@@ test plot #@@@@@ #@@@@@ #@@@@@
seafet_contros_xts <- dplyr::select(d_all, datetime, temp_fb, temp_insitu)#voltINT, phINT_filtered,pH_s_seafet_temp_insitu, T_seaF, temp_fb, temp_insitu, sal)
#seafet_contros_xts <- dplyr::select(d_all, datetime,voltINT)
seafet_contros_xts <- as.xts(seafet_contros_xts, order.by = d_all$datetime)
dygraph(seafet_contros_xts, group = "awipev", main="pH seaFET (in situ)", ylab="pHT") %>%
  #dySeries("voltINT", label = "voltINTINT", color ="blue", strokeWidth=0, pointSize=2) %>%
   # dySeries("pH_s_seafet_temp_insitu", label = "pH spectro", axis= "y2", color =  "green", strokeWidth = 0, pointSize = 4 ) %>%
...
dySeries("temp_fb", label = "temp_fb", color ="blue", strokeWidth=0, pointSize=2) %>%
 # dySeries("sal", label = "sal", color ="black",axis= "y2", strokeWidth=0, pointSize=2) %>%
   # dySeries("T_seaF", label = "T_seaF", color ="grey",axis= "y2", strokeWidth=0, pointSize=2) %>%
 #   dyEvent(as.POSIXct("2017-08-24 12:00:00", tz="GMT"), "seaFET #1005 installed", labelLoc = "bottom", color="red") %>%
  #dyEvent(as.POSIXct("2018-02-02 09:00:00", tz="GMT"), "seaFET settings changed to every 1H", labelLoc = "bottom", color="red") %>%
  #dyEvent(as.POSIXct("2018-04-17 12:00:00", tz="GMT"), "seaFET #0007 installed + continuous mode set", labelLoc = "bottom", color="red") %>%
   # dyEvent(as.POSIXct("2019-09-02 12:00:00", tz="GMT"), "seaFET #0007 out of water for cleaning", labelLoc = "bottom", color="red") %>%
...
# dat <- mutate(sfoffset, diff_pH_sf = round((pH_s_seafet_temp_insitu - pHint_tot_sf),digits  = 4))
...
#   geom_point(data=d_all%>%dplyr::filter(datetime > "2017-07-30"  ), aes(x=datetime, y=pH_s_seafet_temp_insitu, fill="Discrete measurements"), col="red" )+
...
durPeriod <- d_all%>%
  dplyr::select(datetime, pH_s_durafet_temp_insitu)%>%
  dplyr::filter(!is.na(pH_s_durafet_temp_insitu))
...
duroffset <- left_join(durPeriod%>%dplyr::select(datetime,pH_s_durafet_temp_insitu), d_all%>%dplyr::select(datetime,pH_durafet ), by="datetime")
dat_dur <- mutate(duroffset, diff_pH_dur = pH_s_durafet_temp_insitu - pH_durafet)
...
d_all$pH_durafet_corrected <- ifelse(d_all$datetime >= "2018-07-10 00:00:00" & d_all$datetime <= "2018-07-10 05:00:00", NA, d_all$pH_durafet_corrected)
d_all$pH_durafet <- ifelse(d_all$datetime >= "2018-07-10 00:00:00" & d_all$datetime <= "2018-07-10 05:00:00", NA, d_all$pH_durafet)

# DURAFET 
# add spectro points that are not taking into consideration
d_all <- d_all%>%
  dplyr::mutate( pH_s_dur_no_calb = ifelse(datetime == "2017-08-24 13:00:00", pH_s_durafet_temp_insitu, NA ))

durafet_contros_xts <- dplyr::select(d_all, datetime, pH_durafet, pH_s_durafet_temp_insitu,pH_durafet_corrected, pH_s_dur_no_calb)
durafet_contros_xts <- as.xts(durafet_contros_xts, order.by = d_all$datetime)
 dygraph(durafet_contros_xts, group = "awipev", main="pH durafet (in Ferrybox)", ylab="pHT") %>%
    dySeries("pH_durafet", label = "Raw durafet pH", color ="blue", strokeWidth=0, pointSize=2) %>%
    dySeries("pH_s_durafet_temp_insitu", label = "pH spectro for calibration", color =  "red", strokeWidth = 0, pointSize = 6 ) %>%
...
# #plot of pH durafet
#  poster_dur <- ggplot( ) +
#   geom_point(data=d_all%>%dplyr::filter(datetime > "2017-07-30"  ), aes(x=datetime, y= pH_durafet_corrected,col="blue"), col="blue",size=0.5 , rm.na=TRUE) +
#   geom_point(data=d_all%>%dplyr::filter(datetime > "2017-07-30"  ), aes(x=datetime, y=pH_s_durafet_temp_insitu, fill="Discrete measurements"), col="red" )+
...
range_dur <- tbl_df(
  ...
  summarize(pH_range = max(pH_durafet_corrected) - min(pH_durafet_corrected))
)
...
range_month <-
  ...
  #summarize(pH_range_month = mean(pH_range, na.rm=TRUE))%>%
  summarize(Tinsi_max =round( max(temp_insitu, na.rm=TRUE), digits = 2),
            Tinsi_min = round(min(temp_insitu, na.rm=TRUE), digits = 2),
            sal_max = round(max(sal, na.rm=TRUE), digits = 2),
            sal_min = round(min(sal, na.rm=TRUE), digits = 2),
...
pH_sf <- d_all %>%
  dplyr::select(datetime, sal,temp_fb,temp_insitu, pHint_tot_sf)%>%
  dplyr::filter(!is.na(pHint_tot_sf), !is.na(temp_insitu), !is.na(temp_fb), !is.na(sal)) %>%
  dplyr::filter(datetime< "2018-01-01" | datetime> "2018-02-03")
pH_sf$pHint_tot_sf_norm4 <- pHinsi(pH=pH_sf$pHint_tot_sf, ALK=2.273e-3, Tinsi=4, Tlab=pH_sf$temp_insitu, Pinsi=11, S=pH_sf$sal, Pt=0, Sit=0, k1k2 = "x", kf = "x", ks="d", pHscale = "T", b="u74")

pH_dur <- d_all %>%
  dplyr::select(datetime, sal,temp_fb,temp_insitu, pH_durafet_corrected)%>%
  dplyr::filter(!is.na(pH_durafet_corrected), !is.na(temp_insitu), !is.na(temp_fb), !is.na(sal))
pH_dur$pH_durafet_norm4 <- pHinsi(pH=pH_dur$pH_durafet_corrected, ALK=2.273e-3, Tinsi=4, Tlab=pH_dur$temp_insitu, Pinsi=11, S=pH_dur$sal, Pt=0, Sit=0, k1k2 = "x", kf = "x", ks="d", pHscale = "T", b="u74")
...
pH_calc <- discrete %>%
  ...
  dplyr::filter(!is.na(phcalc), !is.na(temp), !is.na(sal))
...
# Spectro / durafet
fit <- lmodel2(data = d_all,  pH_s_durafet_temp_insitu ~ pH_durafet_corrected , nperm = 99)

p_dura_spectro <-  ggplot(d_all,aes(x=pH_durafet_corrected, y= pH_s_durafet_temp_insitu, label = month))+
  geom_text( aes(color=factor(month))) + 
  scale_color_discrete(guide = "none")+
  geom_line(aes(x = pH_calc_norm4, y = pH_calc_norm4), linetype = "dashed") +
  geom_abline(aes(intercept = fit$regression.results[2,2], slope = fit$regression.results[2,3]),colour = "blue") +
...
# Spectro / seafet
fit <- lmodel2(data = d_all,  pH_s_seafet_temp_insitu ~ pHint_tot_sf , nperm = 99)

p_sf_spectro <-  ggplot(d_all,aes(x=pHint_tot_sf, y= pH_s_seafet_temp_insitu, label = month))+
  geom_text( aes(color=factor(month))) + 
  scale_color_discrete(guide = "none")+
  geom_line(aes(x = pH_calc_norm4, y = pH_calc_norm4), linetype = "dashed") +
  geom_abline(aes(intercept = fit$regression.results[2,2], slope = fit$regression.results[2,3]),colour = "blue") +
