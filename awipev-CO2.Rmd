...
#restore datetime, date which are mangled by the mutates above
...
pH_seafet <- pH_seafet %>% 
  dplyr::mutate(ph_s_sf_temp_insi = pHinsi(pH = pH_seafet$ph_s_sf, ALK = pH_seafet$at_mean_interp*1e-6, Tinsi = pH_seafet$temp_insitu_ctd, Tlab = pH_seafet$temp_s_lab, Pinsi = pH_seafet$pressure_insitu_ctd/10, S = pH_seafet$sal_mix, Pt = 0, Sit = 0))
...
##   *****Attention, temp noted by logisticians in d is FB temp with SBE45 ****** $SA$
...
# pH calculated (at and ct) at in situ (11 m) sal and temp
discrete_no_na <- discrete %>%
  dplyr::filter(!is.na(sal_mix)  & !is.na(temp_insitu_11m) & !is.na(at) & !is.na(ct))
discrete_is_na <- discrete %>%
  dplyr::filter(is.na(sal_mix)  | is.na(temp_insitu_11m) | is.na(at) | is.na(ct))
carb <- carb(15, discrete_no_na$at*1e-6, discrete_no_na$ct*1e-6, S = discrete_no_na$sal_mix, T = discrete_no_na$temp_insitu_11m, P = 11/10, Pt = 0, Sit = 0, k1k2 = "l", kf = "dg", ks = "d", pHscale = "T", b = "u74")
discrete_no_na$phcalc_insitu <- carb$pH # pH to compare with durafet pH (at Ferrybox temp and sal)
discrete_no_na$pco2_calc_insitu <- carb$pCO2

#create new variable in "discrete_is_na" to bind "discrete_no_na"
discrete_is_na$phcalc_insitu <- NA
discrete_no_na$pco2_calc_insitu <- NA
discrete <- bind_rows(discrete_no_na, discrete_is_na) %>% 
  dplyr::arrange(date)
