...
# calculate fCO2 from pH and salinity-derived AT
  dplyr::select(datetime, depth, pHint_tot_sf, sal_insitu_ctd, temp_insitu_ctd) %>% 
  drop_na() %>% 
  dplyr::mutate(at_0_4m = (intercept_s_at + slope_s_at * sal_insitu_ctd) * 1e-6,
                pco2_0_4m = seacarb::carb(flag=8, var1=pHint_tot_sf, var2=tmp$at_0_4m)$pCO2)
tmp <- d_all %>% 
  dplyr::select(montht, pHint_tot_sf_temp_4, depth_group) %>%
  tidyr::drop_na() %>% 
  dplyr::filter(depth_group != "NA") %>% 
...
Let's look at the stratification tab above (salinity, temperature, density and pH).


## pCO2 from pH and AT 0-4m
If one calculates pCO2 from seafet pH and AT (from salinity) between 0-4 m, do we get better estimates of air-sea CO2 fluxes?
...
tmp <- d_all %>% 
  dplyr::select(datetime, pHint_tot_sf, sal_insitu_ctd, temp_insitu_ctd) %>% 
  drop_na() %>% 
  dplyr::mutate(at_0_4m = (intercept_s_at + slope_s_at * sal_insitu_ctd) * 1e-6,
                pco2_0_4m = seacarb::carb(flag=8, var1=pHint_tot_sf, var2=tmp$at_0_4m)$pCO2)
ts_pCO2_calc <- tmp %>%
  ...
  geom_point(aes(x = datetime, y = pco2_0_4m), 
             colour="blue", alpha = alpha, na.rm = TRUE, size = size_point) +
  labs(title=expression(paste(pCO[2], "calc at 0-4 m and in situ temperature")),
