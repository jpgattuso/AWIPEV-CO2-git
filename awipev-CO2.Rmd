...
tmp <- d_all %>% 
  dplyr::select(montht, sal_insitu_ctd, depth, depth_group) %>%
  tidyr::drop_na() %>% 
...
temp_delta_plt <- tmp_delta %>% 
  ...
  scale_x_discrete(expand = c(0, 0)) +
  scale_y_continuous("Temperature", limits = c(-3, 3), expand = c(0, 0)) +
  geom_col(fill="blue") +
  geom_hline(yintercept = 0) +
  annotate("text", x = 1.5, y = 2.5, label = substitute("T"["surface"] ~ "> T"["deep"])) +
  annotate("text", x = 1.5, y = -2.5, label = substitute("T"["surface"] ~ "< T"["deep"])) +
  Mytheme(size_labs = 12) +
  theme(panel.grid.major.x = element_blank(), panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank())
