...
tempmixinterp <-approx(d_all$datetime, d_all$temp_insitu_11m_mix, xout=d_all$datetime, method="linear", rule=2)
...
salmixinterp <-approx(d_all$datetime, d_all$sal_fb_mix, xout=d_all$datetime, method="linear", rule=2)
  ...
  dplyr::mutate(
    sal_fb_mix = case_when(
      ...
      date > as.Date("2015-10-01") &
        date <  as.Date("2015-10-05") & sal_fb_mix < 31.5 ~ NA_real_,
...
fit <- lm(data = d_all_at_discrete, at ~ sal_fb_mix)
slope_s_at <- fit$coefficients[[2]]
intercept_s_at <- fit$coefficients[[1]]
at_s <- ggplot(d_all_at_discrete,aes(x=sal_fb_mix, y= at, label = monthd))+
  geom_text( aes(color=factor(monthd))) +
...
# Same plot for paper
p_at_s <- ggplot(d_all_at_discrete,aes(x=sal_fb_mix, y= at)) +
...
# # below is old code. now we use sal_insitu_ctd_mix
# salinterp <-
#   approx(
#     d_all$datetime,
...
# above is old code. now we use sal_insitu_ctd_mix
salinterp <-
  approx(
    d_all$datetime,
...
# TODO the dataframe below is huge, cleaning it would make the script faster
...
pangaea <- as_tibble(d_all) %>% 
  dplyr::select(datetime, depth, sal_insitu_ctd, sal_fb, sal_fb_mix, temp_insitu_11m, temp_insitu_ctd, temp_fb, temp_sf, temp_dur, at, ct, ph_s_sf_temp_insi, ph_s_dur_temp_fb, ph_calc_fb, pco2_fb, pco2_calc_fb, at_calc, pH_dur, pHint_tot_sf) %>%
...
-   sal_insitu_ctd_mix: when CTD salinity data from the REMOS are missing, they are replaced by data from sal_fb_mix (hence at FerryBox from 11 m and REMOS below 8 m). This is not ideal because difference of up to 1 salinity unit occur in the water column. But needed as there are many missig REMOS data.
