...
pco2_fb_remos <- d_all %>% 
  ...
  labs(title = "pCO2 at ctd temperature",
       x=latex2exp::TeX("$pCO_{2}$ 11 m $\\phantom{x}(\\muatm)$"),
       y= latex2exp::TeX("$pCO_{2}$ REMOS at 0-2 m\\phantom{x}($\\muatm)$"))  +
  ggpmisc::stat_ma_line(method = "lmodel2:MA", se = FALSE, size = 1) +
  ggpmisc::stat_ma_eq(method = "lmodel2:MA", aes(label = after_stat(eq.label))) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +  
  theme_bw() + Mytheme(size_labs = 9) + theme(aspect.ratio = 1) +
  coord_fixed(ratio = 1, expand = TRUE, clip = "on")
pco2_fb_remos
ggsave(filename = "figures/pco2_fb_remos.png", plot = pco2_fb_remos, width = 25, height = 25, units = "cm")
```
...
d_all <- d_all %>%
  dplyr::mutate(delta_pco2 = pco2_fb - co2_air, # ppm
                ...
                alpha = seacarb::K0(S=sal_fb_mix,T=temp_insitu_11m,P=0,Patm=p_air/1013.25), #mol/(kg/atm)
                alpha = alpha * (seacarb::rho(S = sal_fb_mix, T = temp_insitu_11m, P = 0)/1000), #mol/(l/atm)
