...
d <- left_join(d, d_all%>%dplyr::select(c(closest_datetime, sal_fb, temp_fb, temp_sf, temp_mix, temp_insitu_11m, sal_mix)), by="closest_datetime") 

# Conversion of spectro pH to insitu mix T°
pH_seafet <- d %>%
  dplyr::filter(!is.na(pH_s_seafet) & !is.na(temp_sf) )
pH_seafet <- pH_seafet %>%
  dplyr::mutate(ph_s_sf_temp_mix = pHinsi(pH=pH_seafet$pH_s_seafet,ALK=pH_seafet$at_mean_interp*1e-6, Tinsi=pH_seafet$temp_sf, Tlab=pH_seafet$temp_s_lab, Pinsi= pH_seafet$depth/10, S=pH_seafet$sal_mix,Pt=0,Sit=0))
...
calib_spectro <- sf_calib(calEint = d_all$voltINT, calEext = d_all$voltEXT, calpH = d_all$ph_s_sf_temp_mix,
                          calT = d_all$temp_sf, calSal = d_all$sal_mix_interp)
