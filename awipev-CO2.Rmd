...
```{r ESSD time series, echo=FALSE, message= FALSE, warning=FALSE}
...
d <- as_tibble(d_all) %>%
  dplyr::select(datetime, pressure_insitu_ctd, temp_insitu_11m, sal_mix, pco2_corr, pHint_tot_sf, 
                ph_dur_corr, pco2_calc_insitu, at_calc)
...
```
```{r ESSD boxplots, echo=FALSE, message= FALSE, warning=FALSE}
...
```
```{r ESSD pCO2, echo=FALSE, warning=FALSE, message=FALSE}
fit <- lmodel2(data = d,  pco2_corr ~ pco2_calc_insitu , nperm = 99)

p <-  ggplot(d, aes(x=pco2_calc_insitu, y= pco2_corr)) +
