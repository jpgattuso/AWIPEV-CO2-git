...
durinterv <- c( as_datetime("2017-08-24 13:00:00"), durPeriod[,4])
 #Change period sfinterv[6]. Because seafet was off and on on the 2018-04-24 19:00:00
durinterv[13] <- ymd_hms("2019-01-17 23:00:00")
 #Change period sfinterv[6]. Because seafet was off and on on the 2018-04-24 19:00:00
...
range_month <-
  ...
  #summarize(pH_range_month = mean(pH_range, na.rm=TRUE))%>%
  summarize(Tinsi_max = max(temp_insitu, na.rm=TRUE),
            Tinsi_min = min(temp_insitu, na.rm=TRUE),
            Tfrbx_max = max(temp_fb, na.rm=TRUE),
            Tfrbx_min = min(temp_fb, na.rm=TRUE),
            pH_sf_max = max(pHint_tot_sf, na.rm=TRUE),
            pH_sf_min = min(pHint_tot_sf, na.rm=TRUE),
            pH_dur_max = max(pH_durafet_corrected, na.rm=TRUE),
            pH_dur_min = min(pH_durafet_corrected, na.rm=TRUE))
kable(range_month)
```
