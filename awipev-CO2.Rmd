...
d_all <- d_all %>%
  ...
  dplyr::mutate(pHint_tot_sf_9m = ifelse(pressure_insitu_ctd > 8, pHint_tot_sf, NA),
                pHint_tot_sf_6m = ifelse(pressure_insitu_ctd > 6, pHint_tot_sf, NA))
...
p <- ggplot(d_all, aes(x=datetime, y= pHint_tot_sf_9m))+
  geom_point(size=1, alpha=0.5, na.rm=TRUE)
p

seafet_depth_xts <- dplyr::select(d_all, datetime, ph_s_sf_temp_insi, pHint_tot_sf_6m, pHint_tot_sf_9m)
...
dySeries("pHint_tot_sf_6m", label = "pHT 6m", color =  RColorBrewer::brewer.pal(3, "Dark2")[2], strokeWidth = 0, pointSize = 1) %>%
...
range <- d_all %>%
  group_by(date=as.Date(datetime), 
           ...
           year=year(datetime)) %>%
  summarize(pH_range_9m = max(pHint_tot_sf_9m) - min(pHint_tot_sf_9m),
            pH_range_6m = max(pHint_tot_sf_6m) - min(pHint_tot_sf_6m))

quant <- quantile(range$pH_range_9m, probs =c(2.5/100, 97.5/100), na.rm =TRUE)
print(quant)
quant <- quantile(range$pH_range_6m, probs =c(2.5/100, 97.5/100), na.rm =TRUE)
print(quant)
...
range_month <-
  ...
  summarize(pH_range_month6m = mean(pH_range_6m, na.rm=TRUE),
            pH_range_month_9m = mean(pH_range_9m, na.rm=TRUE)) %>%
  ungroup()

size_point <- 0.4 #size of data points
p <- range_month %>%
  pivot_longer(-Month, names_to = "depth", values_to = "pH_range") %>%
  ggplot() +
  #geom_violin(aes(x = as.factor(Month), y = pH_range), group = depth, fill='lightblue', alpha=0.5, size=0.2, trim = TRUE) +
  geom_boxplot(aes(x = as.factor(Month), y = pH_range), , group = depth, notch=TRUE, size=0.2, fill="grey50", outlier.color = "blue", outlier.size = 0.3) +
...
p <-  ggplot(d_all, aes(x=ph_s_dur_temp_fb, y= ph_dur_corr, label = month)) +
  geom_text( aes(color=factor(month))) + 
  scale_color_discrete(guide = "none")+
  geom_abline(slope=1, intercept =0, linetype = "dashed") +
  geom_abline(aes(intercept = lm$fit$regression.results[2,2], slope = lm$fit$regression.results[2,3]),
...
p <-  ggplot(d_all, aes(x=ph_s_sf_temp_insi, y= pHint_tot_sf, label = month)) +
  geom_text( aes(color=factor(month))) + 
  scale_color_discrete(guide = "none")+
  geom_abline(slope=1, intercept =0, linetype = "dashed") +
  geom_abline(aes(intercept = lm$fit$regression.results[2,2], slope = lm$fit$regression.results[2,3]),
...
+ seawater pCO2
+ atmospheric CO2 measured at the Zeppelin station and downloaded 2020-08-19 from https://gaw.kishou.go.jp/search/file/0054-6001-1001-01-01-9999 
...
d_all <- d_all %>%
  dplyr::mutate(delta_pco2 = pco2_corr - pco2_air, # ppm
...
air_sea_month <- d_all %>%
  dplyr::select(month, f_air_sea) %>%
  dplyr::group_by(month) %>%
  dplyr::summarise(f_air_sea = 24 * mean(f_air_sea, na.rm=TRUE)/1000) %>% #mol/(m2 d)
  ungroup() %>%
  dplyr::mutate(f_air_sea = days_in_month(month) * f_air_sea)
#air_sea_month
p_f_air_sea_m <- ggplot(data = air_sea_month) +
  geom_point(aes(x = month(month, label = TRUE, abbr = TRUE), y = f_air_sea), col = "blue", size = 2, na.rm = TRUE) +
...
pangaea <- as_tibble(d_all) %>% 
  dplyr::select(datetime, pressure_insitu_ctd, sal_insitu_ctd, sal_fb, sal_mix, temp_insitu_11m, temp_insitu_ctd, temp_fb, temp_sf, temp_dur, at, ct, ph_s_sf_temp_insi, ph_s_dur_temp_fb, phcalc_fb, pco2_corr, pco2_calc_fb, at_calc, ph_dur_corr, pHint_tot_sf) %>%
