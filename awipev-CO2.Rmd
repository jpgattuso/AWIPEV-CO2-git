...
d_hour <- readRDS(file = paste0(path, "fb_awipev-co2_server/ny-alesund/data/NRT_data/nydata_hour.rds"))
d_hour <- d_hour %>%
  dplyr::select(-date) %>%
  dplyr::rename(pco2_corr = pco2_corr_filtered, temp_dur = temp_dur_filtered, pco2_raw = pco2_raw_filtered, sal_fb = sal_fb_filtered, temp_insitu_11m=temp_insitu_11m_filtered, temp_fb = temp_fb_filtered, ph_dur = ph_dur_filtered)
...
d <- left_join(d, d_all%>%dplyr::select(c(closest_datetime, sal_fb, temp_fb, temp_mix, sal_mix, temp_insitu_ctd)), by="closest_datetime") 

# Conversion of spectro pH to insitu mix T°
pH_seafet <- d %>%
  dplyr::filter(!is.na(pH_s_seafet) & !is.na(temp_insitu_ctd) )
pH_seafet <- pH_seafet %>%
  dplyr::mutate(ph_s_sf_temp_mix = pHinsi(pH=pH_seafet$pH_s_seafet,ALK=pH_seafet$at_mean_interp*1e-6, Tinsi=pH_seafet$temp_insitu_ctd, Tlab=pH_seafet$temp_s_lab, Pinsi= pH_seafet$depth/10, S=pH_seafet$sal_mix,Pt=0,Sit=0))
...
qflag <- function(x) {
  summary(as.factor(x))*100/length(x)
}
selected_nydata_minute <- readRDS(file = paste0(path, "fb_awipev-co2_server/ny-alesund/data/NRT_data/nydata_minute.rds")) %>%
dplyr::select(ends_with("_qf"), -dt_qf) 
nam <- selected_nydata_minute %>% names()
selected_nydata_minute %>% map(~qflag(.x))

s1 <- summary(as.factor(selected_nydata_minute$pco2_corr_qf))*100/length(selected_nydata_minute$pco2_corr_qf)
s2 <- summary(as.factor(selected_nydata_minute$sal_fb_qf))*100/length(selected_nydata_minute$sal_fb_qf)
s3 <- summary(as.factor(selected_nydata_minute$sal_insitu_ctd_qf))*100/length(selected_nydata_minute$sal_insitu_ctd_qf)
s4 <- summary(as.factor(selected_nydata_minute$temp_dur_qf))*100/length(selected_nydata_minute$temp_dur_qf)
s5 <- summary(as.factor(selected_nydata_minute$temp_fb_qf))*100/length(selected_nydata_minute$temp_fb_qf)
s6 <- summary(as.factor(selected_nydata_minute$temp_insitu_11m_qf))*100/length(selected_nydata_minute$temp_insitu_11m_qf)
s7 <- summary(as.factor(selected_nydata_minute$phINT_qf))*100/length(selected_nydata_minute$phINT_qf)
s8 <- summary(as.factor(selected_nydata_minute$phEXT_qf))*100/length(selected_nydata_minute$phEXT_qf)
z <- t(bind_rows(s1,s2,s3,s4,s5,s6,s7,s8))
colname(z) <- nam

qflag <- function(x) {
  summary(as.factor(x))*100/length(x)
}
qflags <- tibble(pco2_corr = s1,
                 sal_fb = s2,
                 sal_insitu_ctd = s3,
                 temp_dur = s4,
                 temp_fb = s5,
  ...
  summarise_all(percentcumsum)
  dplyr::mutate_all(funs("percent"= cumsum(.)/100))
p <-  ggplot(data = selected_nydata_minute) +
  geom_bar(aes(x = as.factor(pco2_corr_qf)))
p
# rm(selected_nydata_minute)
```
...
dygraph(pco2_contros_xts,
  ...
  dyEvent(
    as.POSIXct("2016-02-28 12:00:00", tz = "GMT"),
    "CO2FT-0515-001 installed",
    labelLoc = "bottom",
    color = "red"
  ) %>%
  dyEvent(
    as.POSIXct("2017-02-07 12:00:00", tz = "GMT"),
    "CO2FT-0215-001 installed",
    labelLoc = "bottom",
    color = "red"
  ) %>%
  dyEvent(
    as.POSIXct("2018-04-16 12:00:00", tz = "GMT"),
    "CO2FT-0515-001 installed",
    labelLoc = "bottom",
    color = "red"
  ) %>%
  dyEvent(
    as.POSIXct("2018-10-31 15:00:00", tz = "GMT"),
    "CO2FT-0215-001 installed",
    labelLoc = "bottom",
    color = "red"
  ) %>%
  dyEvent(
    as.POSIXct("2019-09-03 15:00:00", tz = "GMT"),
    "CO2FT-0515-001 installed",
    labelLoc = "bottom",
    color = "red"
  ) %>%
...
# pco2_contros_xts <-
#   dplyr::select(d_all, datetime, voltINT, voltEXT, sal_fb, temp_insitu_ctd, ph_s_sf_temp_mix)
# pco2_contros_xts <-
#   as.xts(pco2_contros_xts, order.by = d_all$datetime)
# dygraph(pco2_contros_xts,
#         group = "awipev",
...
####
calib_spectro <- sf_calib(calEint = d_all$voltINT, calEext = d_all$voltEXT, calpH = d_all$ph_s_sf_temp_mix, calT = d_all$temp_sf, calSal = d_all$sal_mix_interp)
...
####
seafet_calib <- sf_calc(calEint = d_all$voltINT, calEext = d_all$voltEXT, E0int25 = d_all$E0int25_sf,  E0ext25 = d_all$E0ext25_sf, calT = d_all$temp_sf, calSal = d_all$sal_mix_interp)
...
seafet_depth_xts <- dplyr::select(d_all, datetime, ph_s_sf_temp_mix, pHint_tot_sf, pH_s_sf_no_calb)
...
* calibrate seafet pH
    ...
    - E0int25_sf: calculé à partir du voltage interne + ph_s_sf_temp_mix (temp 11m + temp_insitu_ctd_9m si trous) + temp CTD 9m + sal CTD 9m (calib seaFET fonction 1)
...
* (Every hour on the hour, a crontab action runs automatically the script ny-alesund/R/ny-alesund_cc.R. This script essentially appends the daily files from AWI-NRT database and cleans the data)
    + 00 *//1 * * * cd $HOME; USER=awipev R --vanilla < ny-alesund/R/ny-alesund_cc.R > ny-alesund/R/log_ny-alesund_cc.txt 2>&1
* (Every hour at 10 min past the hour, the web page is generated)
    + 10 *//1 * * * bash web/awipev-co2.sh > web/awipev-co2.log 2>&1  
