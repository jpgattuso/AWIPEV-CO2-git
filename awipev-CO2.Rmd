...
tmp_delta <- tmp %>%
  ...
  rbind(data.frame(montht="Jun", median8_12m = NA, median0_4m = NA, pco2_sf_med_delta = NA)) %>% 
...
pco2_delta_plt <- tmp_delta %>%
  ...
  labs(y = latex2exp::TeX("$pCO_2$ at in situ temperature ($\\mu atm$)")) +
  geom_col(fill = "blue") +
  geom_hline(yintercept = 0) +
  annotate(
    "text",
...
p <- tmp %>% 
  ...
  geom_boxplot(notch=FALSE, size=0.2, fill="grey50", outlier.color = "blue", outlier.size = 0.3) +
  labs(title = "pco2_sf at 4°C vs depth", x = "Depth (m)", y= "pCO2 (uatm)") +
  scale_x_discrete(labels = depth_labels) +
  facet_wrap(vars(montht), scale = "free_y")
print(p)
...
tmp_delta <- tmp %>%
  group_by(depth_group2, montht) %>%
  dplyr::summarise(pco2_sf_4_med = median(pco2_sf_4, na.rm = FALSE)) %>%
  tidyr::pivot_wider(names_from = depth_group2, values_from = pco2_sf_4_med, values_fill = NA) %>%
  dplyr::rename(median0_4m = "0-4",
                median8_12m = "8-12") %>%   
  dplyr::mutate(pco2_sf_4_med_delta = median0_4m - median8_12m) %>% 
    # this is to bring June back... 
  rbind(data.frame(montht="Jun", median8_12m = NA, median0_4m = NA, pco2_sf_4_med_delta = NA)) %>% 
  dplyr::mutate(montht = factor(montht, levels = month.abb)) %>% 
  dplyr::arrange(montht)
...
pco2_4_delta_plt <- tmp_delta %>%
  ...
  scale_x_discrete("Month", expand = c(0, 0)) +
  scale_y_continuous(limits = c(-46,10), expand = c(0, 0)) +
  labs(y = latex2exp::TeX("$pCO_2$ at 4°C ($\\mu atm$)")) +
  geom_col(fill = "blue") +
  geom_hline(yintercept = 0) +
  annotate(
    "text",
