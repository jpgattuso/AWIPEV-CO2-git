...
reg <- ggreg(
  ...
  #subtitle = "",
  xlab = "Salinity",
  ylab = TeX("Total alkalinity ($\\mu mol\\~kg^{-1})$"),
  proplot = TRUE,
  #ylim(c(30.5,35.5)) + xlim(c(28,35.5)),
  coord_fixed(ratio = 1)
)
...
p_at_s <- d_all_at_discrete %>% 
  ...
  labs(x="Salinity", y=TeX("Total alkalinity ($\\mu mol\\~kg^{-1})$"))  +
...
reg <- ggreg(
  ...
  #subtitle = "",
  xlab = latex2exp::TeX("Measured $pCO_{2}(\\mu atm)$"),
  ylab = latex2exp::TeX("Calculated $pCO_{2}(\\mu atm)$"),
  proplot = TRUE,
  #ylim(c(30.5,35.5)) + xlim(c(28,35.5)),
  coord_fixed(ratio = 1)
)
...
p <-
  ggplot(data = tidyr::drop_na(range_dur), 
         aes(x = as.factor(montht), y = pH_range)) +
