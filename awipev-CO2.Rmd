...
pH_seafet <- pH_seafet %>% 
  dplyr::mutate(
    ph_s_sf_temp_insi = pHinsi(pH = pH_seafet$ph_s_sf, 
                         ...
                         S = pH_seafet$sal_mix, Pt = 0, Sit = 0, k1k2 = "l", kf = "x", ks="d", 
                         pHscale = "T", b="u74"),
   ph_s_sf_norm4 = pHinsi(pH = pH_seafet$ph_s_sf, 
...
pH_seafet_mean <- pH_seafet%>%
  dplyr::select(date, ph_s_sf_temp_insi, ph_s_sf_norm4)%>% 
  dplyr::group_by(date) %>% 
  dplyr::summarise(ph_s_sf_temp_insi_mean = mean(ph_s_sf_temp_insi, na.rm = TRUE),
                   ...
                   ph_s_sf_norm4_mean = mean(ph_s_sf_norm4, na.rm = TRUE)
                   )
...
# colnames(seafetph) <- c("date", "ph_s_sf_insi", "ph_s_sf_norm4")
...
# seaFET pH 
discrete2merge <- discrete %>%
  dplyr::select(closest_datetime, ph_s_sf_temp_insi_mean, ph_s_sf_norm4_mean) %>%
  dplyr::rename(ph_s_sf_temp_insi = ph_s_sf_temp_insi_mean,
                ph_s_sf_norm4 = ph_s_sf_norm4_mean) %>%
  dplyr::filter(!is.na(closest_datetime)) %>%
  rename(datetime=closest_datetime)
d_all <- full_join(d_all, discrete2merge, by="datetime")
d_all <- d_all %>% 
...
# durafet pH 
discrete2merge <- discrete %>%
  dplyr::select(closest_datetime, ph_s_dur_temp_fb_mean) %>%
...
# d_all <- d_all %>%
#   dplyr::mutate(ph_s_sf_norm4 = ifelse(!is.na(pH_seafet$ph_s_sf),
...
pH_sf$pHint_tot_sf_norm4 <- pHinsi(pH=pH_sf$pHint_tot_sf, ALK=mean(d_all$at, na.rm=TRUE), Tinsi=4, Tlab=pH_sf$temp_insitu_ctd_9m, Pinsi=11/10, S=pH_sf$sal_insitu_ctd_9m, Pt=0, Sit=0, k1k2 = "l", kf = "x", ks="d", pHscale = "T", b="u74")

pH_dur <- d_all %>%
  dplyr::select(datetime, sal_fb,temp_fb, ph_dur_corr)%>%
  dplyr::filter(!is.na(ph_dur_corr), !is.na(temp_fb), !is.na(sal_fb))
pH_dur$pH_durafet_norm4 <- pHinsi(pH=pH_dur$ph_dur_corr, ALK=mean(d_all$at, na.rm=TRUE), Tinsi=4, Tlab=pH_dur$temp_fb, Pinsi=11/10, S=pH_dur$sal_fb, Pt=0, Sit=0, k1k2 = "l", kf = "x", ks="d", pHscale = "T", b="u74")
...
pH_calc$pH_calc_fb_norm4 <- pHinsi(pH = pH_calc$phcalc_fb, ALK=pH_calc$at*1e-6, Tinsi=4, Tlab=pH_calc$temp_fb, Pinsi=11/10, S=pH_calc$sal_fb, Pt=0, Sit=0, k1k2 = "l", kf = "x", ks="d", pHscale = "T", b="u74")
...
### Lueker vs Sulpis using means of monthly means
...
mean_at <-  mean(discrete$ct, na.rm = TRUE)
mean_pco2 <-  mean(d_all$pco2_calc_fb)
mean_sal <-  mean(discrete$sal_fb, na.rm = TRUE)
mean_temp <-  mean(discrete$temp_fb, na.rm = TRUE)

# calculate typical uncertainty assuming uncertainties of 3 uatm for pCO2 and 4 umol/kg for AT
errors_pco2_at <- seacarb::errors(flag=24, var1=mean_pco2_calc, var2=1e-6*mean_at_calc, S=mean_sal,
                          T=mean_temp, P=0, Pt=0, Sit=0, k1k2="l", kf="dg", ks="d", pHscale="T",
                          b="u74", evar1=3, evar2 = 4e-6)
errors_at_ct <- seacarb::errors(flag=15, var1=1e-6*mean_at_calc, var2=1e-6*mean_ct, S=mean_sal,
                          T=mean_temp, P=0, Pt=0, Sit=0, k1k2="l", kf="dg", ks="d", pHscale="T",
...
```
The means used are:
`r knitr::kable(means, format = "simple")`
...
### Lueker vs Sulpis using all values pCO2 and at_calc
```{r impact of constants 2, echo=FALSE, warning=FALSE, message=FALSE}
# Compare Lueker and Sulpis using pco2_fb and at_calc
tmp <- d_all %>% 
  tidyr::drop_na(pco2_fb, at_calc, sal_fb, temp_fb) %>% 
  dplyr::select(ph_s_sf_norm4, pco2_fb, at_calc, sal_fb, temp_fb)
...
# normalize derived pH at 4°C
carb_l$pH <- pHinsi(pH = carb_l$pH, 
                         ALK = carb_l$ALK, Tinsi = 4, 
                         Tlab = carb_l$T, Pinsi = 0, 
                         S = carb_l$S, Pt = 0, Sit = 0, k1k2 = "l", kf = "x", ks="d", 
                         pHscale = "T", b="u74")
carb_s <-
  carb(
    24,
...
# normalize derived pH at 4°C
carb_s$pH <- pHinsi(pH = carb_s$pH, 
                         ALK = carb_s$ALK, Tinsi = 4, 
                         Tlab = carb_s$T, Pinsi = 0, 
                         S = carb_s$S, Pt = 0, Sit = 0, k1k2 = "l", kf = "x", ks="d", 
                         pHscale = "T", b="u74")
...
tmp2 <- tibble(ph_s_sf_norm4 = tmp$ph_s_sf_norm4, pH_l = carb_l$pH, pH_s = carb_s$pH) %>% 
  tidyr::drop_na(ph_s_sf_norm4)

# summary of differences
diff_lueker <- as.numeric(summary(tmp2$ph_s_sf_norm4 - tmp2$pH_l))
diff_sulpis <- as.numeric(summary(tmp2$ph_s_sf_norm4 - tmp2$pH_s))
lueker_sulpis_table <- tibble(var = c("Min", "Q1", "Median", "Mean", "Q3", "Max"), diff_lueker, diff_sulpis)

# make plots
p1 <- ggreg(xdata = tmp2$ph_s_sf_norm4, ydata = tmp2$pH_l, type = "MA", plot = TRUE, 
...
p2 <- ggreg(xdata = tmp2$ph_s_sf_norm4, ydata = tmp2$pH_s, type = "MA", plot = TRUE, 
            title = "Sulpis vs measured", subtitle = "both pH normalized at 4°C",
            xlab = "Spectrophotometric pH measured @ 4°C",
            ylab = "pH @ 4°C calculated (pCO2+AT) with Sulpis") 
p <- p1 + p2
ggsave(filename = "figures/lueker-sulpis.png", plot = p, width = 20, height = 15, units = "cm")
```
The overall means of pCO2, AT (calculated from the AT-salinity relationship), salinity and temperature in the FerryBox were used to calculate pH from measured pCO2 and AT (from a salinity-AT relationship) and from AT and CT. using the formulations of Lueker et al. (2000) and Sulpis et al. (2020) of K1 and K2. The difference induced by the choice of the carbonic acid constants is large for pH, pCO2 and DIC and negligible for OmegaAragonite.

`r knitr::kable(diff_ls_quant, digits = c(0, rep(3, 5)), caption = "Lueker minus Sulpis", format = "simple")`

Here we compare pH measured spectrophotometrically with pH calculated from pCO2 and AT using the sets of K1 and K2 of Lueker et al. (2000) and Sulpis et al. (2020). All pH values were normalized to a temperature of 4°C. Both formulations overestimate spectrophotometric pH 
`r knitr::kable(lueker_sulpis_table, digits = 3, caption = "Difference between spectrophotometric pH and Lueker and Sulpis", format = "simple")`
