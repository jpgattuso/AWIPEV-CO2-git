...
kable(quantile(x = tmp$range_diff, na.rm = TRUE), digits = 2, col.names = "Sal. unit", caption = "Quantiles salinity gradient between 0-2 m and > 8 m")
rm(sal_xts)
```
...
-   Offset between calibrated seaFET data and pH spectro. The offset
    must fit between -0.2/0.2 pH units.

"differences between calibrated sensor pH values and values obtained
from discrete, check samples can be expected to be as small as 0.02 pH
units, and perhaps less." (McLaughlin et al. 2017).
...
From now on, one uses seaFET pH from the internal electrode calibrated
with spec pH and seaFET data when the REMOS was below 8 m
(pHint_tot_sf_10m). This unique pH variable is called pHint_tot_sf. All
other pH variables are cleaned up
