...
ts_air_sea <- air_sea_month %>%
  ...
  lims(y = c(-3, 0)) +
  geom_hline(yintercept = 0)+
  labs(title="",x="Month", y=latex2exp::TeX("Air-sea CO$_2$ flux (mol $m^{-2}$ month$^{-1}$)")) +
...
d_all <- d_all %>%
  dplyr::mutate(
    ...
    alpha = alpha * 1000 , # mol/(m3 atm)
    f_air_sea_corr = k * alpha * delta_pco2, # mol/(m2 h 1e6)
    f_air_sea_corr = k * alpha * delta_pco2 * 1e-6 * 1000 # mmol/(m2 h)
  )
...
air_sea <- dplyr::left_join(air_sea_month, air_sea_corr_month/1000, by = "montht") %>% 
...
`r round(sum(air_sea_month$f_air_sea/1000), digits = 0)` and
`r round(sum(air_sea_corr_month$f_air_sea_corr/1000), digits = 0)` mol/(m2
