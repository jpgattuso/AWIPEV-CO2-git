...
#restore datetime, date which are mangled by the murtates above
d_all$datetime <- d_hour$datetime
d_all$date <- d_hour$date
...
d <- left_join(d, at_ct_mean, by='date') %>% 
  dplyr::select(datetime, date, temp_s_lab, pH_s_seafet, pH_s_durafet, location_flag, at_mean, ct_mean) %>% # removed useless variables and also to do unique next
  dplyr::distinct()
...
d <- left_join(d, d_all %>% dplyr::select(c(closest_datetime, sal_fb, temp_fb, temp_mix, sal_mix, temp_insitu_ctd, pressure_insitu_ctd)), by="closest_datetime") 

# Conversion of spectro pH to insitu mix T°
pH_seafet <- d %>%
  dplyr::filter(!is.na(pH_s_seafet) & !is.na(temp_insitu_ctd))
pH_seafet <- pH_seafet %>%
  dplyr::mutate(ph_s_sf_temp_insi = pHinsi(pH = pH_seafet$pH_s_seafet, ALK = pH_seafet$at_mean_interp*1e-6, Tinsi = pH_seafet$temp_insitu_ctd, Tlab = pH_seafet$temp_s_lab, Pinsi = pH_seafet$pressure_insitu_ctd, S = pH_seafet$sal_mix, Pt = 0, Sit = 0))
                
pH_durafet <- d %>%
#  dplyr::filter(!is.na(pH_s_durafet))
    dplyr::filter(!is.na(pH_s_durafet) & !is.na(sal_fb) & !is.na(temp_fb))
...
#  dplyr::mutate(ph_s_dur_temp_fb = pHinsi(pH=pH_durafet$pH_s_durafet, ALK=pH_durafet$at_mean_interp*1e-6, Tinsi=pH_durafet$temp_fb, Tlab=pH_durafet$temp_s_lab, Pinsi= pH_durafet$depth/10, S=pH_durafet$sal_fb, Pt=0, Sit=0))
  dplyr::mutate(ph_s_dur_temp_fb = pHinsi(pH=pH_durafet$pH_s_durafet, ALK=pH_durafet$at_mean_interp*1e-6, Tinsi=pH_durafet$temp_fb, Tlab=pH_durafet$temp_s_lab, Pinsi= 0, S=pH_durafet$sal_fb, Pt=0, Sit=0))
...
pH_seafet_mean <- pH_seafet%>%
  ...
  dplyr::summarize(ph_s_sf_temp_insi = mean(ph_s_sf_temp_insi, na.rm = T))
...
d_all <- full_join(d_all, discrete2merge, by="datetime")
d_all <- d_all %>% 
  dplyr::mutate(ph_s_sf_temp_insi = ifelse(n_depth_ctd == 4, ph_s_sf_temp_insi, NA)) %>% # eliminate ph spectro data when the profiler was at several depth during the same hour
  dplyr::mutate(ph_s_sf_temp_insi = ifelse(!is.na(temp_insitu_ctd_7m) | !is.na(temp_insitu_ctd_9m), 
                                          ph_s_sf_temp_insi, NA)) # only use ph spectro data when profiler was below 6 m
...
# function to express the number of quality flag in pefrcent for each variable
# qflag <- function(x, ...) {
#   tmp <- summary(as.factor(x))*100/length(x)
#   return(tmp)
# }
d <- readRDS(file = paste0(path, "fb_awipev-co2_server/ny-alesund/data/NRT_data/all_nydata_minute.rds")) %>%
dplyr::select(datetime, c(pco2_corr_qf, sal_fb_qf, sal_insitu_ctd_qf, 
                          temp_fb_qf, temp_insitu_11m_qf, ph_dur_qf, phINT_qf))

s1 <- summary(as.factor(d$pco2_corr_qf))*100/length(d$pco2_corr_qf)
s2 <- summary(as.factor(d$sal_fb_qf))*100/length(d$sal_fb_qf)
s3 <- summary(as.factor(d$sal_insitu_ctd_qf))*100/length(d$sal_insitu_ctd_qf)
s4 <- summary(as.factor(d$temp_fb_qf))*100/length(d$temp_fb_qf)
s5 <- summary(as.factor(d$temp_insitu_11m_qf))*100/length(d$temp_insitu_11m_qf)
# durafet and seafet started later
x <- filter(d, datetime >= as.POSIXct("2017-08-29 10:00:00"))
#s6 <- summary(as.factor(x$temp_dur_qf))*100/length(x$temp_dur_qf)
s7 <- summary(as.factor(x$ph_dur_qf))*100/length(x$ph_dur_qf)
x <- filter(d, datetime >= as.POSIXct("2017-08-24 19:00:00"))
...
labels <- c("Good", "Not usable", "Outside regional range", "Outliers (despike)", 
            "Failed visual inspection", "No data")
#colnames(z) <- c("flag", colnames(d)[-1]) # remove first col name (datetime)
...
p <-  ggplot(data = zz, aes(x = name, y = value, fill = Flag)) +
  ...
  #scale_y_reverse() +
  #discrete_scale(labels = labels) + 
  coord_flip() +
  geom_text(aes(label=round(value, digits = 0)), position = position_stack(0.5), size = 3) +
...
fit <- lmodel2(data = d_all,  sal_fb ~ sal_insitu_ctd_9m , nperm = 99)
...
fit <- lmodel2(data = d_all,  temp_insitu_11m ~ temp_insitu_ctd_9m , nperm = 99)

```{r Temp 11m and ctd comparaison, echo=FALSE, message= FALSE, warning=FALSE}

fit <- lmodel2(data = d_all,  temp_insitu_11m ~ temp_insitu_ctd_9m , nperm = 99)

p<-  ggplot(d_all,aes(x=temp_insitu_ctd_9m, y= temp_insitu_11m, label = month))+
  geom_text( aes(color=factor(month))) + 
 # scale_color_viridis_d(option = "C", guide="none")+
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  geom_abline(aes(intercept = fit$regression.results[2,2], slope = fit$regression.results[2,3]),colour = "blue") +
  ...
  theme(aspect.ratio=1,plot.title = element_text(size=7)) + 
  coord_fixed(ratio = 1 )+ scale_colour_discrete(guide="none") +
  labs( x="T CTD 9 m", y="T 11 m ", color = "Months")  +theme_bw()
print(p)
...
p <-  ggplot(d_all,aes(x=pco2_calc, y= pco2_corr, label = month))+
   geom_text(aes(color = factor(month)))+
  scale_color_discrete(guide="none")+
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  geom_abline(aes(intercept = fit$regression.results[2,2], slope = fit$regression.results[2,3]),colour = "blue") +
...
print(p) # pco2_calc_pco2_corr_text
```
...
pco2_contros_xts <-
  dplyr::select(d_all, datetime, pco2_calc, pco2_corr)
pco2_contros_xts <-
  ...
  dplyr::mutate(
    pco2_corr = case_when(
      date > as.Date("2016-04-01") &
        date <  as.Date("2017-02-05") & pco2_corr > 400 ~ NA_real_,
      #date == as.Date("2015-11-19") & sal_mix < 33 ~ NA_real_,
      date > as.Date("2017-08-01") &
        date <  as.Date("2017-09-30") & pco2_corr > 300 ~ NA_real_,
...
#periode 1, mean of 4 next hours for volt int + T seaF
...
# Periode 2, mean of 4 next hours for volt int + T seaF
# mean_voltINT_4h <- d_all %>%
#   dplyr::filter(
...
# period 03, mean of 4 previous hours for volt int + volt ext
# mean_voltINT_4h <- d_all %>%
#   dplyr::filter(
...
# Ce qui suit (7 lignes) est très bizarre
d_all <- d_all %>%
  dplyr::mutate(voltINT = ifelse(
    datetime == as.POSIXct("2017-09-28 13:00:00"),
    mean_voltINT_4h,
...
# period 04 also, mean of 4 previous hours for volt int + volt ext + T seaF
# mean_voltINT_4h <- d_all %>%
#   dplyr::filter(
...
p <- ggplot(d_all, aes(x=datetime, y= pHint_tot_sf))+
  geom_point(size=1, alpha=0.5, na.rm=TRUE)

seafet_depth_xts <- dplyr::select(d_all, datetime, ph_s_sf_temp_insi, pHint_tot_sf, pH_s_sf_no_calb)
...
dySeries("ph_s_sf_temp_insi", label = "pH spectro for calibration", color =  "red", strokeWidth = 0, pointSize = 4 ) %>%
...
pH_sf$pHint_tot_sf_norm4 <- pHinsi(pH=pH_sf$pHint_tot_sf, ALK=2.273e-3, Tinsi=4, Tlab=pH_sf$temp_insitu_ctd_9m, Pinsi=11/10, S=pH_sf$sal_insitu_ctd_9m, Pt=0, Sit=0, k1k2 = "x", kf = "x", ks="d", pHscale = "T", b="u74")
...
pdur_sf_norm_text <-  ggplot(d_all,aes(x=pH_durafet_norm4, y= pHint_tot_sf_norm4, label = month))+
  geom_point( aes(color=factor(month)), size =1, alpha=0.5) + 
...
# Spectro / seafet
# fit <- lmodel2(data = d_all,  ph_s_sf_temp_insi ~ pHint_tot_sf , nperm = 99)
#
# p_sf_spectro <-  ggplot(d_all,aes(x=pHint_tot_sf, y= ph_s_sf_temp_insi, label = month))+
#   geom_text( aes(color=factor(month))) +
#   scale_color_discrete(guide = "none")+
#  geom_abline(slope=1, intercept =0, linetype = "dashed") +
#   geom_abline(aes(intercept = fit$regression.results[2,2], slope = fit$regression.results[2,3]),colour = "blue") +
...
## Figures for ESSD paper

```{r figures for ESSD paper, echo=FALSE, message= FALSE, warning=FALSE}
Mytheme <- function(size_labs = 6, face_font="plain", ...) {
  theme_bw() +
  theme(axis.text.x = element_text(face=face_font, size=size_labs, color="black"),
        axis.title.x = element_text(face="bold", size=size_labs, margin=margin(0,0,0,0,"pt")),
        axis.text.y = element_text(face=face_font, color="black", size=size_labs),
        ...
        aspect.ratio = 1 / 3,
        plot.margin = margin(t = 0, r = 1.5, b = 0, l = 1.5, unit = "lines")
  )
}
# select time series data
d <- as_tibble(d_all) %>%
  dplyr::select(datetime, temp_insitu_11m, sal_mix, pco2_corr, pHint_tot_sf, ph_dur_corr, at_calc)
# time series temp
ts_temp <- d %>%
  dplyr::filter(!is.na(temp_insitu_11m)) %>% 
ggplot() +
  geom_point(aes(x = datetime, y = temp_insitu_11m), col = "blue", size = 0.15) +
  labs(title = "", x = "Time", y = "In situ temperature (°C)") +
  Mytheme(size_labs = 8)
ggsave(filename = "figures/ts_temp.png")
print(ts_temp) 
# time series sal
ts_sal <- d %>%
...
ggplot() +
  ...
  labs(title = "", x = "Time", y = "Salinity") +
  Mytheme(size_labs = 8)
ggsave(filename = "figures/ts_sal.png")
print(ts_sal) 
# time series pco2
ts_pco2 <- d %>%
...
ggplot() +
  ...
  labs(title = "", x = "Time", y = expression(paste("C", O[2], " partial pressure (", mu, "atm)"))) +
  Mytheme(size_labs = 8)
ggsave(filename = "figures/ts_pco2.png")
print(ts_pco2) 
# time series pH
ts_ph <- d %>%
  ...
  labs(title = "", x = "Time", y = expression(paste("p", H[T]))) +
  scale_color_manual(values=c("blue", "red"), 
                       name="",
...
ggsave(filename = "figures/ts_ph.png")
