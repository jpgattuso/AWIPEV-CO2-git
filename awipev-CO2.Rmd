...
d <- left_join(d, d_all%>%dplyr::select(c(closest_datetime, sal_fb, temp_fb, temp_mix, temp_insitu_11m, sal_mix )), by="closest_datetime") 

# Conversion of spectro pH to insitu mix T°
pH_seafet <- d %>%
  dplyr::filter(!is.na(pH_s_seafet) & !is.na(temp_mix) )
pH_seafet <- pH_seafet %>%
...
dygraph(temp_insitu_xts, group = "awipev", main="Temperature (in situ 11 m)", ylab="Temperature") %>%
      ...
      dyRangeSelector(height = 30, dateWindow= NULL)
# TEMP insitu CTD
temp_insitu_xts <- dplyr::select(d_all, datetime, temp_insitu_ctd_1m, temp_insitu_ctd_3m, temp_insitu_ctd_5m, temp_insitu_ctd_7m, temp_insitu_ctd_9m)
temp_insitu_xts <- as.xts(temp_insitu_xts, order.by = temp_insitu_xts$datetime)
...
p_sal_fb_ctd<-  ggplot(d_all,aes(x=sal_insitu_ctd_9m, y= sal_fb))+
...
calib_spectro <- sf_calib(calEint = d_all$voltINT, calEext = d_all$voltEXT, calpH = d_all$ph_s_sf_temp_mix,
                          calT = d_all$temp_sf, calSal = d_all$sal_insitu_ctd)
...
####
seafet_calib <- sf_calc(calEint = d_all$voltINT, calEext = d_all$voltEXT, E0int25 = d_all$E0int25_sf,  E0ext25 = d_all$E0ext25_sf, calT = d_all$temp_sf, calSal = d_all$sal_insitu_ctd)
