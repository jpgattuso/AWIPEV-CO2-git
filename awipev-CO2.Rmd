...
d_hour <- readRDS(file = paste0(path, "fb_awipev-co2_server/ny-alesund/data/NRT_data/all_nydata_hour_copy.rds"))
# load = d_hour
d_hour <- d_hour %>%
  dplyr::select(-date) %>%
  ...
  dplyr::rename(pco2_corr = pco2_corr_filtered, temp_dur = temp_dur_filtered, pco2_raw = pco2_raw_filtered, sal_fb = sal_fb_filtered, temp_insitu_11m=temp_insitu_11m_filtered, temp_fb = temp_fb_filtered, temp_sf = T_seaF, ph_dur = ph_dur_filtered)
...
d_all <- d_all%>%
  dplyr::mutate(depth = ifelse(pressure_insitu_ctd <=2, "1 m", 
                        ifelse(pressure_insitu_ctd >2 & pressure_insitu_ctd <=4, "3 m", 
                        ifelse(pressure_insitu_ctd >4 & pressure_insitu_ctd <=6, "5 m",
                        ifelse(pressure_insitu_ctd >6 & pressure_insitu_ctd <=8, "7 m",
                        ifelse(pressure_insitu_ctd >8, "9 m",NA))))))
```
...
pH_seafet <- pH_seafet %>%
  dplyr::mutate(pH_s_seafet_temp_mix = pHinsi(pH=pH_seafet$pH_s_seafet,ALK=pH_seafet$at_mean_interp*1e-6, Tinsi=pH_seafet$temp_mix, Tlab=pH_seafet$temp_s_lab, Pinsi= pH_seafet$depth/10, S=pH_seafet$sal_mix,Pt=0,Sit=0))
...
pH_seafet_mean <- pH_seafet%>%
  dplyr::select(datetime,pH_s_seafet_temp_mix)%>% 
  dplyr::group_by(datetime) %>% 
  dplyr::summarize(pH_s_seafet_temp_mix = mean(pH_s_seafet_temp_mix, na.rm = T))
...
discrete2merge <- discrete %>%
  #drop_na() %>%
  dplyr::select(closest_datetime, pH_s_seafet_temp_mix) %>%
  dplyr::filter(!is.na(closest_datetime)) %>%
  rename(datetime=closest_datetime)
d_all <- full_join(d_all, discrete2merge, by="datetime")
# #voltINT
...
#SAL CTD profile
sal_xts <- dplyr::select(d_all, datetime, sal_insitu_ctd, sal_fb)
sal_xts <- as.xts(sal_xts, order.by = sal_xts$datetime)
dygraph(sal_xts, group = "awipev", main="Salinities (in situ CTD, all depths + FB)", ylab="Salinity") %>%
      dySeries("sal_insitu_ctd", color = "blue", strokeWidth = 0, label = "CTD") %>%
            dySeries("sal_fb", color = "black", strokeWidth = 1, label = "FB") %>%
...
dygraph(temp_insitu_xts, group = "awipev", main="Temperature (ctd all depths)", ylab="Temperature") %>%
      dySeries("temp_insitu_ctd", color = "blue", strokeWidth = 0, label = "T") %>%
      dyAxis("y", valueRange = c(-2.8, 10)) %>%
      dyHighlight(highlightCircleSize = 8,highlightSeriesBackgroundAlpha = 0.5,hideOnMouseOut = TRUE) %>%
      dyOptions(drawGrid = TRUE, drawPoints = TRUE, pointSize = 2,useDataTimezone = TRUE) %>%
      dyLegend(show = "follow")   %>%
...
pco2_contros_xts <- dplyr::select(d_all, datetime, pco2calc, pco2_corr)
...
dygraph(pco2_contros_xts, group = "awipev", main="pCO2", ylab="pCO2") %>%
    ...
    dyRangeSelector(height = 30, dateWindow= NULL)
# DURAFET 
durafet_contros_xts <- dplyr::select(d_all, datetime, ph_dur, pH_s_durafet_temp_fb)
...
dygraph(durafet_contros_xts, group = "awipev", main="pH durafet (in Ferrybox)", ylab="pHT") %>%
    ...
    dyRangeSelector(height = 30, dateWindow= NULL) 
# SEAFET 
seafet_contros_xts <- dplyr::select(d_all, datetime, phINT,pH_s_seafet_temp_mix)
...
dygraph(seafet_contros_xts, group = "awipev", main="pH seaFET (in situ T CTD)", ylab="pHT") %>%
  ...
  dySeries("pH_s_seafet_temp_mix", label = "pH spectro", color =  "red", strokeWidth = 0, pointSize = 4 ) %>%
...
p_sal_fb_ctd<-  ggplot(d_all%>%dplyr::filter(pressure_insitu_ctd >=8),aes(x=sal_insitu_ctd, y= sal_fb, label = month))+
...
dygraph(at_contros_calc_xts, group = "awipev", main="TA + TA calc from sal_mix + TA reference", ylab="Total alkalinity") %>%
  #dySeries("AT", label= "Raw TA", color ="blue", strokeWidth = 0, pointSize=2) %>%
  dySeries("at_calc", label="TA calc ",color = "blue", strokeWidth = 0, pointSize=2) %>%
  dySeries("at", label="TA ref",color = "red", strokeWidth = 0, pointSize=4) %>%
  dyEvent(as.POSIXct("2016-05-31 10:00:00", tz="GMT"), "HCl Changed", labelLoc = "bottom") %>%
  dyEvent(as.POSIXct("2016-06-24 10:00:00", tz="GMT"), "BCG Changed", labelLoc = "bottom") %>%
  dyEvent(as.POSIXct("2016-02-24 14:00:00", tz="GMT"), "Calibration 1", labelLoc = "bottom") %>%
  dyEvent(as.POSIXct("2016-09-16 10:00:00", tz="GMT"), "Calibration 2", labelLoc = "bottom") %>%
...
fit <- lmodel2(data = d_all,  pco2_corr ~ pco2calc , nperm = 99)

pco2calc_pCO2_contros_text <-  ggplot(d_all,aes(x=pco2calc, y= pco2_corr, label = month))+
   geom_text(aes(color = factor(month)))+
  scale_color_discrete(guide="none")+
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  geom_abline(aes(intercept = fit$regression.results[2,2], slope = fit$regression.results[2,3]),colour = "blue") +
...
# sfPeriod <- d_all%>%
#   dplyr::select(datetime, pH_s_seafet_temp_mix)%>%
#   dplyr::filter(!is.na(pH_s_seafet_temp_mix))
...
####
calib_spectro <- sf_calib(calEint = d_all$voltINT, calEext = d_all$voltEXT, calpH = d_all$pH_s_seafet_temp_mix,
...
d_all <- d_all%>%
  dplyr::mutate( pH_s_sf_no_calb = ifelse(datetime == "2018-04-16 07:00:00" | datetime == "2018-12-14 09:00:00" | datetime == "2019-01-18 09:00:00", pH_s_seafet_temp_mix, NA ))

seafet_contros_xts <- dplyr::select(d_all, datetime, phINT, pH_s_seafet_temp_mix, pHint_tot_sf, pH_s_sf_no_calb, pressure_insitu_ctd, sal_insitu_ctd, temp_insitu_ctd)
...
dySeries("sal_insitu_ctd", axis="y",label = "S ctd", color ="orange", strokeWidth=0, pointSize=2) %>%
dySeries("pressure_insitu_ctd", axis="y2",label = "Pression", color ="pink", strokeWidth=0, pointSize=2) %>%
dySeries("pHint_tot_sf", label = "pHT corrected", color =  RColorBrewer::brewer.pal(3, "Dark2")[1], strokeWidth = 0, pointSize = 2) %>%
dySeries("phINT", label = "Raw data INT", color ="blue", strokeWidth=0, pointSize=2) %>%
dySeries("pH_s_seafet_temp_mix", label = "pH spectro for calibration", color =  "red", strokeWidth = 0, pointSize = 4 ) %>%
...
seafet_depth_xts <- dplyr::select(d_all, datetime, phINT, pH_s_seafet_temp_mix,pHint_tot_sf)
...
dygraph(seafet_depth_xts, group = "depth", main="pH seaFET RAW (in situ)", ylab="pHT") %>%
  dySeries("pH_s_seafet_temp_mix", axis="y2", label = "pH spectro for calibration", color =  "red", strokeWidth = 0, pointSize = 4 ) %>%
  dySeries("phINT", axis="y2",label = "pH RAW", color ="blue", strokeWidth=0, pointSize=2,  stepPlot = TRUE) %>%
...
# # #@@@@@ test plot #@@@@@ #@@@@@ #@@@@@
# seafet_contros_xts <- dplyr::select(d_all, datetime, temp_fb, temp_insitu)#voltINT, phINT,pH_s_seafet_temp_mix, temp_sf, temp_fb, temp_insitu, sal)
# #seafet_contros_xts <- dplyr::select(d_all, datetime,voltINT)
# seafet_contros_xts <- as.xts(seafet_contros_xts, order.by = d_all$datetime)
# dygraph(seafet_contros_xts, group = "awipev", main="pH seaFET (in situ)", ylab="pHT") %>%
#   #dySeries("voltINT", label = "voltINTINT", color ="blue", strokeWidth=0, pointSize=2) %>%
#    # dySeries("pH_s_seafet_temp_mix", label = "pH spectro", axis= "y2", color =  "green", strokeWidth = 0, pointSize = 4 ) %>%
...
# dat <- mutate(sfoffset, diff_pH_sf = round((pH_s_seafet_temp_mix - pHint_tot_sf),digits  = 4))
...
#   geom_point(data=d_all%>%dplyr::filter(datetime > "2017-07-30"  ), aes(x=datetime, y=pH_s_seafet_temp_mix, fill="Discrete measurements"), col="red" )+
...
duroffset <- left_join(durPeriod%>%dplyr::select(datetime,pH_s_durafet_temp_fb), d_all%>%dplyr::select(datetime,ph_dur ), by="datetime")
dat_dur <- mutate(duroffset, diff_pH_dur = pH_s_durafet_temp_fb - ph_dur)
...
durafet_contros_xts <- dplyr::select(d_all, datetime, ph_dur, pH_s_durafet_temp_fb,ph_dur_corr, pH_s_dur_no_calb)
durafet_contros_xts <- as.xts(durafet_contros_xts, order.by = d_all$datetime)
 dygraph(durafet_contros_xts, group = "awipev", main="pH durafet (in Ferrybox)", ylab="pHT") %>%
    dySeries("ph_dur", label = "Raw durafet pH", color ="blue", strokeWidth=0, pointSize=2) %>%
...
# Spectro / durafet
fit <- lmodel2(data = d_all,  pH_s_durafet_temp_fb ~ ph_dur_corr , nperm = 99)

p_dura_spectro <-  ggplot(d_all,aes(x=ph_dur_corr, y= pH_s_durafet_temp_fb, label = month))+
  geom_text( aes(color=factor(month))) + 
  scale_color_discrete(guide = "none")+
  geom_abline(slope=1, intercept =0, linetype = "dashed") +
  geom_abline(aes(intercept = fit$regression.results[2,2], slope = fit$regression.results[2,3]),colour = "blue") +
...
# Spectro / seafet
fit <- lmodel2(data = d_all,  pH_s_seafet_temp_mix ~ pHint_tot_sf , nperm = 99)

p_sf_spectro <-  ggplot(d_all,aes(x=pHint_tot_sf, y= pH_s_seafet_temp_mix, label = month))+
  geom_text( aes(color=factor(month))) + 
  scale_color_discrete(guide = "none")+
 geom_abline(slope=1, intercept =0, linetype = "dashed") +
  geom_abline(aes(intercept = fit$regression.results[2,2], slope = fit$regression.results[2,3]),colour = "blue") +
