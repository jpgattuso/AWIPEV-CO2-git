...
d_hour <- d_hour %>%
  ...
  dplyr::rename(at_contros=AT,pco2_contros=PCO2_corr_contros_filtered,pH_durafet=HW_pH1_filtered, temp_durafet = HW_Temperature1_filtered, temp_insitu_11m=temp_insitu_11m_filtered, sal_fb = sal_fb_filtered, sal_insitu_ctd = sal_insitu_ctd_filtered, temp_fb = temp_fb_filtered, temp_insitu_ctd = temp_insitu_ctd_filtered, temp_sf=T_seaF)
...
# add sal_fb + sal_insitu_ctd + temp_fb + temp_insitu_ctd from d_hour TO d, to use pHinsi
...
d <- left_join(d, d_all%>%dplyr::select(c(closest_datetime, sal_fb, temp_fb, sal_insitu_ctd, temp_insitu_ctd)), by="closest_datetime") 

# Conversion of spectro pH to a same T°
pH_seafet <- d %>%
  dplyr::filter(!is.na(pH_s_seafet)& !is.na(sal_insitu_ctd) )
pH_seafet <- pH_seafet %>%
  dplyr::mutate(pH_s_seafet_temp_ctd = pHinsi(pH=pH_seafet$pH_s_seafet,ALK=pH_seafet$at_mean_interp*1e-6, Tinsi=pH_seafet$temp_insitu_ctd, Tlab=pH_seafet$temp_s_lab, Pinsi= pH_seafet$depth/10, S=pH_seafet$sal_insitu_ctd,Pt=0,Sit=0))
                
pH_durafet <- d %>%
  dplyr::filter(!is.na(pH_s_durafet))
pH_durafet <- pH_durafet %>%
  dplyr::mutate(pH_s_durafet_temp_fb = pHinsi(pH=pH_durafet$pH_s_durafet, ALK=pH_durafet$at_mean_interp*1e-6, Tinsi=pH_durafet$temp_fb, Tlab=pH_durafet$temp_s_lab, Pinsi= pH_durafet$depth/10, S=pH_durafet$sal_fb,Pt=0,Sit=0))
 
# create separate seafet/durafet df to join to initial "d" df
pH_seafet_mean <- pH_seafet%>%
  dplyr::select(datetime,pH_s_seafet_temp_ctd)%>% 
  dplyr::group_by(datetime) %>% 
  dplyr::summarize(pH_s_seafet_temp_ctd = mean(pH_s_seafet_temp_ctd, na.rm = T))
pH_durafet_mean <- pH_durafet%>%
  dplyr::select(datetime,pH_s_durafet_temp_fb)%>% 
  dplyr::group_by(datetime) %>% 
  dplyr::summarize(pH_s_durafet_temp_fb = mean(pH_s_durafet_temp_fb, na.rm = T))
...
discrete2merge <- discrete %>%
  #drop_na() %>%
  dplyr::select(closest_datetime, pH_s_seafet_temp_ctd) %>%
  dplyr::filter(!is.na(closest_datetime)) %>%
  rename(datetime=closest_datetime)
d_all <- full_join(d_all, discrete2merge, by="datetime")
# #voltINT
...
discrete2merge <- discrete %>%
  #drop_na() %>%
  dplyr::select(closest_datetime, pH_s_durafet_temp_fb) %>%
  dplyr::filter(!is.na(closest_datetime)) %>%
  rename(datetime=closest_datetime)
d_all <- full_join(d_all, discrete2merge, by="datetime")
...
dygraph(sal_xts, group = "awipev", main="Salinity (in situ CTD)", ylab="Salinity") %>%
      dySeries("sal_insitu_ctd", color = "blue", strokeWidth = 0, label = "S") %>%
...
dygraph(pco2_contros_xts, group = "awipev", main="pCO2", ylab="pCO2") %>%
    ...
    dyRangeSelector(height = 30, dateWindow= NULL)
# DURAFET 
durafet_contros_xts <- dplyr::select(d_all, datetime, pH_durafet, pH_s_durafet_temp_fb)
...
dygraph(durafet_contros_xts, group = "awipev", main="pH durafet (in Ferrybox)", ylab="pHT") %>%
    ...
    dySeries("pH_s_durafet_temp_fb", label = "pH spectro", color =  "red", strokeWidth = 0, pointSize = 4 ) %>%
    ...
    dyRangeSelector(height = 30, dateWindow= NULL) 
# SEAFET 
seafet_contros_xts <- dplyr::select(d_all, datetime, phINT_filtered,pH_s_seafet_temp_ctd)
...
dygraph(seafet_contros_xts, group = "awipev", main="pH seaFET (in situ T CTD)", ylab="pHT") %>%
  ...
  dySeries("pH_s_seafet_temp_ctd", label = "pH spectro", color =  "red", strokeWidth = 0, pointSize = 4 ) %>%
...
# date format into charactere
d_all$datetime_chr <- as.character(d_all$datetime)

p_sal_fb_ctd<-  ggplot(d_all%>%dplyr::filter(pressure_insitu_ctd_filtered >=8),aes(x=sal_insitu_ctd, y= sal_fb, label = month))+
  geom_point( aes(color=factor(datetime_chr))) + 
...
p_temp_11_fb<-  ggplot(d_all,aes(x=temp_insitu_11m, y= temp_fb, label = month))+
  geom_text( aes(color=factor(datetime_chr))) + 
 # scale_color_viridis_d(option = "C", guide="none")+
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  geom_abline(aes(intercept = fit$regression.results[2,2], slope = fit$regression.results[2,3]),colour = "blue") +
...
# sfPeriod <- d_all%>%
#   dplyr::select(datetime, pH_s_seafet_temp_ctd)%>%
#   dplyr::filter(!is.na(pH_s_seafet_temp_ctd))
...
####
calib_spectro <- sf_calib(calEint = d_all$voltINT, calEext = d_all$voltEXT, calpH = d_all$pH_s_seafet_temp_ctd,
...
d_all <- d_all%>%
  dplyr::mutate( pH_s_sf_no_calb = ifelse(datetime == "2018-04-16 07:00:00" | datetime == "2018-12-14 09:00:00" | datetime == "2019-01-18 09:00:00", pH_s_seafet_temp_ctd, NA ))

seafet_contros_xts <- dplyr::select(d_all, datetime, phINT_filtered, pH_s_seafet_temp_ctd, pHint_tot_sf, pH_s_sf_no_calb, pressure_insitu_ctd_filtered, sal_insitu_ctd, temp_insitu_ctd)
...
dySeries("temp_insitu_ctd", axis="y",label = "T ctd", color ="grey", strokeWidth=0, pointSize=2) %>%
dySeries("sal_insitu_ctd", axis="y",label = "sS ctd", color ="orange", strokeWidth=0, pointSize=2) %>%
dySeries("pressure_insitu_ctd_filtered", axis="y2",label = "Pression", color ="pink", strokeWidth=0, pointSize=2) %>%
dySeries("pHint_tot_sf", label = "pHT corrected", color =  RColorBrewer::brewer.pal(3, "Dark2")[1], strokeWidth = 0, pointSize = 2) %>%
dySeries("phINT_filtered", label = "Raw data INT", color ="blue", strokeWidth=0, pointSize=2) %>%
dySeries("pH_s_seafet_temp_ctd", label = "pH spectro for calibration", color =  "red", strokeWidth = 0, pointSize = 4 ) %>%
...
# p <- ggplot(d_all%>%filter(datetime >= ("2017-07-30") & !is.na(depth) ),aes(x=datetime, y= phINT_filtered, label = depth))+
...
seafet_depth_xts <- dplyr::select(d_all, datetime, phINT_filtered, pH_s_seafet_temp_ctd,pHint_tot_sf)
...
dygraph(seafet_depth_xts, group = "depth", main="pH seaFET RAW (in situ)", ylab="pHT") %>%
  dySeries("pH_s_seafet_temp_ctd", axis="y2", label = "pH spectro for calibration", color =  "red", strokeWidth = 0, pointSize = 4 ) %>%
  dySeries("phINT_filtered", axis="y2",label = "pH RAW", color ="blue", strokeWidth=0, pointSize=2,  stepPlot = TRUE) %>%
  dySeries("pHint_tot_sf", axis="y2", label = "pHT corrected", color =  RColorBrewer::brewer.pal(3, "Dark2")[1], strokeWidth = 0, pointSize = 2) %>%
  dyEvent(as.POSIXct("2017-08-24 12:00:00", tz="GMT"), "seaFET #1005 installed", labelLoc = "bottom", color="red") %>%
  dyEvent(as.POSIXct("2018-02-02 09:00:00", tz="GMT"), "seaFET settings changed to every 1H", labelLoc = "bottom", color="red") %>% 
  dyEvent(as.POSIXct("2018-04-17 12:00:00", tz="GMT"), "seaFET #0007 installed + continuous mode set", labelLoc = "bottom", color="red") %>%
    dyEvent(as.POSIXct("2019-09-02 12:00:00", tz="GMT"), "seaFET #0007 out of water for cleaning", labelLoc = "bottom", color="red") %>%
...
# # #@@@@@ test plot #@@@@@ #@@@@@ #@@@@@
# seafet_contros_xts <- dplyr::select(d_all, datetime, temp_fb, temp_insitu)#voltINT, phINT_filtered,pH_s_seafet_temp_ctd, temp_sf, temp_fb, temp_insitu, sal)
# #seafet_contros_xts <- dplyr::select(d_all, datetime,voltINT)
# seafet_contros_xts <- as.xts(seafet_contros_xts, order.by = d_all$datetime)
# dygraph(seafet_contros_xts, group = "awipev", main="pH seaFET (in situ)", ylab="pHT") %>%
#   #dySeries("voltINT", label = "voltINTINT", color ="blue", strokeWidth=0, pointSize=2) %>%
#    # dySeries("pH_s_seafet_temp_ctd", label = "pH spectro", axis= "y2", color =  "green", strokeWidth = 0, pointSize = 4 ) %>%
...
# dat <- mutate(sfoffset, diff_pH_sf = round((pH_s_seafet_temp_ctd - pHint_tot_sf),digits  = 4))
...
#   geom_point(data=d_all%>%dplyr::filter(datetime > "2017-07-30"  ), aes(x=datetime, y=pH_s_seafet_temp_ctd, fill="Discrete measurements"), col="red" )+
...
durPeriod <- d_all%>%
  dplyr::select(datetime, pH_s_durafet_temp_fb)%>%
  dplyr::filter(!is.na(pH_s_durafet_temp_fb))
...
duroffset <- left_join(durPeriod%>%dplyr::select(datetime,pH_s_durafet_temp_fb), d_all%>%dplyr::select(datetime,pH_durafet ), by="datetime")
dat_dur <- mutate(duroffset, diff_pH_dur = pH_s_durafet_temp_fb - pH_durafet)
...
# add spectro points that are not taking into consideration
d_all <- d_all%>%
  dplyr::mutate( pH_s_dur_no_calb = ifelse(datetime == "2017-08-24 13:00:00", pH_s_durafet_temp_fb, NA ))

durafet_contros_xts <- dplyr::select(d_all, datetime, pH_durafet, pH_s_durafet_temp_fb,pH_durafet_corrected, pH_s_dur_no_calb)
durafet_contros_xts <- as.xts(durafet_contros_xts, order.by = d_all$datetime)
 dygraph(durafet_contros_xts, group = "awipev", main="pH durafet (in Ferrybox)", ylab="pHT") %>%
    dySeries("pH_durafet", label = "Raw durafet pH", color ="blue", strokeWidth=0, pointSize=2) %>%
    dySeries("pH_s_durafet_temp_fb", label = "pH spectro for calibration", color =  "red", strokeWidth = 0, pointSize = 6 ) %>%
...
#   geom_point(data=d_all%>%dplyr::filter(datetime > "2017-07-30"  ), aes(x=datetime, y=pH_s_durafet_temp_fb, fill="Discrete measurements"), col="red" )+
...
pH_dur <- d_all %>%
  dplyr::select(datetime, sal_fb,temp_fb, pH_durafet_corrected)%>%
  dplyr::filter(!is.na(pH_durafet_corrected), !is.na(temp_fb), !is.na(sal_fb))
pH_dur$pH_durafet_norm4 <- pHinsi(pH=pH_dur$pH_durafet_corrected, ALK=2.273e-3, Tinsi=4, Tlab=pH_dur$temp_fb, Pinsi=11/10, S=pH_dur$sal_fb, Pt=0, Sit=0, k1k2 = "x", kf = "x", ks="d", pHscale = "T", b="u74")
...
# Spectro / durafet
fit <- lmodel2(data = d_all,  pH_s_durafet_temp_fb ~ pH_durafet_corrected , nperm = 99)

p_dura_spectro <-  ggplot(d_all,aes(x=pH_durafet_corrected, y= pH_s_durafet_temp_fb, label = month))+
  geom_text( aes(color=factor(month))) + 
  scale_color_discrete(guide = "none")+
  geom_abline(slope=1, intercept =0, linetype = "dashed") +
  geom_abline(aes(intercept = fit$regression.results[2,2], slope = fit$regression.results[2,3]),colour = "blue") +
...
# Spectro / seafet
fit <- lmodel2(data = d_all,  pH_s_seafet_temp_ctd ~ pHint_tot_sf , nperm = 99)

p_sf_spectro <-  ggplot(d_all,aes(x=pHint_tot_sf, y= pH_s_seafet_temp_ctd, label = month))+
  geom_text( aes(color=factor(month))) + 
  scale_color_discrete(guide = "none")+
 geom_abline(slope=1, intercept =0, linetype = "dashed") +
  geom_abline(aes(intercept = fit$regression.results[2,2], slope = fit$regression.results[2,3]),colour = "blue") +
