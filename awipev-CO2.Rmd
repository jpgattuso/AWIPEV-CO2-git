...
if (Sys.getenv("R_PLATFORM") == "aarch64-apple-darwin20") {
  path = "../../kDrive/Documents/experiments/exp168_awipev-CO2/"
  tables_path <- "/Users/gattuso/kDrive/Documents/publications/publications_submitted/2023_gattuso_etal_essd/revised1/tables/"}
#iMac Home
if (Sys.getenv("R_PLATFORM") == "x86_64-apple-darwin20") {
  path = "../../kDrive2/Documents/experiments/exp168_awipev-CO2/"
  tables_path <- "/Users/gattuso/kDrive2/Documents/publications/publications_submitted/2023_gattuso_etal_essd/revised1/tables/"}
# Samir
...
## Stratification {.tabset .tabset-pills}

### REMOS

```{r, echo=FALSE, message= FALSE, warning=FALSE}
# we calculate the duration of each stop 

# Function written by Robert Schlegel
depth_time <- function(df){
  # Get proto event info
  ex1 <- rle(as.numeric(df$depth_group))
  ind1 <- rep(seq_along(ex1$lengths), ex1$lengths)
  s1 <- base::split(base::seq_len(nrow(df)), ind1)
  # Extract the start and end times per depth group
  depth_count <- do.call(rbind, lapply(s1, function(x)
    data.frame(depth_group = tmp$depth_group[min(x)],
               time_start = tmp$datetime[min(x)], 
               time_end = tmp$datetime[max(x)])))
  # Calculate the time spent there in seconds
  depth_count$time_spent <- depth_count$time_end-depth_count$time_start+3600
  # Convert to hours if desired
  depth_count$hours_spent <- as.numeric(depth_count$time_spent)/3600
  # Exit
  return(depth_count)
}

# First create the tibble
depth_labels <- c("0-2", "2-4", "4-6", "6-8", ">8")
tmp <- d_all %>% 
  dplyr::select(datetime, depth_group) %>%
#  dplyr::filter(depth_group != "NA") %>% 
  dplyr::mutate(depth_group = factor(depth_group, levels = depth_labels))

count_depth <- depth_time(tmp) %>% 
  dplyr::filter(depth_group != "NA")
p <- ggplot(count_depth) +
  geom_bar(aes(x=log10(hours_spent)))
ggsave(filename = "figures/REMOS.png", plot = p, width = 15, height = 10, units = "cm")
summary(count_depth$hours_spent)
```
The REMOS spent a highly variable period of time in each depth group.
![Distribution of the number of hours spent by the REMOS at depth](figures/REMOS.png)
...
ggsave(filename = "figures/essd/deltas_pco2.pdf", plot = g, width = 15, height = 20, units = "cm")
```
...
d_all <- d_all %>%
  dplyr::mutate(
    ...
    k600h = 0.266 * ws ^ 2, # cm/h Ho et al. (2006)
    k600d = 0.143 * ws ^ 2, # cm/h Dobashi and Ho (2023)
    Sc = 2073.1 - (125.62 * temp_insitu_ctd) + (3.6276 * temp_insitu_ctd^2) -
      (0.043219 * temp_insitu_ctd^3),    #dimensionless
    kh = (k600h * (Sc / 600) ^ -0.5), #cm/h
    kd = (k600d * (Sc / 600) ^ -0.5), #cm/h
   kh = kh/10, #m/h
   kd = kd/10, #m/h
    alpha = seacarb::K0(
      S = sal_insitu_ctd,
    ...
    alpha = alpha * (seacarb::rho(
      S = sal_insitu_ctd, T = temp_insitu_ctd, P = 0) / 1000), # mol/(l atm)
    alpha = alpha * 1000 , # mol/(m3 atm)
    f_air_sea_h = kh * alpha * delta_pco2, # mol/(m2 h 1e6)
    f_air_sea_h = kh * alpha * delta_pco2 * 1e-6 * 1000, # mmol/(m2 h)
    f_air_sea_d = kd * alpha * delta_pco2, # mol/(m2 h 1e6)
    f_air_sea_d = kd * alpha * delta_pco2 * 1e-6 * 1000 # mmol/(m2 h)
  )

p_f_air_sea_h <- ggplot(data = d_all) +
  geom_point(aes(x = datetime, y = f_air_sea_h), col = "blue", size = 0.15, na.rm = TRUE) +
  labs(title = "Air-sea CO2 flux (all data) with Ho's formulation", x = "Time", y = "CO2 (mmol/(m2 h))") +
  Mytheme(size_labs = 8)
p_f_air_sea_h
ggsave(filename = "figures/p_f_air_sea_h.png", plot = p_f_air_sea_h, width = 20, units = "cm")

air_sea_day_h <- d_all %>%
  dplyr::select(datetime, f_air_sea_h) %>%
  dplyr::group_by(lubridate::yday(datetime)) %>%
  dplyr::summarise(f_air_sea = 24 * mean(f_air_sea_h, na.rm = TRUE)) %>% #mmol/(m2 d)
  ungroup() %>%
  dplyr::rename(day = 1)
  
p_f_air_sea_day_h <- ggplot(data = air_sea_day_h) +
  geom_point(aes(x = day, y = f_air_sea), col = "blue", size = 0.15, na.rm = TRUE) +
  labs(title = "Air-sea CO2 flux (per day) with Ho's formulation", x = "Julian day", y = "CO2 (mmol/(m2 d))") +
  Mytheme(size_labs = 8)
#p_f_air_sea_d
ggsave(filename = "figures/f_air_sea_day_h.png", plot = p_f_air_sea_day_h, width = 20, units = "cm")

air_sea_month_h <- d_all %>%
  dplyr::select(monthd, f_air_sea_h) %>%
  dplyr::group_by(monthd) %>%
  dplyr::summarise(f_air_sea = 24 * mean(f_air_sea_h, na.rm = TRUE)) %>% #mmol/(m2 d)
  ungroup() %>%
...
#air_sea_month
p_f_air_sea_month_h <- ggplot(data = air_sea_month_h) +
  geom_point(aes(x = montht, y = f_air_sea), col = "blue", size = 2, na.rm = TRUE) +
  lims(y = c(-3000, 0)) +
  labs(title = "Air-sea CO2 flux (per month) with Ho's formulation", x = "Month", y = "CO2 (mmol/(m2 month))") +
  Mytheme(size_labs = 8)
ggsave(filename = "figures/f_air_sea_m_h.png", plot = p_f_air_sea_month_h, width = 20, units = "cm")

p <- p_f_air_sea_h / p_f_air_sea_day_h / p_f_air_sea_month_h
ggsave(filename = "figures/air-sea_CO2_flux_h.png", plot = p, width = 20, units = "cm")
knitr::include_graphics("figures/air-sea_CO2_flux_h.png")
...
# temp_insitu_11m
tmp <- d_all %>% 
  dplyr::select(montht, f_air_sea_h)
ts_air_sea_h <- tmp %>%
  ggplot(aes(x = montht, y = f_air_sea_h / 1000), na.rm = TRUE) +
  ...
  theme(aspect.ratio=0.5,
        plot.margin = margin(t = -10, r = 0.2, b = -10, l = 0.2, unit = "cm"),
        axis.title.x = element_text(face = "bold", size = 14),
        axis.title.y = element_text(face = "bold", size = 14),
  ...
  scale_x_discrete(breaks = c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"), 
                   labels = c("Jan", "", "Mar", "", "May", "", "Jul", "", "Sep", "", "Nov", ""))
ts_air_sea_h
ggsave(filename = "figures/ts_air_sea_h.png", plot = ts_air_sea_h, width = 20, units = "cm")
ggsave(filename = "figures/essd/ts_air_sea_h.pdf", plot = ts_air_sea_h, width = 15, height = 10, units = "cm")

# For paper Dobashi & Ho 
# Above, only Ho's formulation is dealt with
# make tibble for Dobashi's formulation to be used in plot
air_sea_month_d <- d_all %>%
  dplyr::select(monthd, f_air_sea_d) %>%
  dplyr::group_by(monthd) %>%
  dplyr::summarise(f_air_sea = 24 * mean(f_air_sea_d, na.rm = TRUE)) %>% #mmol/(m2 d)
  ungroup() %>%
  dplyr::mutate(f_air_sea = days_in_month(monthd) * f_air_sea) %>% 
  dplyr::mutate(montht = month(monthd, label = TRUE, abbr = TRUE))
# air_sea_month_d <- d_all %>%
...
# Now we combine Ho's and Dobashi's formulations
air_sea_month_h <- air_sea_month_h %>% 
  dplyr::rename(f_air_sea_h = f_air_sea) %>% 
  dplyr::select(-monthd)
air_sea_month_d <- air_sea_month_d %>% 
  dplyr::rename(f_air_sea_d = f_air_sea) %>% 
  dplyr::select(-monthd)
air_sea <- dplyr::left_join(air_sea_month_h, air_sea_month_d) 
air_sea_long <- air_sea %>% 
  pivot_longer(!montht, names_to = "Formulation", values_to = "f_air_sea")

size_point <- 1.5 # size of data points
alpha <- 1 #transparency
size_labs <- 14 # font size labels
# temp_insitu_11m
ts_air_sea <- air_sea_long %>%
  ggplot(aes(x = montht, y = f_air_sea / 1000, fill = Formulation), na.rm = TRUE) +
  geom_col(alpha = alpha, position = "dodge") +
  lims(y = c(-3, 0)) +
  geom_hline(yintercept = 0) +
  labs(title=NULL, 
       x="Month", 
       y=latex2exp::TeX("Air-sea CO$_2$ flux (mol $m^{-2}$ month$^{-1}$)", bold = TRUE)) +
  scale_fill_discrete(name = "", labels = c("Dobashi & Ho (2023)", "Ho et al. (2006)")) +
  Mytheme(size_labs = size_labs) +
  theme(aspect.ratio=0.5,
        plot.margin = margin(t = -10, r = 0.2, b = -10, l = 0.2, unit = "cm"),
        axis.title.x = element_text(face = "bold", size = 14),
...
ggsave(filename = "figures/essd/ts_air_sea.pdf", plot = ts_air_sea, width = 15, height = 10, units = "cm")
```
...
d_all <- d_all %>%
  dplyr::mutate(
    delta_pco2_corr = (pco2_fb_ctd_temp + 17) - co2_air, # uatm
    k600h = 0.266 * ws ^ 2, # cm/h Ho et al. (2006)
    k600d = 0.143 * ws ^ 2, # cm/h Dobashi and Ho (2023)
    Sc = 2073.1 - (125.62 * temp_insitu_ctd) + (3.6276 * temp_insitu_ctd^2) -
      (0.043219 * temp_insitu_ctd^3),    #dimensionless
   ...
   kd = kd/10, #m/h
    ...
    alpha = alpha * (seacarb::rho(
      S = sal_insitu_ctd, T = temp_insitu_ctd, P = 0) / 1000), # mol/(l atm)
    alpha = alpha * 1000 , # mol/(m3 atm)
    f_air_sea_h_corr = kh * alpha * delta_pco2_corr, # mol/(m2 h 1e6)
    f_air_sea_h_corr = kh * alpha * delta_pco2_corr * 1e-6 * 1000, # mmol/(m2 h)
    f_air_sea_d_corr = kd * alpha * delta_pco2_corr, # mol/(m2 h 1e6)
    f_air_sea_d_corr = kd * alpha * delta_pco2_corr * 1e-6 * 1000 # mmol/(m2 h)
  )

air_sea_month_h_corr <- d_all %>%
  dplyr::select(monthd, f_air_sea_h_corr) %>%
  dplyr::group_by(monthd) %>%
  dplyr::summarise(f_air_sea = 24 * mean(f_air_sea_h_corr, na.rm = TRUE)) %>% #mmol/(m2 d)
  ungroup() %>%
  dplyr::mutate(f_air_sea = days_in_month(monthd) * f_air_sea) %>% 
  dplyr::mutate(montht = month(monthd, label = TRUE, abbr = TRUE))
...
# Above, only Ho's formulation is dealt with
# make tibble for Dobashi's formulation to be used in plot
air_sea_month_d_corr <- d_all %>%
  dplyr::select(monthd, f_air_sea_d_corr) %>%
  dplyr::group_by(monthd) %>%
  dplyr::summarise(f_air_sea = 24 * mean(f_air_sea_d_corr, na.rm = TRUE)) %>% #mmol/(m2 d)
  ungroup() %>%
  dplyr::mutate(f_air_sea = days_in_month(monthd) * f_air_sea) %>% 
  dplyr::mutate(montht = month(monthd, label = TRUE, abbr = TRUE))
...
# Now we combine Ho's and Dobashi's formulations
air_sea_month_h_corr <- air_sea_month_h_corr %>% 
  dplyr::rename(f_air_sea_h_corr = f_air_sea) %>% 
  dplyr::select(-monthd)
air_sea_month_d_corr <- air_sea_month_d_corr %>% 
  dplyr::rename(f_air_sea_d_corr = f_air_sea) %>% 
  dplyr::select(-monthd)
air_sea <- dplyr::left_join(air_sea_month_h_corr, air_sea_month_d_corr) 
air_sea_long <- air_sea %>% 
  pivot_longer(!montht, names_to = "Formulation", values_to = "f_air_sea")
...
ts_air_sea_corr <- air_sea_long %>%
  ...
  #                  labels = c("Jan", "", "Mar", "", "May", "", "Jul", "", "Sep", "", "Nov", ""))
ts_air_sea_corr
ggsave(filename = "figures/ts_air_sea_corr.png", plot = ts_air_sea_corr, width = 20, units = "cm")
#ggsave(filename = "figures/essd/ts_air_sea_corr.pdf", plot = ts_air_sea_corr, units = "cm")
...
ts_air_sea <- air_sea_long %>%
  ...
  #                  labels = c("Jan", "", "Mar", "", "May", "", "Jul", "", "Sep", "", "Nov", ""))
ts_air_sea_corr
ggsave(filename = "figures/essd/ts_air_sea.pdf", plot = ts_air_sea_corr, units = "cm")
```

The annual air-sea CO2 flux with the Ho et al. (2006) formulation is
`r round(sum(air_sea_month_h$f_air_sea_h/1000), digits = 1)` and
`r round(sum(air_sea_month_h_corr$f_air_sea_h_corr/1000), digits = 1)` mol CO2/(m2
year) without and with correction, respectively. Or `r round(sum(air_sea_month_h$f_air_sea_h*(12/44)/1000), digits = 1)` and
`r round(sum(air_sea_month_h_corr$f_air_sea_h_corr*(12/44)/1000), digits = 1)` mol C/(m2
year) 

The annual air-sea CO2 flux with the Dobashi et al. (2023) formulation is
`r round(sum(air_sea_month_d$f_air_sea_d/1000), digits = 1)` and
`r round(sum(air_sea_month_d_corr$f_air_sea_d_corr/1000), digits = 1)` mol CO2/(m2
year) without and with correction, respectively. Or `r round(sum(air_sea_month_d$f_air_sea_d*(12/44)/1000), digits = 1)` and
`r round(sum(air_sea_month_d_corr$f_air_sea_d_corr*(12/44)/1000), digits = 1)` mol C/(m2
year) 
...
ts_oa <- tmp %>%
  ...
  ylim(0, 3) +
  labs(title=NULL,x="Time", y=latex2exp::TeX("\\textbf{$\\Omega_{aragonite}$}")) +
  Mytheme(size_labs = size_labs) +
    theme(aspect.ratio=0.5,
...
ts_sal <- d_all %>%
  ...
  scale_x_datetime(breaks = date_breaks("1 year"), labels = date_format("%Y")) +
  geom_point(colour="blue", alpha = alpha, na.rm = TRUE, size = size_point) +
  labs(title="A",x=NULL, y="Salinity (< 8 m)") +
...
# to change color of median line
tmp <- ggplot_build(ts_sal_bp)$data[[1]]
ts_sal_bp <- ts_sal_bp + geom_segment(data=tmp, aes(x=xmin, xend=xmax, 
                               y=middle, yend=middle), colour="cyan", size=1)
...
ts_temp_bp <- d_bp %>%
  ...
  #                  labels = c("Jan", "", "Mar", "", "May", "", "Jul", "", "Sep", "", "Nov", ""))
# to change color of median line
tmp <- ggplot_build(ts_temp_bp)$data[[1]]
ts_temp_bp <- ts_temp_bp + geom_segment(data=tmp, aes(x=xmin, xend=xmax, 
                               y=middle, yend=middle), colour="cyan", size=1)
ts_pco2_bp <- d_bp %>%
  ...
  #                  labels = c("Jan", "", "Mar", "", "May", "", "Jul", "", "Sep", "", "Nov", ""))
# to change color of median line
tmp <- ggplot_build(ts_pco2_bp)$data[[1]]
ts_pco2_bp <- ts_pco2_bp + geom_segment(data=tmp, aes(x=xmin, xend=xmax, 
                               y=middle, yend=middle), colour="cyan", size=1)
...
ts_pH_bp <- d_bp %>%
  ...
  #                  labels = c("Jan", "", "Mar", "", "May", "", "Jul", "", "Sep", "", "Nov", ""))
# to change color of median line
tmp <- ggplot_build(ts_pH_bp)$data[[1]]
ts_pH_bp <- ts_pH_bp + geom_segment(data=tmp, aes(x=xmin, xend=xmax, 
                               y=middle, yend=middle), colour="cyan", size=1)
ts_at_bp <- d_bp %>%
  ...
  scale_x_discrete(breaks = c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"), 
                   labels = c("Jan", "", "Mar", "", "May", "", "Jul", "", "Sep", "", "Nov", ""))
# to change color of median line
tmp <- ggplot_build(ts_at_bp)$data[[1]]
ts_at_bp <- ts_at_bp + geom_segment(data=tmp, aes(x=xmin, xend=xmax, 
                               y=middle, yend=middle), colour="cyan", size=1)
