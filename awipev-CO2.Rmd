...
d_hour <- d_hour %>%
  ...
  dplyr::rename(at_contros=AT,pco2_contros=PCO2_corr_contros_filtered,pH_durafet=HW_pH1_filtered,temp_insitu_11m=temp_insitu_11m_filtered, sal_fb = sal_fb_filtered, sal_insitu_ctd = sal_insitu_ctd_filtered, temp_fb = temp_fb_filtered, temp_insitu_ctd = temp_insitu_ctd_filtered, temp_sf=T_seaF)
```
...
```{r Depth filter, fig.width=10, fig.height=4, echo=FALSE, message= FALSE}
#Make groups

d_all <- d_all%>%
  dplyr::mutate(depth = ifelse(pressure_insitu_ctd_filtered <=2, "1 m", 
                        ifelse(pressure_insitu_ctd_filtered >2 & pressure_insitu_ctd_filtered <=4, "3 m", 
                        ifelse(pressure_insitu_ctd_filtered >4 & pressure_insitu_ctd_filtered <=6, "5 m",
                        ifelse(pressure_insitu_ctd_filtered >6 & pressure_insitu_ctd_filtered <=8, "7 m",
                        ifelse(pressure_insitu_ctd_filtered >8, "9 m",NA))))))
```
...
p_sal_fb_sal_insitu<-  ggplot(d_all%>%dplyr::filter(pressure_insitu_ctd_filtered >=8),aes(x=sal_insitu_ctd, y= sal_fb, label = month))+
...
p<-  ggplot(d_all,aes(x=temp_insitu_11m, y= temp_fb, label = month))+
  geom_text( aes(color=factor(month))) + 
 # scale_color_viridis_d(option = "C", guide="none")+
...
d_all$diff_T_fb_insitu <- d_all$temp_sf - d_all$temp_insitu_11m
...
d_all$diff_T_fb_insitu <- d_all$temp_sf - d_all$temp_insitu_ctd
mean_diff_T_fb_insitu <- round(mean(d_all$diff_T_fb_insitu, na.rm = TRUE), digits = 2)
sd_diff_T_fb_insitu <- round(sd(d_all$diff_T_fb_insitu, na.rm = TRUE), digits = 2)
```
...
d_all <-  d_all%>%
  dplyr::mutate(voltINT_sup8m = ifelse(pressure_insitu_ctd_filtered >=8,voltINT, NA),
                voltEXT_sup8m = ifelse(pressure_insitu_ctd_filtered >=8,voltEXT, NA), 
                pH_s_seafet_temp_insitu_sup8m = ifelse(pressure_insitu_ctd_filtered >=8,pH_s_seafet_temp_insitu, NA) )
####
## calculation of E0INT25/E0EXT25 ##
##     new sf_calib function      ##
####
calib_spectro <- sf_calib(calEint = d_all$voltINT_sup8m, calEext = d_all$voltEXT_sup8m, calpH = d_all$pH_s_seafet_temp_insitu_sup8m,
...
####
seafet_calib <- sf_calc(calEint = d_all$voltINT_sup8m, calEext = d_all$voltEXT_sup8m, E0int25 = d_all$E0int25_sf,  E0ext25 = d_all$E0ext25_sf, calT = d_all$temp_mix, calSal = d_all$sal_mix_interp)
...
seafet_contros_xts <- dplyr::select(d_all, datetime, phINT_filtered,pH_s_seafet_temp_insitu,pHint_tot_sf, pH_s_sf_no_calb, pressure_insitu_ctd_filtered)
#seafet_contros_xts <- dplyr::select(d_all, datetime,voltINT)
seafet_contros_xts <- as.xts(seafet_contros_xts, order.by = d_all$datetime)
dygraph(seafet_contros_xts, group = "awipev", main="pH seaFET (in situ)", ylab="pHT") %>%
  dySeries("pressure_insitu_ctd_filtered", axis="y2",label = "Depth", color ="pink", strokeWidth=0, pointSize=2) %>%
...
p <- ggplot(d_all%>%filter(datetime >= ("2017-07-30")),aes(x=datetime, y= phINT_filtered, label = depth))+
  geom_line( aes(color=factor(depth))) 

p_py <- ggplotly(p,dynamicTicks = TRUE)
filename.html <- paste0("/figures/p_py.html")
saveWidget(p_py, filename.html, selfcontained = T, libdir = "lib")

getwd()

seafet_depth_xts <- dplyr::select(d_all, datetime, phINT_filtered, depth)
seafet_depth_xts <- as.xts(seafet_depth_xts, order.by = d_all$datetime)
dygraph(seafet_depth_xts, group = "depth", main="pH seaFET RAW (in situ)", ylab="pHT") %>%
  dySeries("phINT_filtered", axis="y2",label = "pH RAW", color ="green", strokeWidth=0, pointSize=2,  stepPlot = TRUE) %>%
  #dySeries("pHint_tot_sf", label = "pHT corrected", color =  RColorBrewer::brewer.pal(3, "Dark2")[1], strokeWidth = 0, pointSize = 2) %>%
  #dyGroup(depth, drawPoints = TRUE, color = "green")%>%
  dyEvent(as.POSIXct("2017-08-24 12:00:00", tz="GMT"), "seaFET #1005 installed", labelLoc = "bottom", color="red") %>%
  dyEvent(as.POSIXct("2018-02-02 09:00:00", tz="GMT"), "seaFET settings changed to every 1H", labelLoc = "bottom", color="red") %>% 
  dyEvent(as.POSIXct("2018-04-17 12:00:00", tz="GMT"), "seaFET #0007 installed + continuous mode set", labelLoc = "bottom", color="red") %>%
    dyEvent(as.POSIXct("2019-09-02 12:00:00", tz="GMT"), "seaFET #0007 out of water for cleaning", labelLoc = "bottom", color="red") %>%
...
# # #@@@@@ test plot #@@@@@ #@@@@@ #@@@@@
# seafet_contros_xts <- dplyr::select(d_all, datetime, temp_fb, temp_insitu)#voltINT, phINT_filtered,pH_s_seafet_temp_insitu, temp_sf, temp_fb, temp_insitu, sal)
# #seafet_contros_xts <- dplyr::select(d_all, datetime,voltINT)
# seafet_contros_xts <- as.xts(seafet_contros_xts, order.by = d_all$datetime)
# dygraph(seafet_contros_xts, group = "awipev", main="pH seaFET (in situ)", ylab="pHT") %>%
