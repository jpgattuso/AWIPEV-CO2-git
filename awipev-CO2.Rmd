...
pH_seafet <- pH_seafet %>% 
  dplyr::mutate(
    ph_s_sf_temp_insi = pHinsi(pH = pH_seafet$ph_s_sf, 
                         ...
                         S = pH_seafet$sal_mix, Pt = 0, Sit = 0, k1k2 = "x", kf = "x", ks="d", 
                         pHscale = "T", b="u74"),
...
# seaFET pH 
discrete2merge <- discrete %>%
  dplyr::select(closest_datetime, ph_s_sf_temp_insi_mean) %>%
...
diff_ls <- as_tibble(carb_l - carb_s) %>% 
  dplyr::select(pH, pCO2, DIC, OmegaAragonite)
diff_ls_quant <- bind_rows(
  quantile(diff_ls$pH), quantile(diff_ls$pCO2), quantile(diff_ls$DIC * 1e6), 
  quantile(diff_ls$OmegaAragonite)) %>% 
  dplyr::mutate(Variable = c("pH", "pCO2", "DIC", "OmegaAragonite"), .before = 1)
