...
dygraph(seafet_contros_xts, group = "awipev", main="pH seaFET (in situ)", ylab="pHT") %>%
  ...
  dyEvent(as.POSIXct("2018-04-17 12:00:00", tz="GMT"), "seaFET #0007 installed + continuous mode set", labelLoc = "bottom", color="red") %>%
    dyHighlight(highlightCircleSize = 8,highlightSeriesBackgroundAlpha = 0.5,hideOnMouseOut = TRUE) %>%
    dyOptions(drawGrid = TRUE, drawPoints = TRUE, pointSize = c(1,8,1),useDataTimezone = TRUE, digitsAfterDecimal=4) %>%
    dyAxis("y",valueRange = c(7.45, 8.6)) %>%
    dyRangeSelector(height = 30, dateWindow= NULL) 
# TA
...
#   scale_y_continuous(sec.axis = sec_axis(~.*5, name = "Salinity (PSU)"))
# print(poster_temp)
# par(mar=c(5, 4, 4, 6) + 0.1)
# 
# png("poster_sal_temp.png", width= 900, height=600)
# 
# plot(sal_fb ~ datetime,d_all,pch=16, axes=T, ylim=c(30.5,36), cex=0.5, col="red", ylab="", xlab="", yaxt ="n", lty=1, cex.lab = 3) +
...
#    par(new = T)
#  
# plot(as.Date(d_all$datetime,"%Y-%m-d %H:%M:%S", tz= "UTC"), d_all$temp_fb, col="blue", pch=16, cex=0.5, xlab="", ylab="", axes=FALSE,ylim=c(-2,9))+
# axis(side = 4, col="blue",col.axis="blue",las=1, cex.axis =1.8) +
#    # axis.POSIXct(1, at=seq(as.Date("2015-09-01"), max =as.Date("2019-09-01"), by="year"), format="%Y") +
# axis.POSIXct(1,d_all$datetime,format(d_all$datetime,"%Y-%m-%d %H:%M:%S",tz="UTC"), major.format = "%Y-%m",labels = format(d_all$datetime, "%Y-%m-d %H:%M:%S"), cex =3)
# 
# mtext("Time",side=1,col="black",line=0.5)  
# legend("topleft", legend=c("Temperature (°C)", "Salinity (PSU)"),
...
ggsave(paste0("figures/Discrete_TA_salinity_AWIPEV.png"), at_s, width=15, height =10, units="cm")
...
p_at_sal <- ggplot(data = d_all%>%dplyr::filter(at_calc > 2000), aes(x=datetime,y=at_calc,col="TA calculated from salinity") ) +
  ...
  labs(title=  "Calculated TA from salinity (µmoles/kg)", x = "Time", y= "") +
  scale_x_datetime(date_minor_breaks ="1 month")+
  theme_bw() + theme(legend.position= c(0.7, 0.15), 
                     legend.text=element_text(size=15),
                     plot.title = element_text(vjust = 0.5,size = rel(1.5),margin = margin(t = 10, b = -20)),
...
at_datetime <- ggplot(data = at_nov_2018, aes(x=datetime)) +
  geom_point(aes(y=AT_filtered, colour = "before")) +
  geom_point(aes(y=AT_filtered_despike, colour = "after")) +
...
#plot for POSTER june 2019
#plot of TA VS calculated
poster_at_atcal <- ggplot(data = at_nov_2018%>%dplyr::filter(at_calc > 2000), aes(x=at_calc,y=AT_filtered_despike,col="TA calculated from salinity") ) +
  geom_point(col="blue", size=0.5 )+
  labs(x = "Calculated TA", y = "Calculated TA from salinity (µmoles/kg)") +
  theme_bw() + theme(legend.position="bottom",legend.title = element_blank())
print(poster_at_atcal)
ggsave(paste0("figures/poster_at_atcal.png"), width= 30, height=10, poster_at_atcal, units="cm")
```
...
#plot of pCO2
poster_pco2 <- ggplot( ) +
  geom_point(data=d_all%>%dplyr::filter(pco2_contros > 150 & pco2_contros < 410), aes(x=datetime, y= pco2_contros,col="blue"), col="blue",size=0.5 ) +
  geom_point(data=d_all, aes(x=datetime, y=pco2calc, fill="Calculated from AT/CT"), col="red" )+
  labs(x = "Time", y = expression(~italic(p)~CO[2]~"(µatm)")) +
  theme_bw() + theme(legend.position="bottom",legend.title = element_blank())
print(poster_pco2)
...
d_all <- d_all %>%
  dplyr::mutate(deployment_sf = ifelse(datetime >= sfinterv[1] & datetime <= sfinterv[2], 01, 
                                ...
                                ifelse(datetime >= sfinterv[14] , 14,NA )))))))))))))))
...
dygraph(seafet_contros_xts, group = "awipev", main="pH seaFET (in situ)", ylab="pHT") %>%
  ...
  dyShading(from = sfinterv[14], to= Sys.Date(), color = "#E0E0E0") %>%
...
poster_ph <- ggplot( ) +
  ...
  geom_point(data=d_all%>%dplyr::filter(datetime > "2017-07-30"  ), aes(x=datetime, y=pH_s_seafet_temp_field, fill="Reference measured samples"), col="red" )+
  labs(x = "Time", y = expression(pH[T]~seaFET)) +
  theme_bw() + theme(legend.position="bottom",legend.title = element_blank()) 
...
d_all <- d_all %>%
  dplyr::mutate(deployment_dur = ifelse(datetime >= durinterv[1] & datetime <= durinterv[2], 01, 
                                ...
                                ifelse(datetime >= durinterv[15], 14,NA ))))))))))))))))
...
d_all <- d_all%>%
 dplyr::mutate(offset_durafet = ifelse(deployment_dur == 1, dat_dur$diff_pH_dur[1],
                                ...
                                ifelse(deployment_dur == 14, dat_dur$diff_pH_dur[14],0)))))))))))))),
...
durafet_contros_xts <- as.xts(durafet_contros_xts, order.by = d_all$datetime)
 dygraph(durafet_contros_xts, group = "awipev", main="pH durafet (in Ferrybox)", ylab="pHT") %>%
   ...
   dyShading(from = durinterv[14], to= Sys.Date(), color = "#CCCCCC") %>%
    dyHighlight(highlightCircleSize = 8,highlightSeriesBackgroundAlpha = 0.5,hideOnMouseOut = TRUE) %>%
    dyOptions(drawGrid = TRUE, drawPoints = TRUE, pointSize = c(1,8,1),useDataTimezone = TRUE, digitsAfterDecimal=4) %>%
    dyRangeSelector(height = 30, dateWindow= NULL) 
...
poster_dur <- ggplot( ) +
  ...
  geom_point(data=d_all%>%dplyr::filter(datetime > "2017-07-30"  ), aes(x=datetime, y=pH_s_durafet_temp_field, fill="Reference measured samples"), col="red" )+
  labs(x = "Time", y = expression(pH[T]~durafet)) +
  theme_bw() + theme(legend.position="bottom",legend.title = element_blank()) 
...
###**SNAPO**
- **2019-04-30**: Duplicate "Ferrybox" samples (SN0316 and SN264, 2019-02-22 14:05) were sent by mistake in Villefranche for pH measurements. Those results were put in the datafile as "durafet" samples as soon as they are comming from the Ferrybox tape seawater. We will ad a lack of DIC for this week. TA will be measured in Villefranche.
...
- 3 comptes: awipev (partagé avec Samir), gattuso, nydata (pour l’envoi des données par P. Fischer).
...
- Intervention sur le serveur :
    ...
    - update shiny server: follow procedure detailed on the shiny web page 

**Data processing**

- Every night at 01:00 (?) two data files ("all sensors" and AT) are sent by ftp to:
    - awipev-co2.obs-vlfr.fr
    - User name: nydata
    - password:
- Every hour a crontab action runs the script ny-alesund/R/ny-alesund_cc.R. This script essentially appends the daily files added each day around 01:00 and cleans the data
- Every night at 03:00 the web page is generated
- Every night at 05:00 the files are copied to gattuso’s iMac 

**crontab awipev@awipev-co2.obs-vlfr.fr**


<span style="color:red">**Warning:**</span> In the present HTML document double "-" appear with an espace between them (for display reasons). **Do not put the space when typing the code on a terminal wondow.** 

si on veut remplacer intégralement les fichiers de pCloud par ceux de awipev-co2 : ajouter “- -delete” à la commande rsync # m h dom mon dow command 
00 */1 * * * cd ; R - -vanilla < ny-alesund/R/ny-alesund_cc.R

00 03 * * * bash web/awipev-co2.sh > web/awipev-co2.log 2>&1 00 05 * * * /usr/bin/rsync -e "ssh -o StrictHostKeyChecking=no" -avh /home/awipev/awipev-co2_web 'gattuso@gattuso.obs-vlfr.fr:/Users/gattuso/pCloud\ Sync/Documents/experiments/exp168_awipev-CO2/fb_awipev-co2_server/' 
...
- La page web index.html est faite sur awipev@awipev-co2.obs-vlfr.fr
- Création du répertoire web qui contient ce qu’il faut pour générer la page web:
    - AWIPEV-Logo.jpg
    - Discrete_sampling_AWIPEV.csv
    - awipev-CO2_web.Rmd
...
**On utilise un shell script awipev-co2.sh:**
 ...
 # On lance la fabrication du html cd web; R -e 'library("rmarkdown");library("knitr");rmarkdown::render("awipev-CO2_web.Rmd")'; cp awipev-CO2_web.html ../awipev-CO2_web/index.html 
