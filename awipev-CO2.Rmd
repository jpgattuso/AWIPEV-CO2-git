...
```{r Range per month, eval = FALSE, echo=FALSE, message= FALSE, warning=FALSE, results = "asis"}
...
tbl_summary(by = montht,
            ...
            statistic = all_continuous() ~ c(
              ...
              "{min}",
              "{max}"),
            digits = list(all_continuous() ~ c(2, 1, 1, 3)))
range_month
tbl_summary(missing = "no",
            type = sal_fb_mix ~ "continuous2",
            statistic = all_continuous() ~ c(
              "{mean}",    # line 1: mean and SD
              "{median}",
              "{p25}", 
              "{p75}",  # line 2: median and IQR
              "{min}",
              "{max}")
) #%>% 
  #add_n() %>% 
...
#   group_by(montht) %>%
#   my_skim()
# print(range_month, include_summary = FALSE, width = 100)
```
...
ts_pCO2 <- d_all %>%
  ...
  labs(title="pCO2 at in situ temperature (measured in blue; calculated from CT and AT in red)",x="", y="pCO2 (uatm)") +
  Mytheme(size_labs = size_labs)
...
ts_pH <- d_all %>% 
  ...
  labs(title="In situ pH below 8 m at in situ temperature (seaFET in blue; spectrophotometric in red)", x = "", y=expression(paste(pH[T]))) +
  Mytheme(size_labs = size_labs) +
...
ts_at <- d_all %>%
  ...
  labs(title="Total alkalinity (calculated from salinity in blue; discrete measurements in red)", x="", y="AT (umol/kg)") +
  Mytheme(size_labs = size_labs)
...
ts_sal_bp <- d_bp %>%
  ...
  geom_point(aes(x = month(x = datetime, label = TRUE, abbr = TRUE), y = ext_sal_fb), col = "lightblue", size = size_point, na.rm = TRUE) +
