...
p_avail <- pivot_longer(z, -Flag) %>% 
  ...
  coord_flip() +
  labs(x="", y="", title = "Distribution (%) of quality flags") +
  Mytheme(size_labs = 8) #+
...
p_avail_month <- d2_summary %>% 
  ...
  labs(x="Month", y="Number of data (thousands)", title = "") +
  Mytheme(size_labs = 8) #+
...
from ctd (> 8 m). New column "sal_fb_mix" is created and is the one used in
ESSD paper.
...
m). New column "temp_insitu_11m_mix" is created. For the moment, no gaps are
encountered and temp_insitu_11m_mix = temp_insitu_11m.
...
at_s <- d_all_at_discrete %>% 
  ...
  labs(x= latex2exp::TeX("Salinity"),
       y= latex2exp::TeX("Total alkalinity ($\\mu mol\\phantom{x}kg^{-1})$"))  +
  ...
  theme_bw() + Mytheme(size_labs = 9) + theme(aspect.ratio=1) + 
  coord_fixed()
ggsave(filename = "figures/figures/at_s.png", plot = at_s, width = 25, height = 25, units = "cm")
...
`r round(reg$fit$rsquare, digits = 2)` (`r N = reg$fit$n`). Need to compare with other relationships from the literature.
...
Splitting the data below and above salinity 34.5, as done by Nondal et al. (2009), does not prove useful (data not shown). It degrades r2 (0.74 and 0.3 vs 0.81), degrades rmse or marginally improves the rmse (19 and 13.6 vs 16.8).
...
pco2_calc_meas <- d_all %>% 
  ...
  labs(x=latex2exp::TeX("Measured $pCO_{2}\\phantom{x}(\\mu atm)$"),
       y= latex2exp::TeX("Calculated $pCO_{2}\\phantom{x}(\\mu atm)$"))  +
...
interval does include 1; r2 is `r round(reg$fit$rsquare, digits=2)`  (`r N = reg$fit$n`)`.
...
offset_ph <- ph_sf %>% 
  ...
  geom_hline(yintercept=0.02, size=0.3, linetype = "dashed") +
  labs(x = "Date", y=expression(paste(pH[T], " difference")), title = NULL) +
  Mytheme(size_labs = 9, face_font = "plain") + theme(aspect.ratio=1)  + 
  scale_colour_discrete(guide="none") + 
  coord_fixed()
...
Initially (before 2022-08-09), there was 2 outliers on 2019-07-12 and 2018-09-11. On 2022-08-09, code was added above to put a no good qf (3) in order to eliminate those two outliers. Now we are fine. The average offset is `r round(average_offset, digits = 4)`. It considerably improve the plot "pH spectro vs pH seafet (at in situ temperature)".
...
pH INT - pH EXT was plotted here. From now on, one uses seaFET pH from the internal electrode calibrated with spec pH and seaFET data when the REMOS was below 8 m (pHint_tot_sf_10m). This unique pH variable is called pHint_tot_sf. All other pH variables are cleaned up. We also calculate pHint_tot_sf at 4°C: pHint_tot_sf_temp_4.
...
tmp$pHint_tot_sf_temp_4 <- seacarb::pHinsi(pH=tmp$pHint_tot_sf, ALK=mean(tmp$at*1e-6, na.rm = TRUE), Tinsi=4, Tlab=tmp$temp_sf, Pinsi=tmp$depth/10, S=tmp$sal_insitu_ctd_mix, Pt=0, Sit=0, k1k2 = "l", kf = "dg", ks="d", pHscale = "T", b="u74")
d_all <- d_all %>% 
...
# TODO the following throws a warning but seems to work
durPeriod <- d_all %>%
  dplyr::select(datetime, ph_s_dur_temp_fb) %>%
  dplyr::filter(!is.na(ph_s_dur_temp_fb))
durPeriod <- durPeriod %>% 
  #dplyr::arrange(datetime) %>%
  dplyr::mutate(middle =  seconds_to_period(
   c(0, difftime(time1 = datetime[-1], time2 = datetime[-nrow(durPeriod)])/2* 3600 ))) %>%
...
tmp <- d_all %>%
  ...
  tidyr::drop_na()
tmp$pH_dur_temp_4 <- seacarb::pHinsi(pH=tmp$pH_dur, ALK=tmp$at, Tinsi=4, Tlab=tmp$temp_fb, Pinsi=11/10, S=tmp$sal_fb_mix, Pt=0, Sit=0, k1k2 = "l", kf = "dg", ks="d", pHscale = "T", b="u74")
d_all <- d_all %>% 
...
tmp <- discrete %>%
  ...
  tidyr::drop_na()
tmp$pH_calc_fb_temp_4 <- pHinsi(pH = tmp$ph_calc_fb, ALK=tmp$at*1e-6, Tinsi=4, Tlab=tmp$temp_fb, Pinsi=11/10, S=tmp$sal_fb_mix, Pt=0, Sit=0, k1k2 = "l", kf = "dg", ks="d", pHscale = "T", b="u74")
...
```
The relationship between spectrophotometric pH and SeaFET pH) and 
(blue line) is relatively good The slope is
`r signif(reg$fit$regression.results[2,3], 3)`) and its 95% confidence
interval does include 1; r2 is `r round(reg$fit$rsquare, digits=2)`  (`r N = reg$fit$n`)`.
...
diff_all_long <- diff_all %>% 
  ...
  dplyr::mutate(variable = forcats::fct_relevel(variable, 'delta_ph', 'delta_ct', 'delta_OmegaAragonite'), variable = fct_recode(variable, 'DeltapH' = "delta_ph", 'C[T]' = "delta_ct",'Omega[a]' = "delta_OmegaAragonite"))
...
p
# diff_all_long$variable <- factor(diff_all_long$variable, levels = c('C[T]','Omega[a]', 'pH'))
# fct_relevel(diff_all_long$variable, 'delta_ph', 'delta_ct', 'delta_OmegaAragonite')
...
-   large for pH (`r round(diff_ls_quant[4, 2], digits = 3)` pH units on
    ...
    (see above; `r round(errors[1, 7], digits = 3)`). In general, pH is
...
-   small for CT (`r round(diff_ls_quant[4, 3], digits = 1)` umol/kg on
    ...
    (`r round(errors[1, 9], digits = 1)`)
    
-   negligible for OmegaAragonite

`r knitr::kable(diff_ls_quant, digits = c(0, rep(3, 5)), caption = "Lueker minus Sulpis using all discrete Ferrybox data.", format = "simple")`
...
calculated using the formulation of Lueker et al. (2018) is
...
according to seacarb (see above; `r round(errors_pco2_at$pH, digits=4)`).
...
The discrete values of AT, CT, salinity and temperature in the
FerryBox were used to calculate pH using the formulations for K1 and K2
of Lueker et al. (2000) and Sulpis et al. (2020). The differences
induced by the choice of the carbonic acid constants are:
...
-   large for pH (`r round(diff_ls_quant[4, 2], digits = 3)` pH units on
    average; ; median is `r round(diff_ls_quant[3, 2], digits = 3)`)
    compared with the uncertainty calculated with seacarb
    (`r round(errors[1, 7], digits = 3)`). In general, pH is
    higher with Sulpis than with Lueker.
...
`r knitr::kable(diff_ls_quant, digits = c(0, 4, rep(3, 4)), caption = "Lueker minus Sulpis using all discrete Ferrybox data.", format = "simple")`
...
sal_delta_plt <- tmp_delta %>% 
  ...
  geom_col(fill="blue") +
  geom_hline(yintercept = 0) +
  annotate("text", x = 2, y = 0.65, parse = TRUE, label = "S['surface'] > S['deep']") +
  annotate("text", x = 2, y = -0.9, parse = TRUE, label = "S['surface'] <  S['deep']") +
...
temp_delta_plt <- tmp_delta %>% 
  ...
  geom_col(fill="blue") +
  geom_hline(yintercept = 0) +
  annotate("text", x = 2, y = 0.3, parse = TRUE, label = "T['surface'] > T['deep']") +
  annotate("text", x = 2, y = -1.8, parse = TRUE, label = "T['surface'] <  T['deep']") +
  Mytheme(size_labs = 12) +
  theme(panel.grid.major.x = element_blank(), panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank())
...
den_delta_plt <- tmp_delta %>% 
  ...
  geom_col(fill="blue") +
  geom_hline(yintercept = 0) +
  annotate("text", x = 2, y = 0.25, parse = TRUE, label = "D['surface'] > D['deep']") +
  annotate("text", x = 2, y = -1.2, parse = TRUE, label = "D['surface'] <  D['deep']") +
  Mytheme(size_labs = 12) +
  theme(panel.grid.major.x = element_blank(), panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank())
print(den_delta_plt)
...
tmp <- d_all %>%
  ...
  drop_na() %>%
  dplyr::mutate(
    at_sf = (intercept_s_at + slope_s_at * sal_insitu_ctd) * 1e-6,
    pco2_sf = seacarb::carb(
      flag = 8,
      var1 = pHint_tot_sf,
      var2 = at_sf,
      S = sal_insitu_ctd,
  ...
  dplyr::select(datetime, pco2_sf)
d_all <- left_join(d_all, tmp, by = "datetime")
tmp <- d_all %>% 
  dplyr::select(montht, pco2_sf, depth_group) %>%
  tidyr::drop_na() %>% 
  dplyr::filter(depth_group != "NA") %>% 
  group_by(depth_group, montht)
p <- tmp %>% 
  ggplot(aes(x=as.factor(depth_group), y = pco2_sf)) +
  geom_boxplot(notch=FALSE, size=0.2, fill="grey50", outlier.color = "blue", outlier.size = 0.3) +
  labs(title = "pco2_sf vs depth", x = "Depth (m)", y= "fCO2 (uatm)") +
  scale_x_discrete(labels = depth_labels) +
  facet_wrap(vars(montht), scale = "free_y")
print(p)
summary(aov(tmp$pco2_sf ~ tmp$depth_group * tmp$montht))
...
tmp <- d_all %>%
  ...
  dplyr::filter(depth_group2 == '0-4' | depth_group2 == '8-12') %>%
  dplyr::group_by(depth_group2, montht)
summary(aov(tmp$pco2_sf ~ tmp$depth_group2 * tmp$montht))

tmp_delta <- tmp %>%
  group_by(depth_group2, montht) %>%
  dplyr::summarise(pco2_sf_med = median(pco2_sf, na.rm = TRUE)) %>%
  tidyr::pivot_wider(names_from = depth_group2, values_from = pco2_sf_med) %>%
  dplyr::rename(median0_4m = "0-4",
                median8_12m = "8-12") %>%   
  dplyr::mutate(pco2_sf_med_delta = median0_4m - median8_12m)
kable(tmp_delta, digits = 3, caption = "Median seafet pCO2 (median0_4m - median8_12m)")

p <- tidyr::drop_na(tmp) %>% 
  ggplot(aes(x=as.factor(depth_group2), y = pco2_sf)) +
  geom_boxplot(notch=FALSE, size=0.2, fill="grey50", outlier.color = "blue", outlier.size = 0.3) +
...
pco2_delta_plt <- tmp_delta %>%
  ggplot(aes(x = montht, y = pco2_sf_med_delta)) +
  scale_x_discrete("Month", expand = c(0, 0)) +
  scale_y_continuous(substitute("pCO"["2"]), limits = c(-45, 10)) +
  geom_col(fill = "blue") +
  geom_hline(yintercept = 0) +
  annotate(
    "text",
    ...
    y = 5,
    parse = TRUE,
    label = "pCO['2 surface'] > pCO['2 deep']"
  ) +
  annotate(
    "text",
    x = 2.5,
...
tmp <- d_all %>%
  ...
  dplyr::mutate(
    ...
    pco2_sf_4 = seacarb::pCO2insi(pCO2lab = pco2_sf, Tlab = temp_insitu_ctd, SST = 4)
  ) %>% 
  dplyr::select(datetime, pco2_sf_4)
d_all <- left_join(d_all, tmp, by = "datetime")
tmp <- d_all %>% 
  dplyr::select(montht, pco2_sf_4, depth_group) %>%
  tidyr::drop_na() %>% 
  dplyr::filter(depth_group != "NA") %>% 
  group_by(depth_group, montht)
p <- tmp %>% 
  ggplot(aes(x=as.factor(depth_group), y = pco2_sf_4)) +
  geom_boxplot(notch=FALSE, size=0.2, fill="grey50", outlier.color = "blue", outlier.size = 0.3) +
  labs(title = "pco2_sf at 4°C vs depth", x = "Depth (m)", y= "fCO2 (uatm)") +
  scale_x_discrete(labels = depth_labels) +
  facet_wrap(vars(montht), scale = "free_y")
print(p)
summary(aov(tmp$pco2_sf_4 ~ tmp$depth_group * tmp$montht))
...
tmp <- d_all %>%
  ...
  dplyr::filter(depth_group2 == '0-4' | depth_group2 == '8-12') %>%
  dplyr::group_by(depth_group2, montht)
summary(aov(tmp$pco2_sf_4 ~ tmp$depth_group2 * tmp$montht))

tmp_delta <- tmp %>%
  group_by(depth_group2, montht) %>%
  dplyr::summarise(pco2_sf_4_med = median(pco2_sf_4, na.rm = TRUE)) %>%
  tidyr::pivot_wider(names_from = depth_group2, values_from = pco2_sf_4_med) %>%
  dplyr::rename(median0_4m = "0-4",
                median8_12m = "8-12") %>%   
  dplyr::mutate(pco2_sf_4_med_delta = median0_4m - median8_12m)
kable(tmp_delta, digits = 3, caption = "Median seafet pCO2 at 4°C with (median0_4m - median8_12m)")

p <- tidyr::drop_na(tmp) %>% 
  ggplot(aes(x=as.factor(depth_group2), y = pco2_sf_4)) +
  geom_boxplot(notch=FALSE, size=0.2, fill="grey50", outlier.color = "blue", outlier.size = 0.3) +
...
pco2_4_delta_plt <- tmp_delta %>%
  ggplot(aes(x = montht, y = pco2_sf_4_med_delta)) +
  scale_x_discrete("Month", expand = c(0, 0)) +
  scale_y_continuous(substitute("pCO"["2"]), limits = c(-45, 10)) +
  geom_col(fill = "blue") +
  geom_hline(yintercept = 0) +
...
# all deltas
deltas_plt <- sal_delta_plt / temp_delta_plt / den_delta_plt / pco2_delta_plt
...
taka <- d_all %>% # just for this chunk
  dplyr::select(datetime, monthd, sal_fb_mix, temp_insitu_11m, pco2_fb_insi_temp) %>% 
...
Let's look at the stratification tab above (salinity, temperature, density and fCO2): fCO2 is generally higher in the bottom layer than in the surface layer (note that no data is available in May, June and July). Which means that the air-sea CO2 fluxes calculated with pCO2 at 11 m are overestimated.
...
## TO DO: Dobashi R. & Ho D. T., 2022. Air-sea gas exchange in a seagrass ecosystem. EGUsphere 
d_all <- d_all %>%
  dplyr::mutate(delta_pco2 = pco2_fb - co2_air, # ppm
                k600 = 0.266 * ws^2, #cm/hr Ho et al. (2006)
                Sc = 2073.1 - (125.62*temp_insitu_11m) + (3.6276*temp_insitu_11m^2) -
                  (0.043219*temp_insitu_ctd^3), #dimensionless
                k = k600 * (Sc/600)^-0.5, #cm/h
                alp = seacarb::K0(S=sal_fb_mix,T=temp_insitu_11m,P=0,Patm=p_air/1013.25), #mol/(kg/atm)
                alp = alp * (seacarb::rho(S = sal_fb_mix, T = temp_insitu_11m, P = 0)/1000), #mol/(l/atm)
...
tmp <- d_all %>% # just for this chunk
  ...
  dplyr::mutate(OmegaAragonite = seacarb::carb(
    ...
    Patm=1, P=1.1, Pt=0, Sit=0, k1k2="l", kf="dg", ks="d", pHscale="T", b="u74")$OmegaAragonite)
#plot
...
ts_oa <- tmp %>%
  ...
  labs(title="Aragonite saturation state (11 m)",x="", y=latex2exp::TeX("$\\Omega_{aragonite}$")) +
...
pangaea <- as_tibble(d_all) %>% 
  dplyr::select(datetime, depth, sal_insitu_ctd, sal_fb, temp_insitu_11m, temp_insitu_ctd, temp_fb, temp_sf, at, ct, ph_s_sf_temp_insi, ph_s_dur_temp_fb, ph_calc_fb, pco2_fb_insi_temp, pco2_calc_fb_insi_temp, at_calc, pH_dur, pHint_tot_sf) %>%
  dplyr::rename(`pressure [dbar]` = depth,
                `s_insitu [unit]` = sal_insitu_ctd,
                `s_fb [unit]` = sal_fb, 
                `t_11m [°C]` = temp_insitu_11m,
                `t_ctd [°C]` = temp_insitu_ctd, 
                `t_fb [°C]` = temp_fb, 
                `t_sf [°C]` = temp_sf, 
                `at [umol kg-1]` = at, 
                `at_calc [umol kg-1]` = at_calc,
                `ct [umol kg-1]` = ct, 
                `ph_s_sf_t_insi [total scale]` = ph_s_sf_temp_insi, 
                `ph_s_dur_t_fb [total scale]` = ph_s_dur_temp_fb,
                `pco2 [uatm]` = pco2_fb_insi_temp,
                `pco2_calc [uatm]` = pco2_calc_fb_insi_temp,
                `pH_sf [total scale]` = pHint_tot_sf,
                `pH_dur [total scale]` = pH_dur,
                `pH_calc [total scale]` = ph_calc_fb
                )
saveRDS(file= paste0(path, "fb_data/pangaea.rds"), pangaea)
write_csv(pangaea, file = paste0(path, "fb_data/pangaea.csv"))
# Metadata Pangaea
...
ts_sal <- d_all %>%
  ...
  scale_x_datetime(breaks = date_breaks("1 year"), labels = date_format("%Y")) +
  geom_point(colour="blue", alpha = alpha, na.rm = TRUE, size = size_point) +
  labs(title="Salinity (below 8 m)",x="", y="Salinity") +
  Mytheme(size_labs = size_labs)
...
ts_pCO2 <- d_all %>%
  ...
  labs(title=expression(paste(pCO[2], " at 11 m and in situ temperature")),
       subtitle = "measured in blue; calculated from CT and AT in red",
       x="", y=TeX("pCO$_2$ ($\\mu atm$)")) +
  Mytheme(size_labs = size_labs)
...
ts_pH <- d_all %>% 
  ...
  labs(title="In situ pH below 8 m at in situ temperature", 
       subtitle = "seaFET in blue; spectrophotometric in red",
       x = "", y=expression(paste(pH[T]))) +
  Mytheme(size_labs = size_labs) +
...
ts_at <- d_all %>%
  ...
  labs(title="Total alkalinity", 
       subtitle = "calculated from salinity in blue; discrete measurements in red",
       x="", y=TeX("$A_T$ ($\\mu mol\\phantom{x}kg^{-1})$")) +
  Mytheme(size_labs = size_labs)
...
ts_sal_bp <- d_bp %>%
  ...
  geom_point(aes(x = month(x = datetime, label = TRUE, abbr = TRUE), y = ext_sal_fb_mix), col = "lightblue", size = size_point, na.rm = TRUE) +
  labs(title="Salinity (< 8 m)", y = "Salinity", x=NULL) +
  Mytheme() +
  theme(panel.grid.major.x = element_blank(),
        aspect.ratio=0.5)
ts_temp_bp <- d_bp %>%
  ...
  labs(title="In situ temperature (11 m)", y = "Temperature (°C)", x=NULL) +
  Mytheme() +
  theme(panel.grid.major.x = element_blank(),
        aspect.ratio=0.5)
ts_pco2_bp <- d_bp %>%
  ...
  labs(title=expression(paste(pCO[2], " at 11 m and in situ temperature")), y = TeX("pCO$_2$ ($\\mu atm$)"), x=NULL) +
  Mytheme() +
  theme(panel.grid.major.x = element_blank(),
        aspect.ratio=0.5)
ts_pH_bp <- d_bp %>%
  ...
  labs(title="In situ pH below 8 m at in situ temperature", y = expression(paste(pH[T])), x=NULL) +
  Mytheme() +
  theme(panel.grid.major.x = element_blank(),
        aspect.ratio=0.5)
ts_at_bp <- d_bp %>%
  ...
  labs(title="Total alkalinity calculated from salinity", y = TeX("$A_T$ ($\\mu mol\\phantom{x}kg^{-1})$"), x=NULL) +
  Mytheme() +
  theme(panel.grid.major.x = element_blank(),
        aspect.ratio=0.5)
# Combine plots
...
ggsave(filename = "figures/ts_boxplots.png", plot = g2)
...
ggsave(filename = "figures/ts_ts_boxplots.png", plot = g)
rm(list = ls(pattern = "ts_"), d_bp) #delete big ggplot objects and temp data
gc() #garbage collection
```
...
-   sal_fb_mix: when ferrybox data (sal_fb) are missing, they are replaced
    by data from the profiling ctd for depths > 8 m (sal_insitu_ctd_10m)

-   sal_fb_mix_interp: interpolated salinity sal_fb_mix. It is used to have as
    many spec pH to display (some are during missing data)

-   temp_insitu_11m_mix: when data of in situ temperature at 11 m (temp_insitu_11m)
    are missing, they are replaced by data from the profiling ctd for
    depths > 8 m

-   temp_insitu_11m_mix_interp: interpolated temperature temp_insitu_11m_mix. It is used to
    have as many spec pH to display (some are during missing data)

-   sal_insitu_ctd_mix: when CTD salinity data from the REMOS are missing, they are replaced by data from sal_fb_mix (hence at FerryBox from 11 m and REMOS below 8 m). This is not ideal because differences of up to 1 salinity unit occur in the water column. But needed as there are many missing REMOS data.

-   pco2_calc_fb: pco2 calc from AT/CT ferrybox = carb avec temp_fb + sal_fb_mix
