...
# we look at pCO2 0 to 4 m vs more than 8 m
tmp <- d_all %>%
  dplyr::select(montht, pco2_sf, depth_group) %>%
  dplyr::mutate(depth_group2 = as.factor(
    case_when(
...
tmp_delta <- tmp %>%
  ...
  dplyr::rename(median0_4m = "0-4",
                median8_12m = "8-12") %>%   dplyr::mutate(pco2_sf_med_delta = median0_4m - median8_12m)
kable(tmp_delta, digits = 3, caption = "Median seafet fCO2 with (median0_4m - median8_12m)")

p <- tidyr::drop_na(tmp) %>% 
  ggplot(aes(x=as.factor(depth_group2), y = pco2_sf)) +
  geom_boxplot(notch=FALSE, size=0.2, fill="grey50", outlier.color = "blue", outlier.size = 0.3) +
  labs(title = "pCO2", x = "Depth (m)", y= "pHint_tot_sf") +
  facet_wrap(vars(montht), scale = "free_y")
print(p)
...
ts_oa <- tmp %>%
  ...
  labs(title="Aragonite saturation state (11 m)",x="", y=latex2exp::TeX("$\\Omega)$")) +
  Mytheme(size_labs = size_labs)
print(ts_oa)
ggsave(filename = "figures/OmegaAragonite.png", plot = ts_oa, width = 15, height = 30, units = "cm")
```
