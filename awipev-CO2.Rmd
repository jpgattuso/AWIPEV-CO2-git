...
# # #@@@@@ test plot #@@@@@ #@@@@@ #@@@@@
seafet_contros_xts <- dplyr::select(d_all, datetime, at, ct)#voltINT, phINT_filtered,pH_s_seafet_temp_field, T_seaF, temp_fb, temp_insitu, sal)
#seafet_contros_xts <- dplyr::select(d_all, datetime,voltINT)
seafet_contros_xts <- as.xts(seafet_contros_xts, order.by = d_all$datetime)
dygraph(seafet_contros_xts, group = "awipev", main="pH seaFET (in situ)", ylab="pHT") %>%
  ...
  dySeries("ct", label = "ct", color ="red",axis= "y2", strokeWidth=0, pointSize=2) %>%
dySeries("at", label = "at", color ="blue",axis= "y2", strokeWidth=0, pointSize=2) %>%
  #dySeries("temp_insitu", label = "temp_insitu", color ="black",axis= "y2", strokeWidth=0, pointSize=2) %>%
