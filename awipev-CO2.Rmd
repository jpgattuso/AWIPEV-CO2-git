...
d_all <- d_all %>%
  ...
  dplyr::rowwise() %>% 
  dplyr::mutate(n_depth_ctd = sum(is.na( # create new variable to identify the hours 
    # for which there are CTD data at more than 1 depth 
  c(
    sal_insitu_ctd_1m,
    sal_insitu_ctd_3m,
    sal_insitu_ctd_5m,
...
tmp <- d_all %>% 
  ...
  dplyr::mutate(quant_01 = quantile(pHint_tot_sf, na.rm =TRUE, probs = c(0.05)),
                quant_99 = quantile(pHint_tot_sf, na.rm =TRUE, probs = c(0.95)),
                mean = mean(pHint_tot_sf, na.rm =TRUE),
                sd = sd(pHint_tot_sf, na.rm =TRUE)
                ) %>% 
  dplyr::filter(pHint_tot_sf < mean + sd, pHint_tot_sf > mean - sd) # remove outliers
#  dplyr::filter(pHint_tot_sf < quant_99, pHint_tot_sf > quant_01) # remove outliers
