...
# #bind all spectro measurements, vhange name of dataframe only to do the rbind.
# seafetph <- pH_seafet_mean
# colnames(seafetph) <- c("date", "ph_s_sf_insi", "ph_s_sf_temp_4")
# durafetph <- pH_durafet_mean
# colnames(durafetph) <- c("date", "ph_s_dur_temp_fb")
...
discrete2merge <- discrete %>%
  ...
  dplyr::rename(ph_s_dur_temp_fb = ph_s_dur_temp_fb_mean,
                ph_s_dur_temp_4 = ph_s_dur_temp_4_mean) %>%
  dplyr::filter(!is.na(closest_datetime)) %>%
  rename(datetime=closest_datetime)
d_all <- full_join(d_all, discrete2merge, by="datetime")
...
pH_calc$pH_calc_fb_temp_4 <- pHinsi(pH = pH_calc$phcalc_fb, ALK=pH_calc$at*1e-6, Tinsi=4, Tlab=pH_calc$temp_fb, Pinsi=11/10, S=pH_calc$sal_fb, Pt=0, Sit=0, k1k2 = "l", kf = "dg", ks="d", pHscale = "T", b="u74")
...
## Impact of nutrients   {.tabset .tabset-pills}
...
# then check impacts of min and max nutrient values using AT-DIC pair
...
#)
nuts_table <- bind_rows(diff_po4_0a, diff_Si_0a, diff_po4_0b, diff_Si_0b) %>% 
  dplyr::mutate(flag = c("pCO2 and ALK", "pCO2 and ALK", "ALK and DIC", "ALK and DIC"),
...
```
`r knitr::kable(nuts_table, digits = c(1, 1, 4, 2, 4, 3), caption = "Impact of PO4 and Si.", format = "simple")`
...
## Impact of choice of constants  {.tabset .tabset-pills}
### Lueker vs Sulpis using mean discrete values
...
### K1K2 using FerryBox pCO2 and at in situ
```{r impact of constants using FerryBox pCO2 and at in situ, echo=FALSE, warning=FALSE, message=FALSE}
tmp <- d_all %>% 
  tidyr::drop_na(pco2_fb, at, sal_fb, temp_fb) %>% 
  dplyr::select(datetime, ph_s_dur_temp_4, pco2_fb, at_calc, sal_fb, temp_fb)
# Using Lueker  
carb_l <- carb(flag = 24, var1 = tmp$pco2_fb, var2 = tmp$at_calc * 1e-6, S = tmp$sal_fb, 
...
carb_l$pH <- pHinsi(pH = carb_l$pH, ALK = carb_l$ALK, Tinsi = 4, Tlab = carb_l$T, Pinsi = 0, 
                         ...
                         pHscale = "T", b="u74")
# Using Sulpis
carb_s <- carb(flag = 24, var1 = tmp$pco2_fb, var2 = tmp$at_calc * 1e-6, S = tmp$sal_fb, 
...
carb_s$pH <- pHinsi(pH = carb_s$pH, ALK = carb_s$ALK, Tinsi = 4, Tlab = carb_s$T, Pinsi = 0, 
                         ...
                         pHscale = "T", b="u74")
# Using Papadimitriou
carb_p <- carb(flag = 24, var1 = tmp$pco2_fb, var2 = tmp$at_calc * 1e-6, S = tmp$sal_fb, 
               T = tmp$temp_fb, P = 0, Pt = 0, Sit = 0, k1k2 = "p18", kf = "dg", ks = "d", 
               pHscale = "T", b = "u74")
# normalize Papadimitriou-derived pH at 4°C
carb_s$pH <- pHinsi(pH = carb_s$pH, ALK = carb_s$ALK, Tinsi = 4, Tlab = carb_s$T, Pinsi = 0, 
                         S = carb_s$S, Pt = 0, Sit = 0, k1k2 = "p18", kf = "dg", ks="d", 
                         pHscale = "T", b="u74")
# Using Millero et al. (2002)
carb_m <- carb(flag = 24, var1 = tmp$pco2_fb, var2 = tmp$at_calc * 1e-6, S = tmp$sal_fb, 
               T = tmp$temp_fb, P = 0, Pt = 0, Sit = 0, k1k2 = "m02", kf = "dg", ks = "d", 
               pHscale = "T", b = "u74")
# normalize Millero et al. (2002)-derived pH at 4°C
carb_s$pH <- pHinsi(pH = carb_s$pH, ALK = carb_s$ALK, Tinsi = 4, Tlab = carb_s$T, Pinsi = 0, 
                         S = carb_s$S, Pt = 0, Sit = 0, k1k2 = "m02", kf = "dg", ks="d", 
                         pHscale = "T", b="u74")
...
tmp2 <- tibble(datetime = tmp$datetime, ph_s_dur_temp_4 = tmp$ph_s_dur_temp_4, 
               pH_l = carb_l$pH, pH_s = carb_s$pH, 
               pH_p = carb_p$pH, pH_m = carb_m$pH, temp_fb = tmp$temp_fb) %>% 
  tidyr::drop_na(ph_s_dur_temp_4)
# summary of differences
lueker <- as.numeric(summary(tmp2$ph_s_dur_temp_4 - tmp2$pH_l))
sulpis <- as.numeric(summary(tmp2$ph_s_dur_temp_4 - tmp2$pH_s))
papadimitriou <- as.numeric(summary(tmp2$ph_s_dur_temp_4 - tmp2$pH_p))
millero2002 <- as.numeric(summary(tmp2$ph_s_dur_temp_4 - tmp2$pH_m))
diff_table <- tibble(var = c("Min", "Q1", "Median", "Mean", "Q3", "Max"), lueker, sulpis, papadimitriou, millero2002)
# make plots
p1 <- ggreg(xdata = tmp2$ph_s_dur_temp_4, ydata = tmp2$pH_l, type = "MA", plot = TRUE, 
            title = "Lueker vs measured", subtitle = "both pH normalized at 4°C",
            xlab = "Spectrophotometric pH measured @ 4°C",
            ylab = "pH @ 4°C calculated (pCO2+AT) with Lueker") +
  lims(x = c(8, 8.4), y = c(8, 8.4)) +
  theme(plot.title = element_text(size = 10),
        axis.text = element_text(size = 10)) +
  coord_fixed(ratio = 1)
p2 <- ggreg(xdata = tmp2$ph_s_dur_temp_4, ydata = tmp2$pH_s, type = "MA", plot = TRUE, 
  ...
  coord_fixed(ratio = 1)
p3 <- tmp2 %>% 
  dplyr::mutate(delta_lueker = tmp2$ph_s_dur_temp_4 - tmp2$pH_l, 
                delta_sulpis = tmp2$ph_s_dur_temp_4 - tmp2$pH_s, 
                delta_papadimitriou = tmp2$ph_s_dur_temp_4 - tmp2$pH_p, 
                delta_millero2002 = tmp2$ph_s_dur_temp_4 - tmp2$pH_m) %>% 
...
p <- (p1 + p2) / p3
ggsave(filename = "figures/k1k2_all.png", plot = p, width = 25, height = 20, units = "cm")
ggsave(filename = "figures/k1k2_temperature.png", plot = p3, width = 25, height = 20, units = "cm")
```
```{r save elements for k1k2 report, echo=FALSE, message= FALSE, warning=FALSE}
k1k2 <- list(mean_discrete = mean_discrete, const_choice = const_choice, tmp2 = tmp2,
             errors_pco2_at = errors_pco2_at, diff_table = diff_table, 
             diff_ls_quant = diff_ls_quant)
save(k1k2, file="reports/k1k2_data.RData")
```

The discrete values of pCO2, salinity and temperature in the FerryBox and in situ AT were used to calculate pH from measured pCO2 and AT using the formulations for K1 and K2 of Lueker et al. (2000) and Sulpis et al. (2020). Note that this cannot be done with in situ AT and CT because spectrophotometric pH in situ is not at a fixed depth.

The differences induced by the choice of the carbonic acid constants is:

- small for pH (`r round(diff_ls_quant[4, 2], digits = 3)` pH units on average; ; median is `r round(diff_ls_quant[3, 2], digits = 3)`) compared with the uncertainty calculated with seacarb (`r round(const_choice[1, 6], digits = 3)`. In general, pH is higher with Sulpis than with Lueker.
- very good for CT (`r round(diff_ls_quant[4, 3], digits = 1)` umol/kg on average; always lower with Sulpis than with Lueker) compared with the uncertainty calculated with seacarb (`r round(const_choice[1, 8], digits = 1)`)
...
Here we compare pH measured spectrophotometrically with pH calculated from pCO2 and AT using various formulations of K1 and K2. All pH values were normalized to a temperature of 4°C. The absolute differences are large, up to `r round(max(abs(diff_table[, 2:5])), digits=3)` pH units. On average, both formulations overestimate spectrophotometric pH. The calculated pH closer to measured pH is using the formulation of Lueker et al. (2020) with a mean difference of `r round(max(diff_table[4, 2:5]), digits=3)` pH units. This difference is twice larger than the uncertainty for pH calculated from pCO2 and AT according to seacarb (`r round(errors_pco2_at$pH, digits=3)`).
`r knitr::kable(diff_table, digits = 3, caption = "Difference between spectrophotometric pH and pH calculated with pCO2 and AT using different formulations for K1 and K2.", format = "simple")`
`r include_graphics("figures/lueker-sulpis.png")`
...
ts_pH <- d_all %>% 
  ggplot() +
  scale_x_datetime(breaks = date_breaks("1 year"), labels = date_format("%Y")) +
  geom_point(aes(x = datetime, y = pHint_tot_sf, col = pressure_insitu_ctd), alpha=alpha, na.rm=TRUE, size=size_point) + 
