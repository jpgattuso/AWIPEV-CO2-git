...
d_hour <- d_hour %>%
  ...
  dplyr::rename(at_contros=AT,pco2_contros=PCO2_corr_contros_filtered,pH_durafet=HW_pH1_filtered,temp_insitu_11m=temp_insitu_11m_filtered, sal_fb=sal_fb_filtered, sal_insitu_ctds=sal_insitu_ctds_filtered, temp_fb=temp_fb_filtered)
```
...
#SAL
sal_xts <- dplyr::select(d_all, datetime, sal)%>%
  dplyr::filter(sal >28)
...
```{r water masses, fig.width=10, fig.height=4, echo=FALSE, message= FALSE}
...
## Salinities
Several salinity sensors are on site, 1 inside the Ferrybox and 1 outside. Note that several CTD have been replace in situ. The sal_insitu variable combine all these CTD salinity measurements:

- 1 SBE45 inside the Ferrybox (sal_fb)
- 1 CTD at 11 meters depth (sal_insitu)
...
########### Binding sal_fb INTO sal_insitu if we get gaps in sal_insitu = call this new column = sal_mix ########### 
d_all <- d_all %>%
  dplyr::mutate(sal_mix = ifelse(!is.na(sal_fb), sal_fb, sal_insitu))


d_all <- d_all%>%
  dplyr::mutate( sal = ifelse(sal <28, NA, sal) ,
                 sal_fb = ifelse(sal_fb <28, NA, sal_fb),
                 sal_insitu = ifelse(sal_insitu <28, NA, sal_insitu))
  d_all <- d_all%>%
  dplyr::mutate( sal = ifelse(datetime > "2019-12-26 00:00:00", NA, sal),
                 sal_insitu = ifelse(datetime > "2019-12-26 00:00:00", NA, sal_insitu))


fit <- lmodel2(data = d_all,  sal_fb ~ sal_insitu , nperm = 99)

p_sal_fb_sal_insitu<-  ggplot(d_all,aes(x=sal_insitu, y= sal_fb, label = month))+
...
# PLOT
seafet_contros_xts <- dplyr::select(d_all, datetime, sal_fb, sal_insitu, sal)
seafet_contros_xts <- as.xts(seafet_contros_xts, order.by = d_all$datetime)
dygraph(seafet_contros_xts, group = "awipev", main="pH seaFET (in situ)", ylab="pHT") %>%
  dySeries("sal_insitu", label = "sal_insitu", color ="red",axis= "y2", strokeWidth=0, pointSize=2) %>%
dySeries("sal_fb", label = "sal_fb", color ="blue",axis= "y2", strokeWidth=0, pointSize=2) %>%
...
## Temperatures
Several temperature sensors are on site, 1 inside the Ferrybox and 2 outside:

- 1 SBE45 inside the Ferrybox (temp_fb)
- 1 seaFET T°C inside the seaFET at 11 meters depth (T_seaF)
- 1 SBE38 at 11 meters depth (temp_insitu)
...
fit <- lmodel2(data = d_all,  T_seaF ~ temp_insitu , nperm = 99)

pTseaf_Tinsi<-  ggplot(d_all,aes(x=temp_insitu, y= T_seaF, label = month))+
  geom_text( aes(color=factor(month))) + 
 # scale_color_viridis_d(option = "C", guide="none")+
...
d_all$diff_T_seaF_insitu <- d_all$T_seaF - d_all$temp_insitu
...
fit <- lmodel2(data = d_all,  temp_fb ~ temp_insitu , nperm = 99)

pTfb_Tinsi<-  ggplot(d_all,aes(x=temp_insitu, y= temp_fb, label = month))+
  geom_text( aes(color=factor(month))) + 
 # scale_color_viridis_d(option = "C", guide="none")+
...
d_all$diff_T_fb_insitu <- d_all$T_seaF - d_all$temp_insitu
...
```{r TA 2nd cleaning , echo=FALSE, warning=FALSE, message=FALSE, out.width="50%",fig.show='hold', fig.align="center"}
...
d_all_at_discrete <- d_all %>% #  To remove an outlier in red
  dplyr::select(datetime, month,closest_datetime, at, sal)%>%
...
d_all$at_calc <- slope_s_at * d_all$sal + intercept_s_at
```
...
d_all <- d_all%>%
  dplyr::mutate( pH_s_sf_no_calb = ifelse(datetime == "2018-04-16 07:00:00" | datetime == "2018-12-14 09:00:00" | datetime == "2019-01-18 09:00:00", pH_s_seafet_temp_field, NA ))

seafet_contros_xts <- dplyr::select(d_all, datetime, phINT_filtered,pH_s_seafet_temp_field,pHint_tot_sf, pH_s_sf_no_calb)
...
dygraph(seafet_contros_xts, group = "awipev", main="pH seaFET (in situ)", ylab="pHT") %>%
  ...
  dySeries("pHint_tot_sf", label = "pHT corrected", color =  RColorBrewer::brewer.pal(3, "Dark2")[1], strokeWidth = 0, pointSize = 2) %>%
    dySeries("phINT_filtered", label = "Raw data INT", color ="blue", strokeWidth=0, pointSize=2) %>%
    dySeries("pH_s_seafet_temp_field", label = "pH spectro for calibration", color =  "red", strokeWidth = 0, pointSize = 4 ) %>%
...
# # #@@@@@ test plot #@@@@@ #@@@@@ #@@@@@
seafet_contros_xts <- dplyr::select(d_all, datetime, temp_fb, temp_insitu)#voltINT, phINT_filtered,pH_s_seafet_temp_field, T_seaF, temp_fb, temp_insitu, sal)
#seafet_contros_xts <- dplyr::select(d_all, datetime,voltINT)
seafet_contros_xts <- as.xts(seafet_contros_xts, order.by = d_all$datetime)
dygraph(seafet_contros_xts, group = "awipev", main="pH seaFET (in situ)", ylab="pHT") %>%
  #dySeries("voltINT", label = "voltINTINT", color ="blue", strokeWidth=0, pointSize=2) %>%
   # dySeries("pH_s_seafet_temp_field", label = "pH spectro", axis= "y2", color =  "green", strokeWidth = 0, pointSize = 4 ) %>%
...
# dat <- mutate(sfoffset, diff_pH_sf = round((pH_s_seafet_temp_field - pHint_tot_sf),digits  = 4))
...
#   geom_point(data=d_all%>%dplyr::filter(datetime > "2017-07-30"  ), aes(x=datetime, y=pH_s_seafet_temp_field, fill="Discrete measurements"), col="red" )+
...
duroffset <- left_join(durPeriod%>%dplyr::select(datetime,pH_s_durafet_temp_field), d_all%>%dplyr::select(datetime,pH_durafet ), by="datetime")
...
# add spectro points that are not taking into consideration
d_all <- d_all%>%
  dplyr::mutate( pH_s_dur_no_calb = ifelse(datetime == "2017-08-24 13:00:00", pH_s_durafet_temp_field, NA ))

durafet_contros_xts <- dplyr::select(d_all, datetime, pH_durafet, pH_s_durafet_temp_field,pH_durafet_corrected, pH_s_dur_no_calb)
durafet_contros_xts <- as.xts(durafet_contros_xts, order.by = d_all$datetime)
 dygraph(durafet_contros_xts, group = "awipev", main="pH durafet (in Ferrybox)", ylab="pHT") %>%
    dySeries("pH_durafet", label = "Raw durafet pH", color ="blue", strokeWidth=0, pointSize=2) %>%
    dySeries("pH_s_durafet_temp_field", label = "pH spectro for calibration", color =  "red", strokeWidth = 0, pointSize = 6 ) %>%
...
#   geom_point(data=d_all%>%dplyr::filter(datetime > "2017-07-30"  ), aes(x=datetime, y=pH_s_durafet_temp_field, fill="Discrete measurements"), col="red" )+
...
# Spectro / durafet
fit <- lmodel2(data = d_all,  pH_s_durafet_temp_field ~ pH_durafet_corrected , nperm = 99)

p_dura_spectro <-  ggplot(d_all,aes(x=pH_durafet_corrected, y= pH_s_durafet_temp_field, label = month))+
  geom_text( aes(color=factor(month))) + 
  scale_color_discrete(guide = "none")+
  geom_line(aes(x = pH_calc_norm4, y = pH_calc_norm4), linetype = "dashed") +
  geom_abline(aes(intercept = fit$regression.results[2,2], slope = fit$regression.results[2,3]),colour = "blue") +
...
# Spectro / seafet
fit <- lmodel2(data = d_all,  pH_s_seafet_temp_field ~ pHint_tot_sf , nperm = 99)

p_sf_spectro <-  ggplot(d_all,aes(x=pHint_tot_sf, y= pH_s_seafet_temp_field, label = month))+
  geom_text( aes(color=factor(month))) + 
  scale_color_discrete(guide = "none")+
  geom_line(aes(x = pH_calc_norm4, y = pH_calc_norm4), linetype = "dashed") +
  geom_abline(aes(intercept = fit$regression.results[2,2], slope = fit$regression.results[2,3]),colour = "blue") +
