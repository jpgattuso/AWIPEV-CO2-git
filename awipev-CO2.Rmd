...
pCO2 measured in FerryBox expressed at in situ temp vs pCO2 calculated from REMOS data (S, T, pH, salinity-AT) at 0-2 m only.
...
p_pco2_fb_remos <- d_all %>% 
  ...
  geom_point(col="blue", size = 0.5) +
  #geom_text() +
  xlim(250, 400) + ylim(270, 360) +
  labs(title = "pCO2 at ctd temperature",
       x=latex2exp::TeX("$pCO_{2}$ 11 m $\\phantom{x}(\\mu atm)$"),
       y= latex2exp::TeX("$pCO_{2}$ REMOS at 0-4 m\\phantom{x}($\\mu atm)$"))  +
  ggpmisc::stat_ma_line(method = "lmodel2:MA", se = FALSE, size = 1) +
...
p_pco2_fb_remos_month <- tmp %>% 
  ...
  theme_bw() + Mytheme(size_labs = 9) + theme(aspect.ratio = 1) +
  coord_fixed(ratio = 1, expand = TRUE, clip = "on")
p_pco2_fb_remos_month
ggsave(filename = "figures/pco2_fb_remos_monthly.png", plot = p_pco2_fb_remos_month, width = 25, height = 25, units = "cm")
...
g
```
The relationship between pCO2 between 0 and 4 m on the REMOS (estimated from pH and S-derived AT) and pCO2 measured at 11 m (both pCO2 at CTD temperature) is much nicer using monthly median values (b) than using all data (a). The mean and median difference between the two pCO2s are `r round(summary(tmp$delta_pco2_fb_remos_monthly), digits=0)`

```{r air-sea CO2 with measured data, echo=FALSE, message= FALSE, warning=FALSE}
...
d_all <- d_all %>%
  dplyr::mutate(delta_pco2 = pco2_fb_ctd_temp - co2_air, # ppm
                ...
                alpha = seacarb::K0(S=sal_insitu_ctd,T=temp_insitu_ctd,P=0,Patm=p_air/1013.25), #mol/(kg/atm)
                alpha = alpha * (seacarb::rho(S = sal_insitu_ctd, T = temp_insitu_ctd, P = 0)/1000), #mol/(l/atm)
                f_air_sea = k * alpha * delta_pco2 # mmol/(m2 h)
                )
...
```{r air-sea CO2 with corrected data, echo=FALSE, message= FALSE, warning=FALSE}
...
d_all <- d_all %>%
  dplyr::mutate(delta_pco2_corr = (pco2_fb_ctd_temp + 17) - co2_air, # ppm
                k600 = 0.266 * ws^2, #cm/hr Ho et al. (2006)
                Sc = 2073.1 - (125.62*temp_insitu_ctd) + (3.6276*temp_insitu_ctd^2) -
                  (0.043219*temp_insitu_ctd^3), #dimensionless
                k = k600 * (Sc/600)^-0.5, #cm/h
                alpha = seacarb::K0(S=sal_insitu_ctd,T=temp_insitu_ctd,P=0,Patm=p_air/1013.25), #mol/(kg/atm)
                alpha = alpha * (seacarb::rho(S = sal_insitu_ctd, T = temp_insitu_ctd, P = 0)/1000), #mol/(l/atm)
                f_air_sea_corr = k * alpha * delta_pco2_corr # mmol/(m2 h)
                )
...
# air_sea_day_d <- d_all %>%
#   dplyr::select(datetime, f_air_sea_corr) %>%
#   dplyr::group_by(lubridate::yday(datetime)) %>%
#   dplyr::summarise(f_air_sea_corr = 24 * mean(f_air_sea_corr, na.rm = TRUE)/1000) %>% #mol/(m2 d)
#   ungroup() %>%
#   dplyr::rename(day = 1)
...
`r round(sum(air_sea_month$f_air_sea), digits = 0)` and `r round(sum(air_sea_corr_month$f_air_sea_corr), digits = 0)` mol/(m2 year) without and with correction, respectively.
