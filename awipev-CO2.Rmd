...
selected_nydata_minute <- readRDS(file = paste0(path, "fb_awipev-co2_server/ny-alesund/data/NRT_data/nydata_minute.rds")) %>%
  dplyr::select(ends_with("_qf")) #%>%
# s1 <- summary(as.factor(selected_nydata_minute$pco2_corr_qf))*100/length(selected_nydata_minute$pco2_raw_qf)
...
#dySeries("temp_fb", color = "grey", strokeWidth = 0, label = "temp_fb") %>%
#dySeries("temp_mix", color = "black", strokeWidth = 0, label = "temp_mix") %>%
      dyHighlight(highlightCircleSize = 8,highlightSeriesBackgroundAlpha =0.2,hideOnMouseOut =TRUE) %>%
      dyOptions( drawGrid = TRUE, drawPoints = TRUE, pointSize = 2,useDataTimezone = TRUE) %>%
    dyLegend(show = "follow")   %>%
...
pdur_sf_norm_text <-  ggplot(d_all,aes(x=pH_durafet_norm4, y= pHint_tot_sf_norm4, label = month))+
  ...
  #stat_smooth(data = d_all, aes(x = pH_durafet_norm4, y = pHint_tot_sf_norm4, color=month),
...
* temp_mix = temp 11m + temp_insitu_ctd_9m si trous.
* sal_mix = sal FB + sal_insitu_ctd_9m si trous.
...
### Data processing
We use the AWI database [Quick overview AWI NRT database](https://dashboard.awi.de/?dashboard=9736) 
