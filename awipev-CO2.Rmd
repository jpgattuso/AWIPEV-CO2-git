...
at_s <- d_all_at_discrete %>% 
  ...
  labs(x="Salinity", y=TeX("Total alkalinity ($\\mu mol\\phantom{x}kg^{-1})$"))  +
...
pco2_calc_meas <- d_all %>% 
  ...
  ggpmisc::stat_ma_eq(aes(label = after_stat(eq.label))) +
#stat_smooth(method = "lm", se=FALSE, col = "blue", size = 0.5) +
  theme_bw() + Mytheme(size_labs = 9)
...
offset_ph <- ph_sf %>% 
  ...
  scale_colour_discrete(guide="none")
ggsave(filename = "figures/offset_ph.png", plot = offset_ph, width = 10, height = 10, units = "cm")
```
