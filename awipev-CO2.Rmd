...
```{r read data, cleanup and prepare ferrybox data, echo=FALSE, warning=FALSE, message=FALSE}
d_hour <-
  readRDS(file = paste0(
    path,
...
### Using mean values
```{r impact of constants, echo=FALSE, warning=FALSE, message=FALSE}
...
# Compare Lueker and Sulpis
d_ls <- d_all %>% 
  drop_na(pco2_corr, at_calc, sal_fb, temp_fb)
carb_l <-
  carb(
    24,
    d_ls$pco2_corr,
...
# plot
lm <- lm2(x = tmp$sal_insitu_ctd_9m, y = d_all$sal_fb)
p <-  ggplot(d_all,aes(x=sal_insitu_ctd_9m, y= sal_fb))+
  geom_point( aes(color=factor(monthd))) + 
  geom_abline(slope=1, intercept=0, linetype = "dashed")+
  geom_abline(aes(intercept = lm$fit$regression.results[2,2], slope = lm$fit$regression.results[2,3]),colour = "blue") +
...
The difference induces by the choice of the carbonic acid constants is large for pH, pCO2 and DIC and negligible for OmegaAragonite.
