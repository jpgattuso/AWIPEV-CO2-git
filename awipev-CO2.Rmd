...
taka_month_plt <- taka_month %>% 
  ...
  labs(x="Month", y= latex2exp::TeX("Thermal/non-thermal control of $pCO_{2}"))  +
  geom_hline(yintercept = 1, linetype = "dashed") +  
  theme_bw() + Mytheme(size_labs = 12) + theme(aspect.ratio = 1) +
  coord_fixed(ratio = 1, expand = TRUE, clip = "on")
taka_month_plt
ggsave(filename = "figures/taka_month.png", plot = taka_month_plt, width = 25, height = 25, units = "cm")
ggsave(filename = "figures/essd/fig05.pdf", plot = taka_month_plt, width = 25, height = 25, units = "cm")
...
ts_temp_bp <- d_bp %>%
  ...
  labs(title=NULL, y = "Temperature (°C)", x=NULL) +
  Mytheme(size_labs = size_labs) +
  theme(panel.grid.major.x = element_blank(),
        aspect.ratio=0.5)
ts_pco2_bp <- d_bp %>%
  ...
  labs(title=NULL, y = TeX("pCO$_2$ ($\\mu atm$)"), x=NULL) +
  Mytheme(size_labs = size_labs)+
  theme(panel.grid.major.x = element_blank(),
        aspect.ratio=0.5)
ts_pH_bp <- d_bp %>%
  ...
  labs(title=NULL, y = TeX("$pH_T$"), x=NULL) +
  Mytheme(size_labs = size_labs) +
  theme(panel.grid.major.x = element_blank(),
        aspect.ratio=0.5)
ts_at_bp <- d_bp %>%
  ...
  labs(title=NULL, y = TeX("$A_T$ ($\\mu mol\\phantom{x}kg^{-1})$"), x=NULL) +
  Mytheme(size_labs = size_labs) +
  theme(panel.grid.major.x = element_blank(),
        aspect.ratio=0.5)
# Combine plots
g2 <- (ts_sal_bp / ts_temp_bp / ts_pco2_bp / ts_pH_bp / ts_at_bp)
g2 <- g2 + plot_annotation(tag_levels = 'A' & theme(plot.tag = element_text(face = "bold")))
...
g <- g + plot_annotation(tag_levels = 'A' & theme(plot.tag = element_text(face = "bold")))
ggsave(filename = "figures/essd/fig06.pdf", plot = g)
# legend:
# - Salinity below 8m
# - In situ temperature (11m)
# - pCO2 at 11 m and insitu Temperature, measued in blue, calculated from CT an AT in red
