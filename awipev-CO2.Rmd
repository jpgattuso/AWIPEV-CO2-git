...
# # 5th period, with pH spectro at 2018-04-16 07:00:00 must fit one voltINT to calculate E0
# we put the mean of 4 following days because voltINT was not present.
mean_voltINT_4d <- d_all%>%
  dplyr::filter(datetime >= "2018-04-25" & datetime <= "2018-04-29")%>%
  dplyr::summarise( mean(voltINT, na.rm = TRUE))
mean_voltINT_4d <- as.numeric(mean_voltINT_4d)
d_all <- d_all%>%
  dplyr::mutate(voltINT = ifelse(datetime == "2018-04-16 07:00:00" , mean_voltINT_4d, voltINT))
...
# put also a temp value because FB was off
...
# put also a temp insi value because FB was off FOR PERIOD 1 : 2017-08-24
temp_to_replace <- d_all%>%
  dplyr::filter(datetime >= "2017-08-24 12:00:00" & datetime <= "2017-08-24 23:00:00")%>%
  dplyr::summarise( mean(T_seaF, na.rm = TRUE))
temp_to_replace <- as.numeric(temp_to_replace)
d_all$temp_insitu <- ifelse(d_all$datetime == "2017-08-24 14:00:00" , temp_to_replace, d_all$temp_insitu)
...
dygraph(seafet_contros_xts, group = "awipev", main="pH seaFET (in situ)", ylab="pHT") %>%
  dySeries("voltINT", label = "voltINTINT", color ="blue", strokeWidth=0, pointSize=2) %>%
    ...
    dySeries("phINT_filtered", label = "Raw pH INT", color ="red",axis= "y2", strokeWidth=0, pointSize=2) %>%
