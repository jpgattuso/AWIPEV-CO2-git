...
mean_ct <-  mean(monthly_means$mean_ct)
mean_at_calc <-  mean(monthly_means$mean_at_calc)
mean_pco2_calc <-  mean(monthly_means$mean_pco2_calc)
mean_sal <-  mean(monthly_means$mean_sal)
mean_temp <-  mean(monthly_means$mean_temp)
...
The differences are:
`r knitr::kable(const_choice, digits = c(0, 3, 1, 1, 3), caption = "Lueker minus Sulpis", format = "simple")`


### Lueker vs Sulpis using all values pCO2 and at_calc
One uses the pair pCO2-AT_calc
...
# calculate quantiles
diff_ls_quant <- bind_rows(
  quantile(diff_ls$delta_ph), quantile(diff_ls$delta_ct * 1e6),
  quantile(diff_ls$delta_OmegaAragonite)) %>% 
  dplyr::mutate(Variable = c("pH", "DIC", "OmegaAragonite"), .before = 1)
#pander(diff_ls_quant)
...
p1 <- ggreg(xdata = tmp2$ph_s_sf_norm4, ydata = tmp2$pH_l, type = "OLS", plot = TRUE, 
            title = "both pH normalized at 4°C", subtitle = "",
            xlab = "Spectrophotometric pH measured @ 4°C",
            ylab = "pH @ 4°C calculated (pCO2+AT) with Lueker")
p2 <- ggreg(xdata = tmp2$ph_s_sf_norm4, ydata = tmp2$pH_s, type = "OLS", plot = TRUE, title = "both pH normalized at 4°C") 
  
lm <- lm2(x = tmp2$ph_s_sf_norm4, y = tmp2$pH_l)
p <-  ggplot(d_all,aes(x=sal_insitu_ctd_9m, y= sal_fb))+
  geom_point( aes(color=factor(monthd))) + 
  geom_abline(slope=1, intercept=0, linetype = "dashed")+
  geom_abline(aes(intercept = lm$fit$regression.results[2,2], slope = lm$fit$regression.results[2,3]),colour = "blue") +
