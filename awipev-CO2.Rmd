...
<!-- `r include_graphics("figures/kong_map.png")` -->
...
pangaea <- as_tibble(d_all) %>% 
  dplyr::select(datetime, depth, sal_insitu_ctd, sal_fb, temp_insitu_11m, temp_insitu_ctd, temp_fb, temp_sf, at, ct, ph_s_sf_temp_insi, ph_s_dur_temp_fb, ph_calc_fb, pco2_fb_insi_temp, pco2_calc_fb_insi_temp, at_calc, pHint_tot_sf) %>%
  dplyr::rename(`pressure [dbar]` = depth, #        hydrostatic pressure (profiler)
                `s_insitu [unit]` = sal_insitu_ctd, # salinity in situ (profiler)
                `s_fb [unit]` = sal_fb,             # salinity (FerryBox)
                `t_11m [°C]` = temp_insitu_11m,     # temperature in situ (static at 11 m)
                `t_ctd [°C]` = temp_insitu_ctd,     # temperature in situ (profiler)
                `t_fb [°C]` = temp_fb,              # temperature (FerryBox)
                `t_sf [°C]` = temp_sf,              # temperature SeaFET (profiler)
                `at [umol kg-1]` = at,              # total alkalinity in situ, discrete measurements
                `at_calc [umol kg-1]` = at_calc,    # total alkalinity calculated from salinity
                `ct [umol kg-1]` = ct,              # dissolved inotganic carbon in situ, discrete measurements
                `ph_discrete [total scale]` = ph_s_sf_temp_insi, # spectrophotometric pH in situ (total scale) at in situ temperature (discrete)
                #`ph_s_dur_t_fb [total scale]` = ph_s_dur_temp_fb,
                `pco2 [uatm]` = pco2_fb_insi_temp,  # Partial pressure of CO2 (FerryBox)
                #`pco2_calc [uatm]` = pco2_calc_fb_insi_temp,
                `pH_sensor [total scale]` = pHint_tot_sf, # pH in situ at in situ temperature (profiler)
                #`pH_dur [total scale]` = pH_dur,
                #`pH_calc [total scale]` = ph_calc_fb
                ) %>% 
  dplyr::mutate(across(starts_with("pressure"), ~ round(.x, digits = 2)),
                across(starts_with("s_"), ~ round(.x, digits = 2)),
                across(starts_with("t_"), ~ round(.x, digits = 2)),
                across(starts_with("ph"), ~ round(.x, digits = 3)),
                across(starts_with("at") | 
                         starts_with("ct") |
                         starts_with("pco2"), ~ round(.x, digits = 1)),
                )
