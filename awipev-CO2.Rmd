...
p_avail <- pivot_longer(z, -Flag) %>% 
  ...
  coord_flip() +
  labs(x="", y = "Distribution of quality flags (%)") +
...
p_avail_month <- d2_summary %>% 
  ...
  scale_fill_discrete(name = "", labels = c(expression(paste(pCO[2])), "pH")) + 
  labs(x="Month", y="Number of data (thousands)", title = "") +
  Mytheme(size_labs = 12) #+
# Combine plots
p <- (p_avail / p_avail_month)
p <- p + plot_annotation(tag_levels = 'A') & theme(plot.tag = element_text(face = "bold"))
ggsave(filename = "figures/availability.png", plot = p, width = 20, height = 15, units = "cm")
ggsave(filename = "figures/essd/fig01.pdf", plot = p, width = 20, height = 15, units = "cm")
...
at_s <- d_all_at_discrete %>% 
  ...
  theme_bw() + Mytheme(size_labs = 12) + theme(aspect.ratio=1) + 
  coord_fixed()
...
offset_ph <- ph_sf %>% 
  ...
  Mytheme(size_labs = 12, face_font = "plain") + theme(aspect.ratio=1)  + 
  scale_colour_discrete(guide="none") + 
  coord_fixed()
...
p <- ggplot(diff_all_long, aes(x = forcats::fct_relevel(deltas, 'L00-S20', 'L00-P18', 'L00-M02'), y=value))+
  geom_boxplot(notch=FALSE, size=0.2, fill="grey70", outlier.color = "blue", outlier.size = 1) +
...
ggsave(filename = "figures/essd/fig02.pdf", plot = p, width = 25, height = 20, units = "cm")
...
ggsave(filename = "figures/k1k2_pco2_at.png", plot = p, width = 25, height = 20, units = "cm")
...
**Results are shown below but it does not make sense to udse them in the manuscript. Btter to use all pCO2-AT data as above.**
...
ggsave(filename = "figures/essd/fig03.pdf", plot = p, width = 25, height = 20, units = "cm")
# prepare tibble to compare calculated pH and spec pH
tmp2 <- tibble(datetime = tmp$datetime, ph_s_dur_temp_4_l = tmp$ph_s_dur_temp_4_l, 
               ph_s_dur_temp_4_s20 = tmp$ph_s_dur_temp_4_s20,
               ph_s_dur_temp_4_p18 = tmp$ph_s_dur_temp_4_p18,
...
taka_month_plt <- taka_month %>% 
  ggplot() +
  geom_point(aes(x = montht, y = thermal_nonthermal), col="blue", size = 2) +
  labs(x="Month", y= "Thermal/non-thermal control of pCO2")  +
...
p_pco2_fb_remos <- d_all %>% 
  ...
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +  
  theme_bw() + Mytheme(size_labs = 11) + theme(aspect.ratio = 1) +
  coord_fixed(ratio = 1, expand = TRUE, clip = "on")
#p_pco2_fb_remos
...
g <- g + patchwork::plot_annotation(tag_levels = 'A' & theme(plot.tag = element_text(face = "bold")))
ggsave(filename = "figures/pco2_ph_etc.png", plot = g)
ggsave(filename = "figures/essd/fig04.pdf", plot = g)
```
