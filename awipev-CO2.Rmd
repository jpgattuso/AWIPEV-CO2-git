...
The relationship between the measured (Contros) and calculated pCO2 (blue line) is relatively poor. It is far from the 1:1 relationship (black dashed line). The slope is `r signif(fit$regression.results[2,3], 3)`) and its 95% confidence interval does include 1; r2 is `r round(fit$rsquare, digits=2)`.
...
<!--   labs(x="Contros pCO2", y="Calculated pCO2") + theme_bw()   -->
<!-- ``` -->

The relationship between the Contros and calculated pCO2 is not good at all in the sense that it is far from the 1:1 line, despite a good r2 (`r round(fit$rsquare, digits=2)`).
<!-- >>>>>>> cb6a0f5891d0a4e43735bed5507c62760ae133cb -->
