...
seafet_calib_all <- sf_calc(calEint = d_all$voltINT, calEext = d_all$voltEXT, E0int25 = d_all$E0int25_sf,  E0ext25 = d_all$E0ext25_sf, calT = d_all$temp_sf, calSal = d_all$sal_insitu_ctd_interp)
...
# split by depth
d_all <- d_all %>%
  dplyr::mutate(pHint_tot_sf_all_1m = ifelse(depth > 0 & depth < 2, pHint_tot_sf_all, NA),
                pHint_tot_sf_all_3m = ifelse(depth >= 2 & depth < 4, pHint_tot_sf_all, NA),
                pHint_tot_sf_all_5m = ifelse(depth >= 4 & depth < 6, pHint_tot_sf_all, NA),
                pHint_tot_sf_all_7m = ifelse(depth >= 6 & depth < 8, pHint_tot_sf_all, NA),
                pHint_tot_sf_all_10m = ifelse(depth >= 8 , pHint_tot_sf_all, NA),
...
# split by depth
d_all <- d_all %>%
  dplyr::mutate(pHint_tot_sf_10m_1m = ifelse(depth > 0 & depth < 2, pHint_tot_sf_10m, NA),
                pHint_tot_sf_10m_3m = ifelse(depth >= 2 & depth < 4, pHint_tot_sf_10m, NA),
                pHint_tot_sf_10m_5m = ifelse(depth >= 4 & depth < 6, pHint_tot_sf_10m, NA),
                pHint_tot_sf_10m_7m = ifelse(depth >= 6 & depth < 8, pHint_tot_sf_10m, NA),
                pHint_tot_sf_10m_10m = ifelse(depth >= 8 , pHint_tot_sf_10m, NA),
                depth_discrete = case_when(
...
From now on, one uses seaFET pH from the internal electrode calibrated with spec pH and seaFET data when the REMOS was below 8 m (pHint_tot_sf_10m). This unique pH variable is called pHint_tot_sf. All other pH variables are cleaned up
...
d_all <- d_all %>%
  dplyr::rename(
    ...
    "pHint_tot_sf_", "pHext_", "pHint_tot_sf_all"
  )))
