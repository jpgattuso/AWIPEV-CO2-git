...
d_all <- d_all %>%
  dplyr::mutate(
    ...
    #mol/(l/atm)
    f_air_sea = k * alpha * delta_pco2 /1000 # mmol/(m2 h)
  )
...
air_sea_day <- d_all %>%
  ...
  dplyr::summarise(f_air_sea = 24 * mean(f_air_sea, na.rm = TRUE)) %>% #nmol/(m2 d)
  ungroup() %>%
  dplyr::rename(day = 1)
...
d_all <- d_all %>%
  dplyr::mutate(
    ...
    #mol/(l/atm)
    f_air_sea_corr = k * alpha * delta_pco2_corr /1000 # mmol/(m2 h)
  )
...
`r round(sum(air_sea_corr_month$f_air_sea_corr), digits = 0)` mmol/(m2
