...
g
```
The relationship between pCO2 between 0 and 4 m on the REMOS (estimated from pH and S-derived AT) and pCO2 measured at 11 m (both pCO2 at CTD temperature) is much nicer using monthly median values (b) than using all data (a). The mean and median difference between the two pCO2s are

```{r, echo=FALSE, message= FALSE, warning=FALSE}
knitr::kable(summary(tmp$delta_pco2_fb_remos_monthly))
```

### Calculation of fluxes
```{r With Contros data, echo=FALSE, message= FALSE, warning=FALSE}
...
```{r With in situ simulation, echo=FALSE, message= FALSE, warning=FALSE}
