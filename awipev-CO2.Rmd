...
# # read files made available by Philipp with  quality-controlled data
# https://doi.org/10.1594/PANGAEA.929583
# pg_data("10.1594/PANGAEA.929583")
...
from ctd (> 8 m). New column "sal_mix" is created and is the one used in
ESSD paper.
...
m). New column "temp_mix" is created. For the moment, no gaps are
encountered and temp_mix = temp_insitu_11m.
...
`r #round(summary(fit)$r.squared, digits=2)`. Need to compare with other
...
Alliouane. Measurments were performed several months after sampling.
...
Offset between calibrated seaFET data and pH spectro. The offset must be between -0.2 and 0.2 pH units. "differences between calibrated sensor pH values and values obtained from discrete, check samples can be expected to be as small as 0.02 pH units, and perhaps less." (McLaughlin et al. 2017).
...
pH INT - pH EXT was plotted here. From now on, one uses seaFET pH from the internal electrode calibrated
with spec pH and seaFET data when the REMOS was below 8 m (pHint_tot_sf_10m). This unique pH variable is called pHint_tot_sf. All other pH variables are cleaned up. We also calculate pHint_tot_sf. at 4°C: pHint_tot_sf_temp_4.
...
tmp <- d_all %>%
  dplyr::select(datetime, depth, at, sal_insitu_ctd, temp_sf, pHint_tot_sf) %>%
  tidyr::drop_na() %>%
  dplyr::filter(datetime < as.POSIXct("2018-01-01") | datetime > as.POSIXct("2018-02-03"))
tmp$pHint_tot_sf_temp_4 <- seacarb::pHinsi(pH=tmp$pHint_tot_sf, ALK=mean(tmp$at*1e-6, na.rm=TRUE), Tinsi=4, Tlab=tmp$temp_sf, Pinsi=tmp$depth/10, S=tmp$sal_insitu_ctd, Pt=0, Sit=0, k1k2 = "l", kf = "dg", ks="d", pHscale = "T", b="u74")
d_all <- d_all %>% 
...
```{r seafet profiles, eval=FALSE, echo=FALSE, message= FALSE, warning=FALSE, out.width="80%",fig.show='hold', fig.align="center"}
...
tmp <- d_all %>%
  dplyr::select(datetime, at, sal_fb, temp_fb, pH_dur) %>%
  tidyr::drop_na()
tmp$pH_dur_temp_4 <- seacarb::pHinsi(pH=tmp$pH_dur, ALK=mean(tmp$at*1e-6, na.rm=TRUE), Tinsi=4, Tlab=tmp$temp_fb, Pinsi=11/10, S=tmp$sal_fb, Pt=0, Sit=0, k1k2 = "l", kf = "dg", ks="d", pHscale = "T", b="u74")
d_all <- d_all %>% 
...
### **seaFET / durafet pHs (all > 8 m)**

```{r pHs comparaison 4°C, echo=FALSE, message= FALSE, warning=FALSE}
# prepare data and remove sf biofouling period
# TODO consider biofouling period elsewhere too
#   pH_sf <- d_all %>%
#   dplyr::select(datetime, sal_insitu_ctd_10m, temp_insitu_ctd_10m, pHint_tot_sf_temp_4, depth_group) %>%
#   #tidyr::drop_na() %>%
#   dplyr::filter(datetime < as.POSIXct("2018-01-01") | datetime > as.POSIXct("2018-02-03")) %>% 
#   dplyr::mutate(pHint_tot_sf_temp_4_10m = ifelse(depth_group == ">8", pHint_tot_sf_temp_4, NA))
# 
...
# 
# pH_dur <- d_all %>%
#   dplyr::select(datetime, sal_fb,temp_fb, pH_dur)%>%
#   tidyr::drop_na() %>%
# pH_dur$pH_dur_temp_4 <- pHinsi(pH=pH_dur$ph_dur_corr, ALK=mean(d_all$at, na.rm=TRUE), Tinsi=4, Tlab=pH_dur$temp_fb, Pinsi=11/10, S=pH_dur$sal_fb, Pt=0, Sit=0, k1k2 = "l", kf = "dg", ks="d", pHscale = "T", b="u74")
# 
# # add pH sf and pH dur normalized at 4°C ti d_all
# #d_all <- left_join(d_all, pH_sf%>%dplyr::select(datetime, pHint_tot_sf_temp_4 ), by="datetime")
# d_all <- d_all %>% 
#   left_join(pH_dur %> %dplyr::select(datetime, pH_dur_temp_4), by="datetime")
lm <- lm2(x = d_all$pH_dur_temp_4, y = d_all$pHint_tot_sf_temp_4)
p <- d_all %>% 
  dplyr::select(pH_dur_temp_4, pHint_tot_sf_temp_4, monthd) %>% 
  drop_na() %>% 
  ggplot(aes(x = pH_dur_temp_4, y = pHint_tot_sf_temp_4))+
  geom_point(aes(color=factor(monthd))) + 
  geom_abline(slope=1, intercept=0, linetype = "dashed")+
  geom_abline(aes(intercept = lm$intercept_MA, slope = lm$slope_MA), colour = "blue") +
  coord_fixed(ratio = 1 , xlim=c(7.8, 8.35), ylim=c(7.8, 8.35)) +
  labs(title = paste("","\n Adj R2 = ", signif(lm$rsquare, 3),
                     "; Intercept =", signif(lm$intercept_MA, 3),
                     "; Slope =", signif(lm$slope_MA, 3),
                     "; P =", signif(lm$P, 3),
...
# Spectro / durafet
lm <- lm2(x = d_all$ph_s_dur_temp_fb, y = d_all$pH_dur)
p <- d_all %>% 
  dplyr::select(ph_s_dur_temp_fb, pH_dur, monthd) %>% 
  drop_na() %>% 
  ggplot(aes(x = ph_s_dur_temp_fb, y= pH_dur, label = monthd)) +
  geom_text( aes(color=factor(monthd))) + 
  scale_color_discrete(guide = "none")+
  geom_abline(slope=1, intercept =0, linetype = "dashed") +
...
p <- d_all %>% 
  dplyr::select(ph_s_sf_temp_insi, pHint_tot_sf, monthd) %>%
  tidyr::drop_na() %>%
  ggplot(aes(x=ph_s_sf_temp_insi, y= pHint_tot_sf, label = monthd)) +
  geom_text( aes(color=factor(monthd))) + 
  scale_color_discrete(guide = "none")+
  geom_abline(slope=1, intercept = 0, linetype = "dashed") +
  geom_abline(aes(intercept = lm$intercept_MA, slope = lm$slope_MA),
  ...
  theme_bw() +
  theme(aspect.ratio = 1, plot.title = element_text(size=7)) + 
  coord_fixed(ratio = 1 , xlim=c(8, 8.25), ylim=c(8, 8.25))+
...
been flaged "3" by the SNAPO. After a close look, these samples are not
differents from their duplicates. Then we put the flag to "2" (good
result).
...
pdelta_ct_snapo_dygraph <- dygraph(ct_diff_xts, group = "awipev", main="Discrete CT (SNAPO) measurements", ylab="CT") %>%
  ...
  dyAxis("y", axisLineColor="red",label = "", axisLabelColor ="red", axisLineWidth=5) %>%
    dyAxis("y2", axisLineColor="blue", axisLabelColor = "blue", axisLineWidth=5) %>%
       dyLegend(show = "follow")   %>%
    dyHighlight(highlightCircleSize = 8,highlightSeriesBackgroundAlpha = 0.5,hideOnMouseOut = TRUE) %>%
    dyOptions(drawGrid = TRUE, drawPoints = TRUE, pointSize = c(1,8,1), digitsAfterDecimal=2) %>%
    dyRangeSelector(height = 30, dateWindow= NULL) 
...
FerryBox were used to calculate pH using the formulations for K1 and K2
of Lueker et al. (2000) and Sulpis et al. (2020). The differences
induced by the choice of the carbonic acid constants are:

-   large for pH (`r round(diff_ls_quant[4, 2], digits = 3)` pH units on
    average; ; median is `r round(diff_ls_quant[3, 2], digits = 3)`)
    compared with the uncertainty calculated with seacarb
    (`r round(const_choice[1, 6], digits = 3)`). In general, pH is
    higher with Sulpis than with Lueker.
-   small for CT (`r round(diff_ls_quant[4, 3], digits = 1)` umol/kg on
    average; always lower with Sulpis than with Lueker) compared with
    the uncertainty calculated with seacarb
    (`r round(const_choice[1, 8], digits = 1)`)
...
`r round(diff_table_pco2_at[4, 4], digits=3)` pH units. This difference
is `r abs(round(diff_table_pco2_at[4, 4]/errors_pco2_at$pH, digits=1))`
times larger than the uncertainty for pH calculated from pCO2 and AT
according to seacarb (`r round(errors_pco2_at$pH, digits=3)`).
...
temp_delta_plt <- tmp_delta %>% 
  ...
  geom_col(fill="blue") +
  geom_hline(yintercept = 0) +
  annotate("text", x = 1.5, y = 0.3, label = substitute("T"["surface"] ~ "> T"["deep"])) +
...
pHinsi_delta_plt <- tmp_delta %>% 
  ...
  geom_col(fill="blue") +
  geom_hline(yintercept = 0) +
  annotate("text", x = 1.5, y = 0.02, label = substitute("pH"["surface"] ~ "> pH"["deep"])) +
  annotate("text", x = 1.5, y = -0.06, label = substitute("pH"["surface"] ~ "< pH"["deep"])) +
  Mytheme(size_labs = 12) +
  theme(panel.grid.major.x = element_blank(), panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank())
...
pH_delta_plt <- tidyr::drop_na(tmp_delta) %>% 
  ...
  #labs(y = substitute("pH"["T"] ~ "at in situ temp."), x = "Month") +
  scale_x_discrete("Month", expand = c(0, 0)) +
  scale_y_continuous(substitute("pH"["T"] ~ "at 4°C"), limits = c(-0.2, 0.1), expand = c(0, 0)) +
  geom_col(fill="blue") +
  geom_hline(yintercept = 0) +
...
# all deltas
deltas_plt <- sal_delta_plt / temp_delta_plt / den_delta_plt / pH_delta_plt
deltas_plt <- deltas_plt + plot_annotation(tag_levels = 'a')
print(deltas_plt)
ggsave(filename = "figures/deltas.png", plot = deltas_plt, width = 15, height = 20, units = "cm")
```
...
**Conclusion for pH**: The range is significantly larger during some months
when the data deeper than 8 m are considered. Also, pH is sometimes
higher at the surface, at other times it is higher on the bottom (June).
No data are available at 1 m (actually 0 to 2 m). So, to compare surface
and bottom, I combine 1 and 3 m (0 to 4 m) and 9 m (below 8 m).
...
For seasonal analysis, December to February was considered winter, March
to May was considered spring, June to August was considered summer, and
...
`r round(sum(air_sea_month$f_air_sea), digits = 0)` mol/(m2 year).
...
pangaea <- as_tibble(d_all) %>% 
  dplyr::select(datetime, depth, sal_insitu_ctd, sal_fb, sal_mix, temp_insitu_11m, temp_insitu_ctd, temp_fb, temp_sf, temp_dur, at, ct, ph_s_sf_temp_insi, ph_s_dur_temp_fb, ph_calc_fb, pco2_fb, pco2_calc_fb, at_calc, pH_dur, pHint_tot_sf) %>%
...
-   sal_mix: when ferrybox data (sal_fb) are missing, they are replaced
    by data from the profiling ctd for depths > 8 m (sal_insitu_ctd_10m)

-   sal_mix_interp: interpolated salinity sal_mix. It is used to have as
    many spec pH to display (some are during missing data)

-   temp_mix: when data of in situ temperature at 11 m (temp_insitu_11m)
    are missing, they are replaced by data from the profiling ctd for
    depths > 8 m

-   temp_mix_interp: interpolated temperature temp_mix. It is used to
