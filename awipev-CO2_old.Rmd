...
pH_durafet_mean <- pH_durafet %>%
  ...
  dplyr::summarize(ph_s_dur_temp_fb_mean = mean(ph_s_dur_temp_fb, na.rm = TRUE),
                   ...
                   ph_s_dur_temp_4_mean = sd(ph_s_dur_temp_4, na.rm = TRUE))
...
#                    )
# >>>>>>> 0d3b58ee3abb01c904f165290b5c76bfe7270c1f
...
## Takahashi thermal vs biological
```{r Takahashi thermal vs biological, include=FALSE}
# Select data
taka <- d_all %>% # just for this chunk
  dplyr::select(datetime, monthd, sal_fb, temp_insitu_11m, pco2_fb_insi_temp) 

# In order to remove the temperature effect from the observed pCO2, the observed pCO2 values are normalized to a constant temperature (annual mean) -- Takahashi et al. (2002)
temp_mean <- mean(taka$temp_insitu_11m, na.rm = TRUE)
taka <- taka %>% 
  dplyr::mutate(pco2_mean_temp = pco2_fb_insi_temp * exp(0.0423 * (temp_mean - temp_insitu_11m)))
d_pco2_bio <- max(taka$pco2_mean_temp, na.rm = TRUE) - min(taka$pco2_mean_temp, na.rm = TRUE)
d_pco2_temp <- max(taka$pco2_fb_insi_temp, na.rm = TRUE) - min(taka$pco2_fb_insi_temp, na.rm = TRUE)
temp_bio <- d_pco2_temp / d_pco2_bio
```
