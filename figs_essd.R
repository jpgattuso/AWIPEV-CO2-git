...
ts_at <- d %>%
  ...
  ggplot() +
  geom_point(aes(x = datetime, y = at_calc), col = "blue", size = 0.15) +
  scale_x_datetime(limits = xlim) +
...
ts_oa <- d %>%
  ...
  ggplot() +
  geom_point(aes(x = datetime, y = oa), col = "blue", size = 0.15) +
  scale_x_datetime(limits = xlim) +
...
cowplot::ggsave2(filename = "figures/ts_gg.png", plot = cp, height = 20, units = "cm")
...
pco2_bp <- ggplot(filter(d_long, variable=="pco2_corr"), aes(x = month, y = value, group=month)) +
  ...
  mytheme_bp()
ph_bp <- ggplot(filter(d_long, variable=="pHint_tot_sf"), aes(x = month, y = value, group=month)) +
  geom_violin(fill='lightblue', alpha=0.5, size=0.2, trim = TRUE) +
  geom_boxplot(notch=TRUE, size=0.2, fill="grey50", outlier.color = "blue", outlier.size = 0.3) +
  scale_x_discrete(breaks=c("Jan", "", "Mar", "", "May", "", "Jul", "", "Sep", "", "Nov", "")) +
...
p <-  ggplot(d, aes(x=pco2_calc_fb, y= pco2_corr)) +
  ...
  labs(x = expression(paste("Calculated pC", O[2], " (", mu, "atm)")), y = expression(paste("In situ pC", O[2], " (", mu, "atm)"))) + 
  mytheme(size_labs = 10) +
  theme(aspect.ratio=1, plot.title = element_text(size=7))
ggsave(file="figures/pco2.png", p,  width = 14, height = 14, units = "cm")
