...
temp_sal <-  ggplot() +
  ...
  labs(title="Temperature vs salinity, blue lines show density", x="Salinity", y="Temp. (°C)") +
  ...
  geom_contour(data = grid, aes(x = s, y = t, z = z), 
               stat = "contour", binwidth=1, show.legend=FALSE) +
  #stat_contour(aes(colour=..level..), breaks=seq(1025, 1028, by=1)) +
  coord_fixed() +
...
```{r temperature vs salinity regressions, fig.width=8, fig.height=8, echo=FALSE, message= FALSE}
...
fig_reg_temp_sal <- tmp_reg_temp_sal %>%
  ggplot() +
  geom_point(aes(x=date, y=Slope, color=month), size=5) +
