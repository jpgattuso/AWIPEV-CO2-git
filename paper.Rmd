...
d_all <- readRDS(file = paste0(path_data, "fb_data/d_all_copy.rds"))
d <- d_all %>% 
  as_tsibble(index = datetime) %>% 
...
pc_na <- function(x=df) {
  NAs <- as.tibble(map_df(x, function(x) sum(!is.na(x)))) %>% # number of NAs
    dplyr::select(-datetime)
  NAs <- as.data.frame(t(NAs))
  n <- nrow(x) # total number of rows
  pc_na <- n_na/n #fraction of NAs
  NAs$pc <- pc_na$V1
  return(NAs)
}
tmp <- d %>% 
  as_tibble() %>% 
  dplyr::select(pH_s_seafet_temp_ctd, par_air_filtered, pco2_contros, pH_s_seafet_temp_ctd, starts_with("sal_"),
                starts_with("temp_"), at_calc, year_month, quarter) %>%
  summarize()



tmp visdat::vis_miss(tmp)
gg_miss_var(tmp, facet = quarter)
