...
```{r water masses, fig.width=10, fig.height=10, echo=FALSE, message= FALSE}
...
temp_sal <- d_all %>%
  ...
  geom_point(aes(colour=factor(month)), na.rm=TRUE, size=size_point) + 
  labs(title="Temperature vs salinity SBE45",x="Salinity", y="Temp. (°C)") +
  facet_wrap(~ year) +
  stat_smooth(method=lm, na.rm=TRUE, show.legend=TRUE, se=FALSE, aes(fill=factor(month), colour=factor(month))) +
  coord_fixed() +
  Mytheme(size_labs = 8) 
  #scale_color_viridis(name="Month", discrete=FALSE, option="plasma")
print(temp_sal)
