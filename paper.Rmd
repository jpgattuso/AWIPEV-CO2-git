...
We explore the use of a TS diagram to investigate changes in water sources. Now need to calculate slopes and r2, and see whether some months have a consistently different behaviour.

```{r temperature vs salinity, fig.width=10, fig.height=10, echo=FALSE, message= FALSE}
# One tries to identify water masses coming from the Atlantic or freshwater
den <- rho(S = s, T = t, P = 1)
s <- c(seq(30.4, 35.3, by=0.1)) # pCO2
t <- seq(-2, 8, by=0.1) #temperature throughout the year
grid <- as_tibble(expand.grid(s, t)) %>%
  rename(s=Var1, t=Var2) %>%
  mutate(z=rho(S = s, T = t, P = 1))


tmp <- ggplot(data = z, 
              aes(x=s, y=s, z = z), na.rm = TRUE) +
  geom_contour()
...
temp_sal <-  ggplot() +
  geom_point(data = d, aes(x = sal_fb, y = temp_insitu, fill=month), na.rm=TRUE,
             size=size_point) + 
  labs(title="Temperature vs salinity SBE45", x="Salinity", y="Temp. (°C)") +
  facet_wrap(~ year, ncol = 5) +
  stat_smooth(data = d, aes(x = sal_fb, y = temp_insitu, color=month),
              method=lm, na.rm=TRUE, show.legend=TRUE, se=FALSE) +
  geom_contour(data = d, aes(x = s, y = t, z = z)) +
  coord_fixed() +
