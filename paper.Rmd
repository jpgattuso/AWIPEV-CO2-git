...
**To do**: Filter pH values at more than 8 m depth.
...
ts_pH <- d_all %>%
  ...
  scale_x_datetime(breaks = date_breaks("1 year"), labels = date_format("%Y")) +
  geom_point(colour="blue", alpha=alpha, na.rm=TRUE, size=size_point) + 
  labs(title="In situ pH",x="", y=expression(paste(pH[T]))) +
  Mytheme(size_labs = 8)
...
```{r temperature vs salinity regressions, echo=FALSE, message= FALSE, out.width="50%"}
