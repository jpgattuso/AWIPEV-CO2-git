...
d_ts <- d %>%
  #dplyr::mutate(pHint_tot_sf = ifelse(depth >= 8, pHint_tot_sf, NA)) %>% # keep only seafet data below 8 m depth
  dplyr::select(datetime, temp_insitu_11m, sal_fb, par_air, pco2_corr,
...
ts_pCO2 <- d_all %>%
  ggplot() +
  scale_x_datetime(breaks = date_breaks("1 year"), labels = date_format("%Y")) +
  geom_point(aes(x = datetime, y = pco2_corr), 
